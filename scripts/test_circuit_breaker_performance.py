...
try:
    from claude_mpm.core.socketio_pool import CircuitBreaker, CircuitState, SocketIOConnectionPool
    POOL_AVAILABLE = True
...
class CircuitBreakerPerformanceTest:
    ...
    def test_failure_threshold(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'circuit_opens_at_threshold': failure_states[4]['circuit_state'] == 'open',  # 5th failure (index 4)
    ...
    def test_recovery_timeout(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'circuit_opened_correctly': open_state == 'open' and not open_can_execute,
                'remained_closed_during_timeout': during_timeout_state == 'open' and not during_timeout_can_execute,
                'transitioned_to_half_open': after_timeout_state == 'half_open' and after_timeout_can_execute,
    ...
    def test_half_open_recovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'half_open_working': half_open_state == 'half_open' and half_open_can_execute,
                'success_recovery_working': success_recovery_state == 'closed' and success_recovery_can_execute,
                'failure_recovery_working': failure_recovery_state == 'open' and not failure_recovery_can_execute
            }
    ...
    def test_integration_with_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dropped_events = [
                {'namespace': '/hook', 'event': 'dropped1', 'data': {'test': 'dropped'}},
                {'namespace': '/hook', 'event': 'dropped2', 'data': {'test': 'dropped'}},
            ]
            ...
            result = {
                ...
                'circuit_opened_on_failures': circuit_state_after_failures in ['open', 'half_open'],
                'events_dropped_when_open': after_drop_stats['batch_queue_size'] <= mid_stats.get('batch_queue_size', 0),
                'circuit_attempted_recovery': final_circuit_state == 'half_open' or 
                                             (final_circuit_state == 'closed' and circuit_state_after_failures == 'open'),
        ...
        for test_name, result in self.results.items():
            if result.get('status') == 'completed':
                ...
                if test_name == 'failure_threshold':
                    if result.get('threshold_respected') and result.get('circuit_opens_at_threshold'):
                        summary['circuit_breaker_features'].append('Failure threshold working correctly (5 failures)')
                    else:
                        summary['issues_found'].append('Failure threshold not working as expected')
                
                elif test_name == 'recovery_timeout':
                    if result.get('timeout_respected') and result.get('transitioned_to_half_open'):
                        summary['circuit_breaker_features'].append('Recovery timeout working correctly')
                    else:
                        summary['issues_found'].append('Recovery timeout not working as expected')
                
                elif test_name == 'half_open_recovery':
                    if (result.get('half_open_working') and 
                        result.get('success_recovery_working') and 
                        result.get('failure_recovery_working')):
                        summary['circuit_breaker_features'].append('HALF_OPEN state transitions working correctly')
                    else:
                        summary['issues_found'].append('HALF_OPEN state recovery not working properly')
                
                elif test_name == 'fail_fast_behavior':
                    if result.get('fail_fast_working') and result.get('consistently_fast'):
                        summary['circuit_breaker_features'].append('Fail-fast behavior working correctly')
                    else:
                        summary['issues_found'].append('Fail-fast behavior not working as expected')
                
                elif test_name == 'integration_with_pool':
                    if result.get('integration_working') and result.get('circuit_opened_on_failures'):
                        summary['circuit_breaker_features'].append('Integration with connection pool working')
                    else:
                        summary['issues_found'].append('Circuit breaker integration issues detected')
