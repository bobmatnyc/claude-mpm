...
class MemoryMonitor:
    ...
    def __init__(self, process_id: Optional[int] = None, enable_logging: bool = True):
        ...
        self.warnings_issued: Dict[int, bool] = {t: False for t in self.WARNING_THRESHOLDS}
    ...
    def measure(self, label: str = "") -> Dict[str, float]:
        ...
        # Log measurement
        logger.info(f"Memory measurement{' - ' + label if label else ''}: {current_memory:.2f}MB")
    ...
    def continuous_monitor(self, interval: float = 1.0, duration: Optional[float] = None):
        """
...
def main():
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run test memory allocation"
    )
