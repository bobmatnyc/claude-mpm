...
def fix_dashboard_metrics():
    ...
    base_path = Path(
        "/Users/masa/Projects/claude-mpm/src/claude_mpm/dashboard/static/built"
    )
    ...
    fix_pattern = (
        r'(this\.eventViewer=new EventViewer\("events-list",this\.socketClient\))'
    )
    ...
    with open(event_viewer_file) as f:
        content = f.read()
    ...
    fix_pattern4 = (
        r"(updateDisplay\(\)\{this\.updateEventTypeDropdown\(\),this\.applyFilters\(\))"
    )
    ...
    with open(socket_client_file) as f:
        content = f.read()
