...
async def verify_hook_details():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            structured_data = await page.query_selector('[data-testid*="structured"], .structured-data, text=/structured data/i')
            if structured_data:
            ...
            event_properties = [
                'Hook Name', 'Event Type', 'Tool', 'Session ID',
                'pre_tool', 'post_tool', 'subagent_stop', 
                'PlaywrightTest', 'Read', 'Bash'
            ]
            ...
            print(f"  Hook events visible: {'YES' if has_hook_events else 'NO'} ({len(hook_events)} found)")
            print(f"  Event details available: {'YES' if has_details else 'NO'} ({len(found_properties)} properties)")
            print(f"  EventBus relay status: {'✅ WORKING' if has_hook_events else '❌ NEEDS FIX'}")
            ...
            if has_hook_events and has_details:
                ...
                print("   - Event details show hook metadata (tool names, session IDs, etc.)")
                print("   - EventBus relay is successfully delivering hook events to UI")
            else:
