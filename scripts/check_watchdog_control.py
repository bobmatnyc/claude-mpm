...
def find_watchdog_control():
    ...
    try:
        result = subprocess.run(
            ["grep", "-n", "AgentModificationTracker\\|modification_tracker", "-r", "src/"],
            capture_output=True,
            text=True,
            cwd=Path.cwd(),
            check=False
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-n", "-A5", "-B5", "args.monitor\\|--monitor", "src/claude_mpm/cli/commands/run.py"],
            capture_output=True,
            text=True,
            cwd=Path.cwd(),
            check=False
        ...
        if result.stdout:
            ...
            if "watchdog" in result.stdout.lower() or "observer" in result.stdout.lower():
