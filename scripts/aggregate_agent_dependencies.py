...
class DependencyAggregator:
    ...
    def __init__(self, project_root: Path, dry_run: bool = False, verbose: bool = False):
        ...
        self.agent_paths = [
            ...
            project_root / "src" / "claude_mpm" / "agents" / "templates"  # SYSTEM level
        ]
    ...
    def resolve_version_conflicts(self, dependencies: Dict[str, List[str]]) -> Dict[str, str]:
        """
            ...
            try:
                ...
                for spec_set in specifier_sets:
                    for spec in spec_set:
                        if spec.operator in (">=", "==", ">"):
                            if not highest_min or version.parse(spec.version) > version.parse(highest_min.split(">=")[-1]):
