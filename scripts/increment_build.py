...
def get_current_build_number(build_file: Path) -> int:
    ...
    try:
        ...
        print(f"Warning: Could not read build number from {build_file}: {e}", file=sys.stderr)
        return 0
...
def write_build_number(build_file: Path, build_number: int) -> None:
    ...
    try:
        ...
        print(f"Error: Could not write build number to {build_file}: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force increment regardless of changes'
    )
    parser.add_argument(
        '--staged-only',
        action='store_true',
