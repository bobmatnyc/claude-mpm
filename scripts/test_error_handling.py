...
def test_invalid_data_handling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_cases = [
            ...
            ("Valid request", "Valid response", {"agent": "test", "large_data": "Z" * 50000}),  # 50KB metadata
            ...
            ("Request with émojis 🚀🔥", "Response with spéciál chärs ñ", {"agent": "tëst_ågënt"}),
        ...
        return {
            ...
            "graceful_handling": len(results["errors"]) == 0,  # Should handle without exceptions
            "success": len(results["errors"]) < len(test_cases) * 0.1  # Allow up to 10% exception rate
        }
...
def test_concurrent_error_scenarios():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(200):  # Enough to overflow queue
                try:
                    ...
                    success = logger.log_response(
                        request,
                        response,
                        {"agent": f"thread_{thread_id}", "index": i}
                    )
        ...
        return {
            ...
            "logger_stats": stats,
            "graceful_degradation": results["exceptions"] < total_requests * 0.05,  # <5% exception rate
            "success": results["successful"] > 0 and results["exceptions"] < total_requests * 0.1
        }
...
def test_recovery_after_errors():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(10):
            try:
                success = logger.log_response(
                    ...
                    "X" * 50000 if i % 5 == 0 else f"Response {i}",  # Some very large responses
        ...
        return {
            ...
            "success": normal_logs >= 18 and phase2_stats['errors'] == 0  # Should recover cleanly
        }
