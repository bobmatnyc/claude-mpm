    ...
    async def start_monitoring(self):
        ...
        async def claude_event(data):
            ...
            if 'prompt_text' in event_data:
                prompt = event_data['prompt_text'][:50] + "..." if len(event_data['prompt_text']) > 50 else event_data['prompt_text']
            ...
            if data.get('subtype') == 'heartbeat':
                ...
                if active_sessions:
                    ...
                    for session in active_sessions:
                        print(f"   - {session.get('agent', 'pm')} session: {session.get('session_id', 'unknown')[:8]}...")
