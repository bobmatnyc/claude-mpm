...
class SocketIOMonitor:
    ...
    async def start_monitoring(self, duration: int = 60):
        ...
        logger.info(
            f"🔍 Starting Socket.IO monitoring on port {self.port} for {duration} seconds..."
        )
    ...
    def _setup_monitoring_hooks(self):
        ...
        async def connect(sid):
            connection_info = {
                ...
                "sid": sid,
                "client_count": len(
                    self.socket_server.sio.manager.get_participants("/", "/")
                ),
            }
        ...
        async def monitored_emit(
            ...
            **kwargs,
            ...
            if event != "ping":  # Skip ping spam
                logger.info(
                    f"📡 Emitting '{event}' to {room or to or 'all'} ({emission_info['data_size']} chars)"
                )

            # Call original emit
            return await original_emit(
                event, data, to, room, skip_sid, namespace, callback, **kwargs
            )
        ...
        while time.time() - start_time < duration and self.monitoring:
            ...
            if time.time() - last_report >= 10:
                ...
                connections_count = len(
                    [
                        c
                        for c in self.connections_log
                        if c["event"] == "client_connected"
                    ]
                )

                logger.info(
                    f"⏱️  Status: {elapsed}s elapsed, {remaining}s remaining | "
    ...
    def generate_report(self) -> str:
        ...
        total_connections = len(
            [c for c in self.connections_log if c["event"] == "client_connected"]
        )
        current_connections = sum(
            1 if c["event"] == "client_connected" else -1 for c in self.connections_log
        )
...
"""
        ...
        for event_name, count in sorted(
            emissions_by_event.items(), key=lambda x: x[1], reverse=True
        ):
...
"""
            for event_type, count in sorted(
                emissions_by_type.items(), key=lambda x: x[1], reverse=True
            ):
...
async def main():
    ...
    parser.add_argument(
        "--generate-test",
        action="store_true",
        help="Generate test events during monitoring",
    )
...
"""
    ...
    try:
        ...
        if args.generate_test:
            ...
            await asyncio.gather(monitoring_task, generate_periodic_test_events())
        else:
