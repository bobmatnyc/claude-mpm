...
def update_agent_json(file_path: Path, reduction_stats: Dict) -> None:
    ...
    if 'optimization_note' not in agent_data['metadata']:
        agent_data['metadata']['optimization_note'] = f"Optimized template - reduced from {orig_lines} to {new_lines} lines"
    ...
    # Track statistics
    reduction_pct = ((orig_lines - new_lines) / orig_lines * 100) if orig_lines > 0 else 0
...
def main():
    ...
    for agent_file in priority_agents:
        ...
        try:
            ...
            print(f"   ✅ Reduced from {stats.get('original', '?')} to {stats.get('optimized', '?')} lines ({stats.get('reduction', '?')})")
    ...
    for agent, stats in reduction_stats.items():
        print(f"{agent:25} {stats['original']:4} → {stats['optimized']:4} lines ({stats['reduction']})")
