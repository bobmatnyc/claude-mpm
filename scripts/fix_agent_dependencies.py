...
def main():
    ...
    parser.add_argument(
        "--specific-agent",
        help="Only check/fix dependencies for a specific agent"
    )
    ...
    if missing_python:
        ...
        if compatible:
            print(f"Installing {len(compatible)} compatible packages with retry logic...")
            print()
            ...
            if not final_missing:
                ...
                print(f"⚠️  Still missing {len(final_missing)} packages after installation")
                if args.verbose:
    ...
    final_results = loader.analyze_dependencies()
    if final_results['summary']['missing_python'] or final_results['summary']['missing_system']:
        return 1
    return 0
