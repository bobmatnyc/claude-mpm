...
def update_method_calls():
    ...
    manager_path = Path("/Users/masa/Projects/claude-mpm/src/claude_mpm/services/agents/memory/agent_memory_manager.py")
    content = manager_path.read_text()
    
    # Replace method calls
    replacements = [
        (r'self\._parse_memory_list\(', 'self.format_service.parse_memory_list('),
        (r'self\._clean_template_placeholders_list\(', 'self.format_service.clean_template_placeholders_list('),
        (r'self\._clean_template_placeholders\(', 'self.format_service.clean_template_placeholders('),
        (r'self\._build_simple_memory_content\(', 'self.format_service.build_simple_memory_content('),
        (r'self\._categorize_learning\(', 'self.categorization_service.categorize_learning('),
        (r'self\._parse_memory_sections\(', 'self.format_service.parse_memory_sections('),
    ]
    ...
    print(f"✓ Updated method calls to use services")
            ...
            if f"def {method}(" in lines[i]:
                ...
                while j < len(lines):
                    if lines[j].strip():  # Non-empty line
                        ...
                        if current_indent <= indent and lines[j].strip().startswith('def '):
