...
class SocketIODebugConsumer:
    ...
    def _subscribe_to_events(self):
        ...
        hook_events = [
            ...
            "hook.pre_response", "hook.post_response",
        ...
        # System events
        system_events = [
            "system.heartbeat", "system.status",
        ...
        logger.info(f"✅ Subscribed to {len(hook_events + system_events + agent_events + test_events)} event types")
    ...
    def _determine_source(self, event_name: str) -> str:
        ...
        elif event_name.startswith("test."):
            return EventSource.TEST.value
        elif event_name.startswith("agent.") or event_name.startswith("subagent."):
            return EventSource.AGENT.value
        elif event_name in ["system.heartbeat", "session.started", "session.ended"]:
    ...
    async def _async_emit(self, event_data: Dict[str, Any]):
        ...
        if self.server.sio:
            ...
            logger.debug(f"📤 Emitted to Socket.IO: claude_event")
