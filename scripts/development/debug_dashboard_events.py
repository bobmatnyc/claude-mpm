...
class DashboardEventDebugger:
    ...
    def _setup_event_monitoring(self):
        ...
        async def monitor_eventbus_event(event: Event):
            self.events_generated.append({
                ...
                'event_type': f"{event.type}.{event.subtype}" if hasattr(event, 'subtype') else event.type,
                'data_keys': list(event.data.keys()) if hasattr(event, 'data') and event.data else []
            })
            logger.info(f"📨 EventBus event: {event.type}.{getattr(event, 'subtype', '')}")
        ...
        original_emit = None
        if hasattr(self.socket_server, 'sio') and hasattr(self.socket_server.sio, 'emit'):
    ...
    async def generate_test_events(self, duration: int = 30):
        ...
        while time.time() < end_time:
            for event_type, subtype in event_types:
                ...
                try:
                    ...
                    event_data = self._generate_event_data(event_type, subtype, event_count)
                    ...
                    logger.info(f"📨 Generated {event_type}.{subtype} event #{event_count}")
        ...
        logger.info(f"✓ Test event generation completed. Generated {event_count} events.")
    
    def _generate_event_data(self, event_type: str, subtype: str, count: int) -> Dict[str, Any]:
        ...
        if event_type == "hook":
            ...
            return {
                **base_data,
                'todos': [
                    {'id': f'todo_{count}', 'content': f'Test todo #{count}', 'status': 'pending'}
                ]
            }
    ...
    async def monitor_dashboard_connection(self, duration: int = 30):
        ...
        if hasattr(self.socket_server, 'sio'):
            ...
            if not clients:
                logger.warning("⚠️  No dashboard clients connected! Please open http://localhost:8765 in your browser.")
        ...
        while time.time() - start_time < duration:
            ...
            logger.info(f"⏱️  Monitoring... {elapsed}s elapsed, {len(self.events_generated)} events generated, {len(self.events_emitted)} events emitted")
...
"""
        ...
        if len(self.events_emitted) == 0:
            ...
            report += "   ❌ ONLY HEARTBEAT EVENTS - EventBus events not reaching Socket.IO\n"
        else:
...
async def main():
    ...
    parser.add_argument('--test-duration', type=int, default=30, help='Test duration in seconds (default: 30)')
