...
def check_page_content(url, page_name):
    ...
    try:
        ...
        has_no_data_message = any(indicator in page_text for indicator in no_data_indicators)
        ...
        if 'agents' in url.lower():
            ...
            if 'Research Agent' in page_text or 'Engineer Agent' in page_text or 'Project Manager' in page_text:
                has_data = True
            ...
            tool_names = ['Read', 'Write', 'Edit', 'Bash', 'Grep', 'WebSearch', 'TodoWrite']
            ...
            file_indicators = ['dashboard.js', 'main.css', 'README.md', 'test_dashboard.py']
        ...
        if has_data and non_zero_stats and not has_no_data_message:
            ...
            if stat_values:
                print(f"   - Stats found: {[s.get_text().strip() for s in stat_values[:5]]}")
