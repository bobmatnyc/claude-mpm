...
def test_method(name, command, cwd=None):
    ...
    try:
        ...
        if "Server instance created successfully" in stderr or "MCP Gateway Server" in stderr:
            print(f"  ✓ PASSED - Server started successfully")
            return True
        elif "Failed to import" in stderr or "FATAL ERROR" in stderr:
            print(f"  ✗ FAILED - Import or startup error")
            error_lines = [l for l in stderr.split('\n') if 'Error' in l or 'Failed' in l]
            for line in error_lines[:3]:
                print(f"    {line}")
            return False
        else:
            print(f"  ? UNCERTAIN - Server started but status unclear")
            return True
