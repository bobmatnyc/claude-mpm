...
def backup_original_handler():
    ...
    backup_file = src_file.with_suffix(
        f".py.backup_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    )
...
def install_eventbus_handler():
    ...
    src_file = (
        Path(__file__).parent.parent
        / "src/claude_mpm/hooks/claude_hooks/hook_handler_eventbus.py"
    )
    dest_file = (
        Path(__file__).parent.parent
        / "src/claude_mpm/hooks/claude_hooks/hook_handler.py"
    ...
    print("✅ Installed EventBus hook handler")
    return True
...
def rollback(backup_file):
    ...
    print("✅ Rolled back to original hook handler")
    return True
...
def main():
    ...
    print("You may need to manually check the installation")
