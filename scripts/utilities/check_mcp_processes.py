        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'create_time', 'memory_info']):
            try:
                ...
                if cmdline and any('mcp' in str(arg).lower() for arg in cmdline):
                    ...
                    if any('claude-mpm' in str(arg) or 'mcp_wrapper' in str(arg) 
                           or 'stdio_server' in str(arg) for arg in cmdline):
    ...
    def display_processes(self, verbose: bool = False):
        ...
        if not mcp_processes:
            ...
            for details in process_table:
                ...
                print(f"  {pid:<7} {memory:<9} {cpu:>4.1f}% {duration:<9} {status:<9} {cmd}")
        ...
        stale_pids = [info for info in pid_file_info 
