    ...
    async def generate_test_events(self, count: int = 10, interval: float = 2.0):
        ...
        for i in range(count):
            ...
            try:
                ...
                print(f"📤 Emitted test event #{i + 1}: {test_event['type']}.{test_event['subtype']}")
    ...
    async def generate_hook_events(self, count: int = 5):
        ...
        for i, event_template in enumerate(hook_events[:count]):
            ...
            try:
                ...
                print(f"📤 Emitted hook event #{i + 1}: {event['type']}.{event['subtype']}")
...
async def main():
    ...
    parser.add_argument('--count', type=int, default=10, help='Number of test events (default: 10)')
    parser.add_argument('--interval', type=float, default=2.0, help='Interval between events in seconds (default: 2.0)')
    parser.add_argument('--hook-events', action='store_true', help='Generate hook-style events instead of generic test events')
...
""")
    ...
    try:
        ...
        if args.hook_events:
            ...
            await generator.generate_test_events(count=args.count, interval=args.interval)
