...
class MCPConfigRestorer:
    ...
    def __init__(self, config_path: Optional[Path] = None, app_type: str = "both"):
        ...
        if app_type in ["desktop", "both"]:
            if config_path and app_type == "desktop":
                ...
                if system == "Darwin":  # macOS
                    self.configs["desktop"] = Path.home() / "Library" / "Application Support" / "Claude" / "claude_desktop_config.json"
                elif system == "Linux":
                    self.configs["desktop"] = Path.home() / ".config" / "Claude" / "config.json"
                elif system == "Windows":
                    self.configs["desktop"] = Path.home() / "AppData" / "Roaming" / "Claude" / "claude_desktop_config.json"
                else:
                    self.configs["desktop"] = Path.home() / ".config" / "Claude" / "config.json"
        ...
        if system == "Darwin":  # macOS
            self.backup_locations.extend([
                Path.home() / "Library" / "Application Support" / "Claude" / "backups",
                Path.home() / ".config" / "Claude" / "config.json.backup",
                Path.home() / ".config" / "claude-desktop" / "claude_desktop_config.json.backup",
            ])
        elif system == "Linux":
            self.backup_locations.extend([
                Path.home() / ".config" / "claude-desktop" / "claude_desktop_config.json.backup",
        ...
        for location in self.backup_locations:
            if location.is_file():
                ...
                for backup_file in location.glob("*.json*"):
                    if "backup" in backup_file.name or backup_file.parent.name == "backups":
    ...
    def restore_from_backup(self, backup_path: Optional[Path] = None, merge_all: bool = False) -> bool:
        ...
        if backup_path:
            ...
            try:
                choice = input(f"\nSelect backup to restore (1-{len(backups) + 1}): ").strip()
    ...
    def show_comparison(self) -> None:
        ...
        if backups:
            ...
            for path, config in backups:
                ...
                print(f"\n   {path.name}:")
                print(f"     Servers: {', '.join(servers.keys()) if servers else '(none)'}")
                if unique_in_backup:
...
def main():
    ...
    parser.add_argument(
        "--merge",
        action="store_true",
        help="Merge all available backups"
    )
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Compare current config with backups"
    )
    parser.add_argument(
        "--config-path",
        type=Path,
        help="Custom path to Claude Desktop config.json"
    )
