...
def test_experimental_warning():
    ...
    if "Memory Guardian is an experimental feature that is currently disabled" in result.stdout:
        print("✅ Command correctly reports feature is disabled")
    else:
        print("⚠️  Command did not report feature as disabled (may be enabled in config)")
    ...
    result = subprocess.run(
        [sys.executable, "-m", "claude_mpm.cli", "run-guarded", "--accept-experimental", "--help"],
        capture_output=True,
        text=True,
        env=env
    )
