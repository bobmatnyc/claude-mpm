...
from claude_mpm.utils.log_cleanup import LogCleanupUtility, LogCleanupConfig
...
def print_statistics(stats: dict, title: str = "Log Directory Statistics"):
    ...
    if stats.get('oldest_log'):
        print(f"Oldest log: {stats['oldest_log']['name']} "
...
def print_cleanup_summary(summary: dict):
    ...
    if total_impact > 0:
        ...
        if ops.get('space_saved_mb', 0) > 0:
            print(f"  - Space saved by compression: {format_size(ops['space_saved_mb'])}")
...
"""
    ...
    parser.add_argument(
        '--logs-dir',
        type=Path,
        help='Base log directory (default: .claude-mpm/logs)'
    )
    ...
    ops_group.add_argument(
        '--session-dirs',
        action='store_true',
        help='Clean old session directories'
    )
    ...
    any_operation = any([
        args.all, args.session_dirs, args.archived_logs,
        args.old_logs, args.empty_dirs, args.compress
    ])
    ...
    if any_operation:
        if args.all:
            ...
            if args.archived_logs:
                ...
                if not args.quiet and not args.json:
                    print(f"Archives: Removed {removed} files, "
                          f"freed {format_size(space)}")

            if args.old_logs:
                removed, space = cleaner.cleanup_old_logs(
                    args.log_days, args.dry_run
                )
                operations['logs'] = {'removed': removed, 'space_mb': space}
                if not args.quiet and not args.json:
                    print(f"Logs: Removed {removed} files, "
                          f"freed {format_size(space)}")
            ...
            if args.compress:
                ...
                if not args.quiet and not args.json:
                    print(f"Compression: Compressed {compressed} files, "
