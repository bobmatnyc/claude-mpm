...
def validate_agent():
    ...
    template_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "templates" / "agentic_coder_optimizer.json"
    ...
    for field, expected in required_fields.items():
        if field not in template:
            ...
            warnings.append(f"Field {field} has value '{template[field]}', expected '{expected}'")
        else:
    ...
    if metadata.get("name") != "Agentic Coder Optimizer":
        warnings.append(f"Metadata name is '{metadata.get('name')}', expected 'Agentic Coder Optimizer'")
    else:
    ...
    if deploy_path.exists():
        ...
        with open(deploy_path, "r") as f:
            ...
            if "Agentic Coder Optimizer" in content:
                ...
                warnings.append("Deployment file exists but may not contain correct content")
    else:
        warnings.append(f"Agent not deployed to project: {deploy_path}")
    
    # 6. Check agent is in metadata
    metadata_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "agents_metadata.py"
