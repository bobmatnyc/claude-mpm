...
async def test_http_endpoints():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Test directory API
        try:
            async with session.get('http://localhost:8765/api/directory/list?path=.') as resp:
                if resp.status == 200:
                    data = await resp.json()
...
async def test_socketio_connection():
    ...
    async def code_file_analyzed(data):
        print(f"  ✅ Received code:file:analyzed: {len(data.get('elements', []))} elements")
    ...
    try:
        ...
        expected_events = ['connect', 'connection_test', 'git_branch_response', 'code_file_analyzed']
...
async def test_mock_ast_analysis():
    ...
    async def code_file_analyzed(data):
        ...
        print(f"  ✅ AST analysis result received:")
        ...
        for elem in data.get('elements', []):
            print(f"    - {elem.get('type')}: {elem.get('name')} (line {elem.get('line')})")
            if elem.get('methods'):
                for method in elem['methods']:
                    print(f"      - method: {method.get('name')} (line {method.get('line')})")
