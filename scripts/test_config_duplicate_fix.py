...
def test_single_success_message():
    ...
    assert config1 is config2 is config3, "Config instances should be the same (singleton)"
    ...
    if len(success_messages) == 0:
        ...
        print(f"✗ FAILURE: {len(success_messages)} success messages logged (expected 0 or 1)")
        return False
...
def test_with_explicit_config_file():
    ...
    if config_file.exists():
        ...
        if len(success_messages) == 1:
            ...
            print(f"✗ FAILURE: {len(success_messages)} success messages logged (expected 1)")
            return False
    else:
