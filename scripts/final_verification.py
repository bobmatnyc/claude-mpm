...
async def verify_pipeline():
    ...
    if hasattr(server, 'eventbus_integration'):
        ...
        print(f"    ✓ EventBus Integration: {'Active' if integration.is_active() else 'Inactive'}")
    ...
    try:
        ...
        test_scenarios = [
            ("hook.pre_tool", {"tool_name": "Read", "file_path": "/test.txt", "sessionId": "test-123"}),
            ("hook.post_tool", {"tool_name": "Write", "result": "success", "sessionId": "test-123"}),
            ("hook.subagent_stop", {"agent_type": "Engineer", "result": "complete", "sessionId": "test-123"}),
        ]
        ...
        if len(received_events) >= len(test_scenarios):
            ...
            print(f"    - Expected {len(test_scenarios)} events, received {len(received_events)}")
