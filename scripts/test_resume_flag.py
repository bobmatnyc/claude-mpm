...
from claude_mpm.cli.parsers.base_parser import preprocess_args, create_parser
...
def test_resume_flag_parsing():
    ...
    test_cases = [
        ...
        (['run', '--', '--resume'],
         'Claude native --resume without session ID'),
         
        # Test case 3: --mpm-resume (MPM flag)
        (['run', '--mpm-resume', 'session456'],
         'MPM --mpm-resume with session ID'),
...
def test_actual_command():
    ...
    cmd = [sys.executable, '-m', 'claude_mpm.cli', 'run', '--debug', '--', '--resume', 'test123', '--dry-run']
