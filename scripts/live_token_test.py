...
class SocketIOTestClient:
    ...
    def setup_handlers(self):
        ...
        def on_connect():
            self.connected = True
            print(f"✅ Connected to Socket.IO server at {datetime.now().strftime('%H:%M:%S')}")

        @self.sio.on('disconnect')
        def on_disconnect():
            self.connected = False
            print(f"❌ Disconnected from Socket.IO server at {datetime.now().strftime('%H:%M:%S')}")
        ...
        def on_session_event(data):
            ...
            if data.get('type') == 'token_usage_updated' or data.get('subtype') == 'token_usage_updated':
                print(f"   🎯 TOKEN USAGE EVENT!")
...
def check_build_version():
    ...
    if os.path.exists(chunks_dir):
        ...
        if result.returncode == 0:
            ...
            print(f"   ❌ 'token_usage_updated' NOT found in chunk files")
        ...
        for i, event in enumerate(client.events_received, 1):
            ...
            if event['event'] == 'session_event':
                ...
                if data.get('type') == 'token_usage_updated':
                    print(f"      ✅ Token usage event received!")
