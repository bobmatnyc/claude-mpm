...
def main():
        ...
        try:
            ...
            if result["had_duplicate"]:
                ...
                if validate_json(filepath):
                    print(
                        f"✓ Removed version '{result['version_value']}' from {result['filename']}"
                    )
                else:
                    print(
                        f"✗ ERROR: Invalid JSON after processing {result['filename']}"
                    )
    ...
    print(
        f"Agents with duplicate version removed: {sum(1 for r in results if r['had_duplicate'])}"
    )
    print(
        f"Agents without duplicate version: {sum(1 for r in results if not r['had_duplicate'])}"
    )

    # Verify version fields
    print("\nVersion field verification:")
    print(
        f"  - Templates with agent_version: {sum(1 for r in results if r['has_agent_version'])}"
    )
    print(
        f"  - Templates with template_version: {sum(1 for r in results if r['has_template_version'])}"
    )
    print(
        f"  - Templates with schema_version: {sum(1 for r in results if r['has_schema_version'])}"
    )
