...
class ResponseLoggingHealthChecker:
    ...
    def _check_configuration_loading(self) -> None:
        ...
        try:
            ...
            if not response_logging:
                ...
                session_dir = response_logging.get('session_directory', '.claude-mpm/responses')
    ...
    def _check_session_logger(self) -> None:
        ...
        try:
            config = Config()
            response_logging = config.get('response_logging', {})
            session_dir = Path(response_logging.get('session_directory', '.claude-mpm/responses'))
            ...
            log_path = logger.log_response('test request', 'test response', test_entry, 'health_check')
                ...
                if not os.access(session_dir, os.W_OK):
                    ...
                    if self.auto_fix:
                        try:
                            ...
                            self.fixes_applied.append(f"Fixed permissions for {session_dir}")
                        except Exception as e:
...
def main():
    ...
    # Run health check
    checker = ResponseLoggingHealthChecker(
