...
def simulate_command_building(resume=False, extra_args=None, launch_method="exec"):
    ...
    if args.resume:
        ...
        if "--resume" not in raw_claude_args:
            ...
            print(f"   ✅ Added --resume to beginning")
        else:
            print(f"   ℹ️ --resume already present")
    ...
    print(f"   Base command: ['claude', '--model', 'opus', '--dangerously-skip-permissions']")
    ...
    if "--resume" in cmd:
        ...
        print(f"   Command will resume Claude's last conversation")
    else:
        print(f"   ❌ FAILURE: --resume NOT in final command!")
        print(f"   Claude will start a new conversation")
    
    # Step 6: Launch method impact
    print(f"\n6️⃣ Launch Method Impact ({launch_method}):")
    if launch_method == "exec":
        print(f"   • Using os.execvpe() - replaces current process")
        print(f"   • Command: {cmd}")
        print(f"   • All arguments passed directly to Claude")
    else:
        print(f"   • Using subprocess - runs as child process")
        print(f"   • Command: {cmd}")
        print(f"   • Arguments passed via subprocess.run()")
...
def main():
    ...
    parser.add_argument(
        "extra_args",
        nargs="*",
        help="Additional arguments to test"
    )
