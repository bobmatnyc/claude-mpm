...
class DocumentationAuditor:
    ...
    def __init__(self, project_root: Path, verbose: bool = False):
        ...
        self.expected_structure = {
            "docs/": {
                ...
                "required_files": [],
                "subdirs": {
                    "archive/": {"type": "directory", "description": "Historical documentation, QA reports"},
                    "assets/": {"type": "directory", "description": "Documentation assets (images, diagrams)"},
                    "dashboard/": {"type": "directory", "description": "Dashboard documentation"},
                    "design/": {"type": "directory", "description": "Design documents and technical specifications"},
                    "developer/": {"type": "directory", "description": "Developer documentation (API, internals, guides)"},
                    "examples/": {"type": "directory", "description": "Usage examples and configurations"},
                    "qa/": {"type": "directory", "description": "QA reports and test documentation"},
                    "user/": {"type": "directory", "description": "User-facing documentation"}
                }
            }
        }
        ...
        self.subdir_pattern = r'^[0-9]+-[a-z][a-z-]*$|^[a-z][a-z-]*$'  # lowercase with hyphens
    ...
    def audit(self) -> AuditSummary:
        ...
        if not self.docs_root.exists():
            self.add_issue("structure", "error", str(self.docs_root), None, 
    ...
    def add_issue(self, category: str, severity: str, file_path: str, 
                  line_number: Optional[int], message: str, suggestion: Optional[str] = None,
                  fixable: bool = False):
        """Add an audit issue"""
        issue = AuditIssue(
            category=category,
    ...
    def _audit_structure(self):
        ...
        for missing_dir in missing_dirs:
            self.add_issue(
                "structure", "warning", f"docs/{missing_dir}", None,
        ...
        for unexpected_dir in unexpected_dirs:
            ...
            if re.match(r'^[0-9]+-', unexpected_dir.rstrip('/')):
                continue
            self.add_issue(
                "structure", "info", f"docs/{unexpected_dir}", None,
        ...
        for subdir in self.docs_root.iterdir():
            if subdir.is_dir():
                if not re.match(self.subdir_pattern, subdir.name):
                    self.add_issue(
                        "naming", "warning", str(subdir), None,
    ...
    def _audit_duplicate_content(self):
        ...
        for md_file in self.docs_root.rglob("*.md"):
            ...
            try:
                ...
                self.add_issue(
                    "content", "warning", str(md_file), None,
        ...
        for content_hash, file_list in content_hashes.items():
            if len(file_list) > 1:
                self.add_issue(
                    "duplicates", "warning", file_list[0], None,
                ...
                for line_num, line in enumerate(lines, 1):
                    for pattern in self.deprecated_patterns:
                        if re.search(pattern, line, re.IGNORECASE):
                            self.add_issue(
                                "deprecated", "warning", str(md_file), line_num,
                    ...
                    for link_text, link_url in links:
                        ...
                        if link_url.startswith('./') or link_url.startswith('../') or not link_url.startswith('/'):
                            ...
                            if not target_path.exists():
                                self.add_issue(
                                    "links", "error", str(md_file), line_num,
            ...
            for pattern, expected_locations in placement_rules.items():
                if re.search(pattern, md_file.name, re.IGNORECASE):
                    ...
                    if not in_correct_location:
                        self.add_issue(
                            "placement", "warning", file_path, None,
    ...
    def _audit_redundant_content(self):
        ...
        for md_file in self.docs_root.rglob("*.md"):
            try:
                ...
                header_keywords = [
                    word.lower() for header in header_keywords 
    ...
    def fix_issues(self) -> int:
        ...
        for issue in self.issues:
            ...
            try:
                if issue.category == "structure" and "Create directory" in issue.suggestion:
...
def main():
    ...
    parser.add_argument(
        ...
        help='Fail on any issues (for CI/CD)'
    )
    parser.add_argument(
        '--project-root', type=Path, default=Path(__file__).parent.parent,
    ...
    if not (project_root / "docs" / "STRUCTURE.md").exists():
        ...
        print("Please run from the project root or specify --project-root", file=sys.stderr)
        sys.exit(1)
