...
On first startup after upgrading, `claude-mpm` removes redundant project-level copies of bundled
skills from each project's `.claude/skills/` directory, deferring to the user-level copy in
`~/.claude/skills/`. A project-level skill is only removed when **both** conditions are true:
