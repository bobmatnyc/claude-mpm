...
- [Bundled vs. Optional (External) Agents — Where to Contribute Changes](#bundled-vs-optional-external-agents--where-to-contribute-changes)
...
## Bundled vs. Optional (External) Agents — Where to Contribute Changes
...
- **Skills**: `src/claude_mpm/skills/bundled/` (e.g., `code-review.md`, `git-workflow.md`)
- **System agents**: Framework agent templates and instructions in `src/claude_mpm/agents/` (e.g., `PM_INSTRUCTIONS.md`, `BASE_AGENT.md`)
- **Openrouter agent**: `src/claude_mpm/agents/openrouter_code_reviewer.md` — a single bundled agent example

These are part of the MPM codebase and are released with every version of claude-mpm.

**2. Optional / external agents** — sourced from the `bobmatnyc/claude-mpm-agents` repository

- Located in the external repo at `agents/` (organized by category, e.g., `universal/`, `qa/`, etc.)
- Cached locally at `~/.claude-mpm/cache/agents/bobmatnyc/claude-mpm-agents/`
- Deployed into a project at `.claude/agents/` (e.g., `code-analyzer.md`, `code-critic.md`)

These are NOT part of the claude-mpm package; they are fetched separately and can be updated independently.

### Deployed Files Are Build Artifacts

Files in `.claude/agents/` and `.claude/skills/` are **deployment artifacts**, not sources:

- Generated by the build process at CLI startup
- Automatically overwritten on the next rebuild or sync
- **Never edit them directly** — changes will be lost

### Contribution Decision Tree

Use this table to determine where to make changes:

| Scenario | Source Location | Procedure |
|----------|-----------------|-----------|
| **Improve a bundled skill** (e.g., `code-review`, `git-workflow`) | `src/claude_mpm/skills/bundled/<category>/<skill>.md` | Edit in claude-mpm repo, commit, and release in next version |
| **Improve an external agent** (e.g., `code-analyzer`, `code-critic`) | `bobmatnyc/claude-mpm-agents` repo (agents/ directory) | Edit in claude-mpm-agents repo, merge, then sync in projects that use it |
| **Fix PM system instructions** (e.g., core PM guidance) | `src/claude_mpm/agents/PM_INSTRUCTIONS.md`, `WORKFLOW.md`, etc. | Edit in claude-mpm repo, commit, and release in next version |
| **Customize for a single project** | `.claude/agents/` or `.claude/skills/` in the project | For one-off testing only; redeploy will overwrite. For permanent changes, contribute upstream |

### How to Tell Which Is Which

**Machine-readable provenance — the `source` frontmatter field**

Agent definitions carry a `source` field in their YAML frontmatter that identifies where they originate:

| Value | Meaning |
|-------|---------|
| `bundled` | The definition ships inside the `claude-mpm` package (e.g., `src/claude_mpm/agents/openrouter_code_reviewer.md`). Changes belong in this repo and are released with the package. |
| `external` | The definition is owned by `bobmatnyc/claude-mpm-agents` and synced/deployed separately. Changes belong in that repo. |
| _(absent)_ | Provenance is unknown — typically user-created agents or older definitions that predate this field. |

This field is optional and validated at deploy time. An invalid value (e.g. `source: vendor`) is a hard validation error. The `agents list` command shows the `source` column in both text and table output so provenance is visible at a glance.
...
source: bundled
description: Reviews code using OpenRouter-hosted models.
---
```

**File existence fallback**: If `source` is absent, the file-system check is still reliable:

```bash
# Check if it's bundled
find /path/to/claude-mpm/src/claude_mpm -name "<agent-or-skill>.md" -o -name "<agent-or-skill>.json"

# If found: it's bundled → edit in claude-mpm/src/...
# If not found: it's external → edit in bobmatnyc/claude-mpm-agents and sync
```
...
- **Bundled changes**: Commit in claude-mpm, release a new version. Users running `claude-mpm init` or `pip install --upgrade` will get the new definitions.
- **External changes**: Commit in claude-mpm-agents, then users re-sync the agent cache via `claude-mpm init` or explicit `fetch` commands to pull the latest definitions.

### Precedence: Where Agents Are Found

If an agent definition exists in multiple places, deployment precedence is:

1. **Project-local** — `.claude/agents/<name>.md` (project-specific override)
2. **User cache** — `~/.claude-mpm/cache/agents/`  (synced external agents)
3. **System/framework** — bundled agents in the package

This allows projects and users to override or pin specific agents as needed.
