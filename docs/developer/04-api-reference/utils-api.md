...
3. [Subprocess Runner](#subprocess-runner)
4. [Enhanced Subprocess Runner](#enhanced-subprocess-runner)
...
Basic subprocess execution utility.
...
class SubprocessRunner:
    """Utility for running subprocesses with timeout and logging."""
...
## Enhanced Subprocess Runner

Advanced subprocess runner with real-time output streaming and better control.

### Location
`src/claude_mpm/utils/enhanced_subprocess_runner.py`

### EnhancedSubprocessRunner Class

```python
class EnhancedSubprocessRunner:
    """Enhanced subprocess runner with streaming and callbacks."""
...
- `on_stdout` (Optional[Callable]): Callback for stdout lines
- `on_stderr` (Optional[Callable]): Callback for stderr lines
- `timeout` (Optional[float]): Timeout in seconds
...
**Parameters:**
- `command` (List[str]): Command to run
- `input_handler` (Optional[Callable]): Function to get user input
...
```python
def terminate_process(self, process: subprocess.Popen, timeout: float = 5.0)
def kill_process_tree(self, pid: int)
def get_process_info(self, process: subprocess.Popen) -> Dict[str, Any]
```
