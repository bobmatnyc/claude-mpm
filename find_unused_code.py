...
class UnusedCodeFinder:
        ...
        for file_path, imports in self.imports.items():
            ...
            for import_name in imports:
                ...
                if base_name not in file_usages and import_name not in file_usages:
                    ...
                    if base_name not in content.replace(f"import {import_name}", "").replace(f"from {import_name}", ""):
                        unused.append({
                            'file': str(file_path.relative_to(self.root_path)),
                            'import': import_name
                        })
    ...
    def find_dead_code_patterns(self):
        ...
        for file_path, content in self.file_contents.items():
            ...
            for i, line in enumerate(lines, 1):
                stripped = line.strip()
                if stripped.startswith('#') and len(stripped) > 10:  # Likely commented code
                    if not in_block:
                        in_block = True
                        block_start = i
            ...
            for i, line in enumerate(lines, 1):
                if re.match(r'^\s*(def|class)\s+\w+', line):
                    ...
                    non_empty = [l for l in next_lines if l.strip() and not l.strip().startswith('"""') and not l.strip().startswith("'''") and l.strip() != 'pass']
                    if len(non_empty) <= 1:
                        empty_defs.append(i)
            ...
            if todos or commented_blocks or empty_defs:
                dead_patterns.append({
                    ...
                    'total_issues': len(todos) + len(commented_blocks) + len(empty_defs)
                })
...
def main():
    ...
    for item in results['dead_code_patterns'][:15]:
        print(f"  {item['file']}: {item['todos']} TODOs, {item['commented_blocks']} comment blocks, {item['empty_definitions']} empty defs")
