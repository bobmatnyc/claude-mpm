...
from claude_mpm.validation.agent_validator import (
    AgentValidator,
...
class AgentMigrator:
    ...
    def migrate_agent(self, old_agent: Dict[str, Any], filename: str) -> Dict[str, Any]:
        ...
        clean_id = self.AGENT_ID_MAPPING.get(
            agent_type, agent_type.replace("_agent", "")
        )

        # Extract instructions
        instructions = (
            old_agent.get("narrative_fields", {}).get("instructions", "")
            or old_agent.get("instructions", "")
            or ""
        )
        ...
        if len(instructions) > 8000:
            logger.warning(
                f"Truncating instructions for {clean_id} from {len(instructions)} to 8000 characters"
            )
        ...
        new_agent = {
            ...
            "metadata": {
                ...
                "description": config.get(
                    "description", f"{clean_id.title()} agent for Claude MPM"
                ),
        ...
        if any(
            tool in new_agent["capabilities"]["tools"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate agent templates to new schema format"
    )
    parser.add_argument(
        "--templates-dir",
        type=Path,
