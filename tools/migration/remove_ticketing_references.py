...
def remove_ticketing_references(content):
    ...
    content = re.sub(
        r"\*\*Ticket Reference\*\*: \[ISS-XXXX if applicable\]\n", "", content
    )
    ...
    content = re.sub(
        r"## Ticketing Guidelines\n\n.*?(?=##|\Z)", "", content, flags=re.DOTALL
    )

    # Remove ticket references from delegation templates
    content = re.sub(
        r"- Always include ticket reference in delegation:.*\n", "", content
    )
    ...
    content = re.sub(
        r"📝 Code Review Summary for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"🐛 Bug Fix Complete for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"📦 Deployment Summary for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"🔄 Migration Summary for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"🔌 API Integration Complete for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"🔒 Security Finding for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
        r"📋 Compliance Status for ISS-\d+:.*?```", "", content, flags=re.DOTALL
    )
    content = re.sub(
