...
from claude_mpm.services.async_session_logger import LogFormat, get_async_logger
...
class ClaudeMPMIntegration:
    ...
    async def process_request_async(self, agent: str, request: str) -> str:
        """
        ...
        context.data.update(
            {"response": response, "duration_ms": duration_ms, "success": True}
        )
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, self.process_request_async(agent, request)
                )
    ...
    async def process_batch_async(self, requests: list) -> list:
        ...
        tasks = [
            self.process_request_async(req["agent"], req["request"]) for req in requests
        ]
...
async def demonstrate_async_performance():
    ...
    requests = [{"agent": "research", "request": f"Query {i}"} for i in range(10)]
