...
async def demonstrate_health_monitoring():
    ...
    network_checker = NetworkConnectivityChecker(
        host="localhost", port=8765, timeout=1.0
    )
...
async def demonstrate_recovery_system():
    ...
    scenarios = [
        ...
        (
            ...
            [HealthMetric("cpu_usage", 75, HealthStatus.WARNING)],
        ),
        (
            "Critical condition",
            HealthStatus.CRITICAL,
            [HealthMetric("memory_usage", 95, HealthStatus.CRITICAL)],
        ),
    ]
...
async def demonstrate_circuit_breaker():
    ...
    cb = CircuitBreaker(failure_threshold=3, timeout_seconds=2, success_threshold=2)
    ...
    status = cb.get_status()
    print(
        f"After timeout: State = {status['state']}, Can proceed = {status['can_proceed']}"
    )
    print()
...
async def demonstrate_integrated_scenario():
    ...
    print(
        f"  Total recoveries: {recovery_status['recovery_stats']['total_recoveries']}"
    )
