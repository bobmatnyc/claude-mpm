    ...
    def check_file(self, file_path: Path) -> Tuple[bool, str]:
        ...
        if self.name == "python_scripts_in_root":
            ...
            if parent_dir == Path():
                return (
                    False,
                    f"Python script '{file_path.name}' should not be in project root",
                )
        ...
        if self.name == "shell_scripts_in_root":
            ...
            if parent_dir == Path():
                return (
                    False,
                    f"Shell script '{file_path.name}' should not be in project root",
                )
            return True, ""  # Files in subdirectories are fine
        ...
        if self.name == "test_files_misplaced":
            ...
            if not str(relative_path).startswith("tests/"):
                return (
                    False,
                    f"Test file '{file_path.name}' should be in /tests/ directory",
                )
        ...
        if self.name == "old_release_notes_in_root":
            ...
            if parent_dir == Path():
                return (
                    False,
                    f"Historical release notes file '{file_path.name}' should be in docs/release-notes/ directory",
                )
            return True, ""  # Files in subdirectories are fine
...
class StructureLinter:
    ...
    def _report_violations(self):
        ...
        for rule_name, violations in by_rule.items():
            print(
                f"📋 {rule_name.replace('_', ' ').title()} ({len(violations)} violations):"
            )
            for v in violations:
                print(f"  ❌ {v['file']}")
                print(f"     {v['message']}")
    ...
    def _can_auto_fix(self, violation: Dict) -> bool:
        ...
        return violation["rule"] in [
            ...
            "shell_scripts_in_root",
            "test_files_misplaced",
    ...
    def _auto_fix_violation(self, violation: Dict) -> bool:
        ...
        try:
            ...
            if (
                violation["rule"] == "python_scripts_in_root"
                or violation["rule"] == "shell_scripts_in_root"
            ):
    ...
    def _check_version_consistency(self):
        ...
        if versions.get("VERSION"):
            ...
            if changelog_path.exists():
                content = changelog_path.read_text()
                # Match both formats: ## v5.4.7 and ## [5.4.7]
                version_pattern = rf"##\s+(?:v|)\[?{re.escape(versions['VERSION'])}\]?"
...
def main():
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to automatically fix violations"
    )
