...
def check_todowrite_format():
    ...
    agent_template_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "BASE_AGENT_TEMPLATE.md"
    agent_content = agent_template_path.read_text()
    
    # Read BASE_PM.md
    pm_template_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "BASE_PM.md"
    ...
    if "TODO (High Priority):" in agent_content or "TODO (Medium Priority):" in agent_content:
