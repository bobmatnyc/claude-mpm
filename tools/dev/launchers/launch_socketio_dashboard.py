...
)  # Go up to project root from src/claude_mpm/scripts/
...
Examples:
  ...
  python launch_socketio_dashboard.py --setup-only      # Just create files
    ...
    parser.add_argument(
        "--port", type=int, default=3000, help="Socket.IO server port (default: 3000)"
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Run server in background mode"
    )
    parser.add_argument(
