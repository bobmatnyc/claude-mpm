...
def enhance_hud_debugging():
    ...
    hud_visualizer_path = (
        project_root / "src/claude_mpm/dashboard/static/js/components/hud-visualizer.js"
    )
    ...
    hud_manager_path = (
        project_root / "src/claude_mpm/dashboard/static/js/components/hud-manager.js"
    )
    ...
    if "[HUD-MANAGER-DEBUG]" in content:
        ...
        if (
            "Retrieved ${allEvents ? allEvents.length : 0} events from event viewer"
            not in content
        ):
            ...
            replacement = new_session_check.replace(
                "        // Get all events (not just filtered ones) to build complete tree structure\n        const allEvents = this.eventViewer.getAllEvents();",
                "",
            )
