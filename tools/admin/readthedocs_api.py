...
class ReadTheDocsClient:
    ...
    def activate_version(self, version_slug: str, active: bool = True) -> Dict[str, Any]:
    ...
    def create_redirect(self, from_url: str, to_url: str, redirect_type: str = "page") -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--project",
        default="claude-mpm",
        help="Project slug on Read the Docs"
    )
    ...
    version_parser.add_argument("--active", action="store_true", help="Show only active versions")
    ...
    build_parser.add_argument("--wait", action="store_true", help="Wait for build to complete")
    ...
    try:
        ...
        if args.command == "info":
            ...
            for version in versions:
                ...
                print(f"[{active}] {version.get('slug', 'N/A')} - {version.get('ref', 'N/A')}")
            ...
            if args.wait:
                ...
                if final_status == "finished":
                    print(f"Documentation available at: https://claude-mpm.readthedocs.io/en/{args.version}/")
            ...
            if redirects:
                for redirect in redirects:
                    print(f"{redirect.get('from_url')} → {redirect.get('to_url')} ({redirect.get('type')})")
            else:
