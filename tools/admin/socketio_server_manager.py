...
class ServerManager:
    ...
    def start_server(self, port: int = None, server_id: str = None, 
        ...
        if existing_server:
            ...
            if management_style == 'daemon':
                print(f"💡 To stop daemon server: {self.project_root / 'bin' / 'socketio-daemon'} stop")
            else:
    ...
    def stop_server(self, port: int = None, server_id: str = None) -> bool:
        ...
        print(f"💡 Try using the bin/socketio-daemon stop command if this is a daemon-managed server")
        return False
    ...
    def status(self, verbose: bool = False) -> None:
        ...
        if not running_servers:
            ...
            print(f"  • Start with daemon: {self.project_root / 'bin' / 'socketio-daemon'} start")
            return
        ...
        for server in running_servers:
            ...
            if verbose:
                ...
                if management_style == 'daemon':
                    print(f"   Stop command: {self.project_root / 'bin' / 'socketio-daemon'} stop")
                else:
