...
"""
    ...
    assert not project_pm_file.exists() or project_pm_file.stat().st_mtime < user_pm_file.stat().st_mtime, \
...
"""
    ...
    assert project_eng_file.exists(), "Engineer memory should be saved to project directory"
    ...
    assert old_pm_file.exists() or backup_file.exists(), "Old PM file should be backed up, not deleted"
    ...
    assert "PM always coordinates multi-agent workflows" in pm_memory_from_other or \
           "PM Memory" in pm_memory_from_other, \
...
def test_memory_hook_integration():
    ...
    assert "PM coordinates all agent interactions" in pm_memory or \
           "PM uses Task tool for delegation" in pm_memory, \
