...
class OutputStyleTester:
    ...
    def create_manager_with_mock_version(self, version: str = "1.0.85") -> OutputStyleManager:
    ...
    def test_fresh_installation(self) -> bool:
        ...
        assert settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should be set to claude-mpm"
    ...
    def test_existing_style_replacement(self) -> bool:
        ...
        assert actual_content == new_content, f"Content should be replaced. Expected: {new_content}, Got: {actual_content}"
        
        # Check settings were updated
        settings = json.loads(settings_file.read_text())
        assert settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should be updated to claude-mpm"
    ...
    def test_settings_activation_from_different_style(self) -> bool:
        ...
        settings = json.loads(settings_file.read_text())
        assert settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should be changed to claude-mpm"
        assert settings.get("otherSetting") == "preserve-me", "Other settings should be preserved"
        assert settings.get("theme") == "dark", "Existing settings should be preserved"
        
        logger.info("  ✓ activeOutputStyle changed from user-custom-style to claude-mpm")
    ...
    def test_missing_settings_file(self) -> bool:
        ...
        settings = json.loads(settings_file.read_text())
        assert settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should be set"
    ...
    def test_multiple_runs_idempotent(self) -> bool:
        ...
        assert second_settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should still be claude-mpm"
        ...
        assert third_settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should still be claude-mpm"
    ...
    def test_corrupted_settings_file(self) -> bool:
        ...
        settings = json.loads(settings_file.read_text())
        assert settings.get("activeOutputStyle") == "claude-mpm", "activeOutputStyle should be set correctly"
    ...
    def test_version_support_check(self) -> bool:
        ...
        manager = self.create_manager_with_mock_version("1.0.85")
        assert manager.supports_output_styles(), "Version 1.0.85 should support output styles"
        
        # Test minimum supported version
        manager = self.create_manager_with_mock_version("1.0.83")
        assert manager.supports_output_styles(), "Version 1.0.83 should support output styles"
        
        # Test unsupported version
        manager = self.create_manager_with_mock_version("1.0.82")
        assert not manager.supports_output_styles(), "Version 1.0.82 should not support output styles"
        
        # Test no version detected
        manager = self.create_manager_with_mock_version(None)
        assert not manager.supports_output_styles(), "No version should not support output styles"
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            ...
            test_cases = [
                ...
                ("Settings Activation from Different Style", self.test_settings_activation_from_different_style),
    ...
    def print_test_summary(self):
        ...
        failed_tests = sum(1 for _, status, _ in self.test_results if status in ["FAIL", "ERROR"])
...
def main():
    ...
    if success:
        logger.info("\n🔴 QA SIGN-OFF: PASS 🔴")
        logger.info("The output style installer fix works correctly in all tested scenarios:")
        ...
        logger.info("- Handles fresh installations, existing files, and corrupted settings")
