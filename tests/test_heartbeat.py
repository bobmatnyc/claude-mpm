...
async def test_heartbeat():
    ...
    async def on_connect():
        ...
        print("Note: For testing, you may want to temporarily reduce the interval in server.py")
    ...
    try:
        print(f"[{datetime.now().isoformat()}] Connecting to Socket.IO server at http://localhost:8765...")
        await sio.connect('http://localhost:8765')
        
        # Wait for heartbeat (3 minutes = 180 seconds, plus some buffer)
        print(f"[{datetime.now().isoformat()}] Connected! Waiting up to 200 seconds for heartbeat...")
        ...
        for i in range(200):
            ...
            if i % 30 == 0 and i > 0:
                print(f"[{datetime.now().isoformat()}] Still waiting... ({i} seconds elapsed)")
