...
def generate_test_event(event_id: int, method: str) -> dict:
    ...
    return {
        ...
        "metadata": {
            "generator": "test_event_generator.py",
            "method": method
        }
    }
...
def send_via_http(event: dict, host: str = "localhost", port: int = 8765) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return True
        print(f"❌ HTTP: Failed to send event {event['data']['event_id']} - Status: {response.status_code}")
        return False
...
async def main():
    ...
    parser.add_argument('--method', choices=['eventbus', 'http', 'both'], 
                       default='both', help='Method to send events')
