...
def validate_code_analyzer_template():
    ...
    template_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "templates" / "code_analyzer.json"
    ...
    required_fields = ["schema_version", "agent_id", "agent_version", "capabilities", "knowledge", "instructions"]
    ...
    mermaid_expertise = any("mermaid" in exp.lower() or "visualization" in exp.lower() for exp in domain_expertise)
    ...
    mermaid_tags = any(tag in ["mermaid", "visualization", "architecture-diagrams"] for tag in tags)
    if not mermaid_tags:
    ...
    print("\n" + "=" * 60)
    if checks_passed:
        print("✅ Code Analyzer agent template successfully updated with Mermaid capabilities!")
        print("\nThe agent can now:")
