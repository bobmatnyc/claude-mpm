...
"""
        ...
        logger.info(f"Create result: success={result.success}, duration={result.duration_ms:.1f}ms")
...
"""
        ...
        logger.info(f"Update result: success={result.success}, duration={result.duration_ms:.1f}ms")
        
        if result.success:
            logger.info(f"Agent updated to version: {result.metadata.get('new_version')}")
        else:
...
async def test_delete_agent():
    ...
    try:
        ...
        logger.info(f"Delete result: success={result.success}, duration={result.duration_ms:.1f}ms")
        
        if result.success:
            logger.info(f"Agent deleted, backup at: {result.metadata.get('backup_path')}")
        else:
