...
def test_multi_source_discovery():
    ...
    try:
        ...
        print(f"Discovered agents from multiple sources:")
        ...
        for agent_name, agent_info in selected.items():
            print(f"  {agent_name}: v{agent_info['version']} from {agent_info['source']}")
        ...
        for agent_name, (expected_version, expected_source) in expected.items():
            if agent_name in selected:
                ...
                if actual_version == expected_version and actual_source == expected_source:
                    print(f"  ✓ {agent_name}: Correct (v{expected_version} from {expected_source})")
                else:
                    print(f"  ✗ {agent_name}: Expected v{expected_version} from {expected_source}, "
