...
def test_basic_functionality():
    ...
    try:
        ...
        if manager.start_daemon_background():
            ...
            log_test("Daemon startup", True, f"Server PID: {server_pid}, Supervisor: {supervisor_pid}, Port: {port}")
...
def test_crash_recovery():
    ...
    try:
        ...
        print(f"Initial: Server PID {initial_server_pid}, Supervisor PID {supervisor_pid}")
        
        # Kill server process
        try:
            os.kill(initial_server_pid, signal.SIGKILL)
            log_test("Crash simulation", True, f"Killed server process {initial_server_pid}")
        except Exception as e:
        ...
        while time.time() - start_time < recovery_timeout:
            ...
            if new_server_pid > 0 and new_server_pid != initial_server_pid:
                ...
                if metrics.get('restarts', 0) > 0:
                    log_test("Restart tracking", True, f"Restarts: {metrics['restarts']}")
                else:
...
def test_configuration():
    ...
    try:
        if manager.start_daemon_background():
            ...
            if 9600 <= port <= 9610:
                log_test("Custom port configuration", True, f"Using port {port} from custom range")
            else:
                log_test("Custom port configuration", False, f"Port {port} not in custom range")
            ...
            log_test("Configuration test setup", False, "Could not start daemon with custom config")
    ...
    try:
        if manager.start_daemon_background():
            ...
            if metrics and metrics.get('health_checks_passed', 0) > 0:
                log_test("Health checks execution", True, f"Passed: {metrics['health_checks_passed']}")
            else:
            ...
            if metrics.get('status') in ['healthy', 'running']:
                ...
                log_test("Health status tracking", False, f"Unexpected status: {metrics.get('status')}")
        else:
...
def main():
    ...
    if TEST_RESULTS:
        ...
        critical_tests = [
            "Daemon startup", "Port accessibility", "Status command",
            "Automatic recovery", "Service continuity"
        ]
        
        critical_passed = sum(1 for r in TEST_RESULTS 
                            if r['passed'] and any(ct in r['name'] for ct in critical_tests))
        critical_total = sum(1 for r in TEST_RESULTS 
                           if any(ct in r['name'] for ct in critical_tests))
        ...
        if critical_total > 0:
            ...
            if critical_percentage >= 90:
                ...
                print("⚠️  PRODUCTION READY WITH MONITORING - Most critical tests passed")
            else:
