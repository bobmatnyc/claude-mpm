...
def test_port_configuration():
    ...
    assert NetworkConfig.SOCKETIO_PORT_RANGE == expected_range, (
        f"SOCKETIO_PORT_RANGE should be {expected_range}, got {NetworkConfig.SOCKETIO_PORT_RANGE}"
    )
    print(f"✓ SOCKETIO_PORT_RANGE: {NetworkConfig.SOCKETIO_PORT_RANGE[0]}-{NetworkConfig.SOCKETIO_PORT_RANGE[1]}")
    ...
    print(f"   Port range: {NetworkConfig.SOCKETIO_PORT_RANGE[0]}-{NetworkConfig.SOCKETIO_PORT_RANGE[1]}")
    return True
