...
class TestMCPSummarizerTool:
    ...
    async def test_summarize_executive(self, server):
        ...
        assert result is not None
        assert "OVERVIEW" in result or "FINDINGS" in result or "RECOMMENDATIONS" in result
    ...
    async def test_max_length_enforcement(self, server):
        ...
        for max_length in [10, 25, 50, 100]:
            ...
            assert word_count <= max_length * 1.1, f"Expected <= {max_length * 1.1} words, got {word_count}"
    ...
    async def test_tool_registration(self, server):
        ...
        assert hasattr(server, '_summarize_content')
