...
def test_memory_hooks_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Patch PathResolver to use temp directory
        with patch('claude_mpm.utils.paths.PathResolver.get_project_root', return_value=Path(tmpdir)):
            ...
            memory_manager.add_learning('engineer', 'architecture', 'Uses microservices pattern')
            memory_manager.add_learning('engineer', 'architecture', 'REST API with JWT auth')
            memory_manager.add_learning('engineer', 'mistake', "Don't forget input validation")
...
"""
            ...
            assert 'bcrypt for password hashing' in pre_result2.data['context']['agent_memory']
            assert 'rate limit authentication endpoints' in pre_result2.data['context']['agent_memory']
        ...
        with patch('claude_mpm.utils.paths.PathResolver.get_project_root', return_value=Path(tmpdir)):
            ...
            post_context = HookContext(
                ...
                data={
                    'agent': 'QA',
                    'result': {
                        'content': 'Discovered pattern: Important finding'
                    }
                },
                metadata={},
                timestamp=datetime.now()
