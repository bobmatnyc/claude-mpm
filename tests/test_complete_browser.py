...
def test_simple_browser():
    ...
    try:
        response = requests.get(f"{base_url}/static/js/components/code-simple.js", timeout=10)
        if response.status_code == 200:
    ...
    try:
        ...
        response = requests.get(f"{base_url}/api/directory/list", 
                              params={"path": test_path}, timeout=10)
        
        if response.status_code == 200:
            print("✅ API endpoint responds successfully")
            
            data = response.json()
            print(f"✅ API returns JSON data")
            ...
            if data.get('exists') and data.get('is_directory') and data.get('contents'):
                ...
                print(f"❌ API response invalid: exists={data.get('exists')}, is_directory={data.get('is_directory')}, contents={len(data.get('contents', []))}")
                return False
    ...
    for test_path in test_paths:
        try:
            response = requests.get(f"{base_url}/api/directory/list", 
                                  params={"path": test_path}, timeout=5)
            ...
            if response.status_code == 200:
                ...
                if test_path == "/nonexistent/path":
                    if not data.get('exists'):
                        ...
                        print(f"❌ Should report non-existent path as not existing: {test_path}")
                else:
                    if data.get('exists'):
