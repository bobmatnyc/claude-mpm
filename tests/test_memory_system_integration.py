...
class TestMemorySystemIntegration:
    ...
    def setup_method(self):
        ...
        self.content_manager = MemoryContentManager(
            self.memory_manager.memory_limits
        )
    ...
    def test_timestamp_updates_on_modifications(self):
        ...
        initial_timestamp = initial_content.split("<!-- Last Updated: ")[1].split(" -->")[0]
        ...
        updated_timestamp = updated_content.split("<!-- Last Updated: ")[1].split(" -->")[0]
        ...
        assert f"- {new_item}" in updated_content, "New item should be added with bullet point"
    ...
    def test_memories_field_complete_replacement(self):
        ...
        success = self.memory_manager.extract_and_update_memory(agent_id, f"Response text\n```json\n{mock_response}\n```")
        ...
        assert "Initial memory 1" not in updated_content, "Old memories should be removed"
        assert "Initial memory 2" not in updated_content, "Old memories should be removed"
        
        # Verify new memories are present
        for memory in new_memories:
            assert f"- {memory}" in updated_content, f"New memory should be present: {memory}"
        
        print(f"✓ All old memories removed and new memories added")
        print(f"✓ MEMORIES field replacement successful")
    ...
    def test_remember_field_incremental_updates(self):
        ...
        assert "- Existing memory item" in updated_content, "Existing memory should be preserved"
        
        # Verify new memories are added
        for learning in new_learnings:
            assert f"- {learning}" in updated_content, f"New learning should be added: {learning}"
        
        print(f"✓ Existing memories preserved")
        print(f"✓ New learnings added incrementally")
    ...
    def test_hook_processing_memories_field(self):
        ...
        with patch('claude_mpm.hooks.claude_hooks.memory_integration.get_memory_manager', return_value=mock_memory_manager):
            ...
            if memory_file.exists():
                content = memory_file.read_text()
                print(f"Memory file content after hook processing:")
                print(content)
                
                for memory in memories_data:
                    assert f"- {memory}" in content, f"Memory should be processed by hook: {memory}"
                    
                print(f"✓ Hook processing successfully extracted MEMORIES field")
            else:
                print(f"✓ Hook processing completed (memory manager was mocked)")
    ...
    def test_memory_deduplication_optimization(self):
        ...
        deduplicated_content, removed_count = self.content_manager.deduplicate_list(content_before)
        ...
        # Count unique items
        dedupe_items = self.content_manager.parse_memory_content_to_list(deduplicated_content)
        print(f"Items after deduplication: {len(dedupe_items)}")
        
        assert len(dedupe_items) < len(items), "Should have fewer items after deduplication"
        
        # Verify important content is preserved
        assert any("databases" in item.lower() for item in dedupe_items), "Unique content should be preserved"
        
        print(f"✓ Deduplication removed {removed_count} items")
        print(f"✓ Unique content preserved")
    ...
    def test_project_based_memories_storage(self):
        ...
        assert self.memories_dir.parent.name == ".claude-mpm", "Should be in .claude-mpm directory"
        ...
        for agent_id in agent_ids:
            ...
            # Verify content
            content = memory_file.read_text()
            assert f"- {test_memory}" in content, f"Memory should be saved for {agent_id}"
        ...
        assert "Agent Memory System" in readme_content, "README should have proper title"
        assert "Manual Editing" in readme_content, "README should have manual editing section"
        
        print(f"✓ README.md created with proper content")
        
        # Verify all files are in the correct location
        memory_files = list(self.memories_dir.glob("*_memories.md"))
        assert len(memory_files) == len(agent_ids), f"Should have {len(agent_ids)} memory files"
    ...
    def test_full_end_to_end_memory_flow(self):
        ...
        for memory in complete_memories:
            assert f"- {memory}" in final_content, f"Final memory should be present: {memory}"
        
        # Old memories should be gone (replaced by MEMORIES field)
        for memory in incremental_memories:
            assert f"- {memory}" not in final_content, f"Old memory should be replaced: {memory}"
        
        assert "New incremental memory from response" not in final_content, "Remember field memory should be replaced"
        ...
        assert len(memory_items) == len(complete_memories), f"Should have {len(complete_memories)} memory items"
    ...
    def run_all_tests(self):
        ...
        tests = [
            ("Memory File Creation Simple List Format", self.test_memory_file_creation_simple_list_format),
            ("Timestamp Updates on Modifications", self.test_timestamp_updates_on_modifications),
            ("MEMORIES Field Complete Replacement", self.test_memories_field_complete_replacement),
            ("Remember Field Incremental Updates", self.test_remember_field_incremental_updates),
            ("Hook Processing MEMORIES Field", self.test_hook_processing_memories_field),
            ("Memory Deduplication Optimization", self.test_memory_deduplication_optimization),
            ("PROJECT-based Memories Storage", self.test_project_based_memories_storage),
