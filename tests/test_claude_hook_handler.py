...
class TestClaudeHookHandlerSecurity(unittest.TestCase):
    ...
    def test_write_outside_working_dir_blocked(self, mock_logger, mock_exit, mock_print):
    ...
    def test_path_traversal_blocked(self, mock_logger, mock_exit, mock_print):
        ...
        event = self.create_event(
            "PreToolUse",
            "Edit",
            {"file_path": f"{self.working_dir}/../../../etc/passwd", "old_string": "root", "new_string": "hacked"}
        )
        self.handler.event = event
        self.handler.hook_type = "PreToolUse"
    ...
    @patch('sys.exit')
    @patch('claude_mpm.hooks.claude_hooks.hook_handler.logger')
    def test_multiedit_outside_working_dir_blocked(self, mock_logger, mock_exit, mock_print):
    ...
    @patch('sys.exit')
    @patch('claude_mpm.hooks.claude_hooks.hook_handler.logger')
    def test_notebook_edit_outside_working_dir_blocked(self, mock_logger, mock_exit, mock_print):
