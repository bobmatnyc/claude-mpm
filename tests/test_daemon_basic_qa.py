...
def test_basic_startup_shutdown():
    ...
    result = subprocess.run([sys.executable, str(DAEMON_SCRIPT), "start"], 
    ...
    if server_pid > 0 and supervisor_pid > 0 and port > 0:
        ...
        if result.returncode == 0 and "RUNNING" in result.stdout:
            ...
            log_test("Status command", False, f"Status command failed")
        
        # Test shutdown
        print("Stopping daemon...")
        result = subprocess.run([sys.executable, str(DAEMON_SCRIPT), "stop"], 
                              capture_output=True, text=True, timeout=15)
        time.sleep(3)
        ...
        if server_pid_after == 0 and supervisor_pid_after == 0:
            ...
            log_test("Clean shutdown", False, f"Processes still running: server={server_pid_after}, supervisor={supervisor_pid_after}")
...
def test_crash_recovery():
    ...
    if new_server_pid > 0 and new_server_pid != initial_server_pid:
        ...
        log_test("Automatic recovery", False, f"Recovery failed. New PID: {new_server_pid}")
...
def test_concurrent_protection():
    ...
    if second_server_pid == first_server_pid:
        ...
        log_test("Concurrent protection", False, f"Multiple instances: {first_server_pid}, {second_server_pid}")
