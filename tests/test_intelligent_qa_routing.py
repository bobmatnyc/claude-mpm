...
def test_qa_agent_selection_logic():
    ...
    def select_qa_agent(implementation_context, available_agents=['qa', 'api_qa', 'web_qa']):
        """Simulate the QA agent selection logic."""
        backend_keywords = ['api', 'endpoint', 'route', 'rest', 'graphql', 
                           'server', 'backend', 'auth', 'database', 'service', 'jwt']
        ...
        if any(ext in implementation_context for ext in ['.py', '.go', '.java', '/api/', '/routes/', '/graphql/']):
            has_backend = True
        ...
        if has_backend and has_frontend:
            return ['api_qa', 'web_qa'] if 'api_qa' in available_agents and 'web_qa' in available_agents else ['qa']
...
def test_api_qa_agent_configuration():
    ...
    api_qa_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "templates" / "api_qa.json"
    ...
    try:
        with open(api_qa_path, 'r') as f:
            config = json.load(f)
        
        # Validate required fields
        required_fields = ["schema_version", "agent_id", "agent_version", "metadata", "capabilities", "instructions"]
        ...
        if "WebFetch" not in config["capabilities"]["tools"]:
            print(f"⚠️  WARNING: WebFetch tool not in API QA capabilities")
        
        print(f"✅ PASS: API QA agent configuration is valid")
...
def test_workflow_configuration():
    ...
    workflow_path = Path(__file__).parent.parent / "src" / "claude_mpm" / "agents" / "WORKFLOW.md"
