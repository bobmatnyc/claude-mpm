...
class TestClaudeHookHandler(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.handler.connection_pool, self.handler.connection_manager.connection_pool)
        self.assertEqual(self.handler.event_bus, self.handler.connection_manager.event_bus)
    ...
    def test_route_event_user_prompt(self):
        ...
        self.handler.event_handlers.handle_user_prompt_fast.assert_called_once_with(event)
    ...
    def test_route_event_subagent_stop(self):
        ...
        self.handler.event_handlers.handle_subagent_stop_fast.assert_called_once_with(event)
    ...
    @patch('sys.stdin')
    @patch('select.select')
    def test_handle_with_timeout(self, mock_select, mock_stdin, mock_stdout, mock_alarm):
    ...
    def test_memory_operation_extraction(self):
        ...
        self.handler.subagent_processor.process_subagent_stop.assert_called_once_with(event)
    ...
    def test_concurrent_event_handling(self):
        ...
        self.handler._read_hook_event = Mock(side_effect=[
    ...
    def test_delegation_tracking(self):
        ...
        self.handler.state_manager.get_delegation_agent_type = Mock(return_value="test_agent")
...
class TestEventProcessing(unittest.TestCase):
    ...
    def test_process_tool_events(self):
        ...
        self.handler.event_handlers.handle_pre_tool_fast.assert_called_once_with(pre_event)
        ...
        self.handler.event_handlers.handle_post_tool_fast.assert_called_once_with(post_event)
    ...
    def test_process_notification_event(self):
        ...
        self.handler.event_handlers.handle_notification_fast.assert_called_once_with(event)
...
class TestIntegration(unittest.TestCase):
    ...
    @patch('select.select')
    @patch('signal.alarm')
    def test_complete_event_flow(self, mock_alarm, mock_select, mock_stdout, mock_stdin):
        ...
        with patch('src.claude_mpm.hooks.claude_hooks.hook_handler.StateManagerService'), \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.ConnectionManagerService'), \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.DuplicateEventDetector') as MockDuplicate, \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.SubagentResponseProcessor'), \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.MemoryHookManager'), \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.ResponseTrackingManager'), \
             patch('src.claude_mpm.hooks.claude_hooks.hook_handler.EventHandlers'):
