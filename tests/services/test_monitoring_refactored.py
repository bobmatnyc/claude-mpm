...
class TestProcessResourceChecker:
    ...
    @patch("claude_mpm.services.infrastructure.monitoring.process.PSUTIL_AVAILABLE", False)
    ...
    @patch("claude_mpm.services.infrastructure.monitoring.process.PSUTIL_AVAILABLE", True)
    ...
    async def test_socket_creation_failure(self):
        ...
        with patch("socket.socket") as mock_socket_class:
            ...
            mock_socket_class.side_effect = [
                mock_socket1,
                socket.error("No file descriptors"),
            ]
...
class TestServiceHealthChecker:
    ...
    async def test_healthy_service_metrics(self):
        ...
        if "event_processing_rate" in metric_dict:
            assert metric_dict["event_processing_rate"].value == pytest.approx(10.0, rel=0.5)  # ~10 events/sec
...
class TestAdvancedHealthMonitor:
    ...
    def test_determine_overall_status(self):
        ...
        metrics = [
            HealthMetric(f"m{i}", i, HealthStatus.WARNING)
            for i in range(4)
        ]
...
class TestIntegration:
    ...
    async def test_full_monitoring_cycle(self):
        ...
        # Add process checker
        with patch("claude_mpm.services.infrastructure.monitoring.process.PSUTIL_AVAILABLE", True):
            with patch("claude_mpm.services.infrastructure.monitoring.process.psutil") as mock_psutil:
                mock_process = MagicMock()
                mock_process.is_running.return_value = True
                mock_process.status.return_value = "running"
