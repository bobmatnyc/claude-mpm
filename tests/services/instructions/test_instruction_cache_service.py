...
class TestInstructionCacheService:
    ...
    def test_update_cache_creates_cache_on_first_run(
        ...
        # Verify cache file starts with the AUTO-GENERATED banner and ends with
        # the supplied content.  The banner is prepended at write time and is
        # excluded from the hash, so it must not affect cache-validity checks.
        cached_text = cache_service.cache_file.read_text()
        assert cached_text.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_text.endswith(test_instruction_content)

        # Idempotency: assembling identical content a second time must produce
        # the exact same file bytes (no timestamp or per-run variation in banner).
    ...
    def test_update_cache_updates_when_content_changes(self, cache_service):
        ...
        # Verify cache file contains new content (after the banner).
        cached_text = cache_service.cache_file.read_text()
        assert cached_text.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_text.endswith(new_content)
    ...
    def test_atomic_cache_update(self, cache_service):
        ...
        # After update, cache file must contain the new content (after the banner).
        cached_text = cache_service.cache_file.read_text()
        assert cached_text.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_text.endswith(new_content)
    ...
    def test_cache_service_with_large_content(self, cache_service):
        ...
        # Verify cache file ends with the large content (banner is prepended).
        cached_text = cache_service.cache_file.read_text()
        assert cached_text.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_text.endswith(large_content)
