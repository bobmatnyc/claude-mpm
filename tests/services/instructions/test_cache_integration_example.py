...
def test_cache_assembled_instruction_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Step 5: Verify cache content: starts with the AUTO-GENERATED banner,
        # followed by the assembled instruction content.  The banner is prepended
        # at write time and is excluded from the content hash so it never causes
        # spurious cache invalidation on repeated runs.
        cached_content = cache_file.read_text()
        assert cached_content.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_content.endswith(assembled_instruction)
        ...
        # Cache now has updated content (banner + new instruction).
        cached_content_new = cache_file.read_text()
        assert cached_content_new.startswith("<!-- AUTO-GENERATED by claude-mpm")
        assert cached_content_new.endswith(updated_instruction)
