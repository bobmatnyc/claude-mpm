...
class TestAgentValidationService:
    ...
    def service(self, mock_validator, mock_registry):
        ...
        with patch('claude_mpm.services.cli.agent_validation_service.FrontmatterValidator', return_value=mock_validator):
            with patch('claude_mpm.services.cli.agent_validation_service.AgentRegistryAdapter') as mock_adapter:
                mock_adapter.return_value.registry = mock_registry
                service = AgentValidationService()
                service._registry = mock_registry  # Direct assignment to bypass lazy loading
    ...
    def test_validate_all_agents(self, service, mock_validator, mock_registry):
        ...
        validation_results = [
            ...
            ValidationResult(is_valid=False, errors=["Error 1"], warnings=["Warning 1"], corrections=["Fix 1"]),
            ValidationResult(is_valid=True, errors=[], warnings=["Warning 2"], corrections=[])
        ]
    ...
    def test_validate_all_agents_with_missing_files(self, service, mock_validator, mock_registry):
        ...
        assert result["total_errors"] == 1
        assert len([r for r in result["results"] if r.get("error") == "File not found"]) == 1
    
    def test_fix_agent_frontmatter_dry_run(self, service, mock_validator, mock_registry):
        ...
        mock_validator.correct_file.assert_called_once_with(Path("/path/to/agent.md"), dry_run=True)
    ...
    def test_fix_all_agents(self, service, mock_validator, mock_registry):
        ...
        correction_results = [
            ValidationResult(is_valid=True, errors=[], warnings=[], corrections=[]),
            ValidationResult(is_valid=False, errors=["Error"], warnings=[], corrections=["Fix"])
        ]
...
"""
    ...
    def test_validate_deployment_state(self, service, mock_registry):
        ...
        mock_registry.list_agents.return_value = {
            ...
            "agent3": {"path": "/path/to/agent3.md", "name": "Agent One"},  # Duplicate name
    ...
    def test_registry_initialization_error(self):
        ...
        with patch('claude_mpm.services.cli.agent_validation_service.AgentRegistryAdapter', side_effect=Exception("Init error")):
            with pytest.raises(RuntimeError, match="Could not initialize agent registry"):
                _ = service.registry