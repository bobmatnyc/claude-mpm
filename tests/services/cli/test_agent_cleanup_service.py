...
class TestAgentCleanupService:
    ...
    def mock_deployment_service(self):
        ...
        service = MagicMock()
        service.clean_deployment = MagicMock(return_value={
    ...
    def mock_multi_source_service(self):
        ...
        service = MagicMock()
        service.cleanup_orphaned_agents = MagicMock(return_value={
            "orphaned": [
                {"name": "old_agent", "file": "/path/to/old_agent.md", "version": "1.0.0"}
            ],
            "removed": [],
            "errors": []
        })
        service.discover_agents_from_all_sources = MagicMock(return_value={
            "Agent1": [{"file_path": "/path/to/agent1.md"}],
            "Agent2": [{"file_path": "/path/to/agent2.md"}]
        })
        service.detect_orphaned_agents = MagicMock(return_value=[
    ...
    def test_clean_deployed_agents_success(self, cleanup_service, mock_deployment_service):
    ...
    def test_clean_deployed_agents_exception(self, cleanup_service):
        ...
        cleanup_service._deployment_service.clean_deployment.side_effect = Exception("Test error")
    ...
    def test_clean_orphaned_agents_dry_run(self, mock_path, cleanup_service, mock_multi_source_service):
        ...
        mock_path.cwd.return_value.__truediv__.return_value.__truediv__.return_value = mock_agents_dir
        
        # Patch multi-source service
        with patch.object(cleanup_service, "_get_multi_source_service", return_value=mock_multi_source_service):
    ...
    @patch("claude_mpm.services.cli.agent_cleanup_service.Path")
    def test_clean_orphaned_agents_actual_removal(self, mock_path, cleanup_service, mock_multi_source_service):
        ...
        mock_multi_source_service.cleanup_orphaned_agents.return_value = {
            "orphaned": [{"name": "old_agent", "file": "/path/to/old_agent.md", "version": "1.0.0"}],
            "removed": ["old_agent"],
            "errors": []
        }
    ...
    @patch("claude_mpm.services.cli.agent_cleanup_service.Path")
    def test_get_orphaned_agents(self, mock_path, cleanup_service, mock_multi_source_service):
    ...
    @patch("claude_mpm.services.cli.agent_cleanup_service.Path")
    def test_perform_cleanup_all(self, mock_path, cleanup_service, mock_multi_source_service):
        ...
        deployed_op = next(op for op in result["operations"] if op["type"] == "deployed")
        assert deployed_op["result"]["success"] is True
        
        # Check orphaned cleanup operation
        orphaned_op = next(op for op in result["operations"] if op["type"] == "orphaned")
    ...
    @patch("claude_mpm.services.cli.agent_cleanup_service.Path")
    def test_perform_cleanup_orphaned_only(self, mock_path, cleanup_service, mock_multi_source_service):
        ...
        # Patch multi-source service
        with patch.object(cleanup_service, "_get_multi_source_service", return_value=mock_multi_source_service):
            result = cleanup_service.perform_cleanup(cleanup_type="orphaned", dry_run=True)
    ...
    def test_validate_cleanup_with_orphans(self, mock_path, cleanup_service):
        ...
        with patch.object(cleanup_service, "get_orphaned_agents", return_value=orphaned):
            result = cleanup_service.validate_cleanup()
