...
class TestStateManager:
    ...
    async def test_capture_state(self, state_manager, monkeypatch):
        ...
        monkeypatch.setattr(
            "claude_mpm.services.infrastructure.state_manager.get_process_memory",
            mock_memory,
        )
    ...
    @pytest.mark.asyncio
    async def test_persist_and_load_state(
        self, state_manager, sample_complete_state, temp_dir
    ):
        """Test state persistence and loading."""
        # Persist state
        result = await state_manager.persist_state(
            sample_complete_state, compress=False
        )
        assert result is True
        ...
        assert (
            loaded_state.restart_state.reason
            == sample_complete_state.restart_state.reason
        )

    @pytest.mark.asyncio
    async def test_persist_state_compressed(
        self, state_manager, sample_complete_state, temp_dir
    ):
    ...
    async def test_restore_state(
        self, state_manager, sample_complete_state, monkeypatch
    ):
        ...
        mock_chdir.assert_called_once_with(
            sample_complete_state.process_state.working_directory
        )
    ...
    @pytest.mark.asyncio
    async def test_get_conversation_context_large_file(
        self, state_manager, temp_dir, monkeypatch
    ):
    ...
    async def test_cooldown_between_captures(self, state_manager, monkeypatch):
        ...
        # Wait for cooldown
        state_manager.last_capture_time = (
            time.time() - state_manager.capture_cooldown - 1
        )
