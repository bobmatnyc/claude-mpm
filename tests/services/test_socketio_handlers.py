...
class TestBaseEventHandler:
    ...
    async def test_broadcast_event_with_skip_sid(self, base_handler):
        ...
        await base_handler.broadcast_event("test_event", {"data": "test"}, skip_sid="sid2")
    ...
    def test_log_error(self, base_handler):
        ...
        with patch.object(base_handler.logger, 'error') as mock_error:
            base_handler.log_error("test_operation", Exception("Test error"), {"context": "test"})
...
class TestConnectionEventHandler:
    ...
    async def test_connect_event(self, connection_handler):
        ...
        with patch.object(connection_handler, '_send_event_history', new_callable=AsyncMock) as mock_send_history:
    ...
    async def test_get_status_event(self, connection_handler):
        ...
        with patch('claude_mpm.services.socketio.handlers.connection.datetime') as mock_datetime:
    ...
    async def test_send_event_history_filtered(self, connection_handler):
        ...
        await connection_handler._send_event_history("test-sid", event_types=["file_write"], limit=10)
...
class TestGitEventHandler:
    ...
    async def test_check_file_tracked_success(self, git_handler):
        ...
        with patch('subprocess.run', return_value=mock_result):
            await check_tracked_handler("test-sid", {
    ...
    def test_sanitize_working_dir_invalid_states(self, git_handler):
        ...
        with patch('os.getcwd', return_value='/current/dir'):
            for invalid_state in invalid_states:
                result = git_handler._sanitize_working_dir(invalid_state, "test_operation")
    ...
    async def test_validate_directory_exists(self, git_handler):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            result = await git_handler._validate_directory("test-sid", "/valid/dir", "test_response")
    ...
    async def test_validate_directory_not_exists(self, git_handler):
        ...
        with patch('os.path.exists', return_value=False):
            result = await git_handler._validate_directory("test-sid", "/invalid/dir", "test_response")
    ...
    async def test_validate_directory_not_directory(self, git_handler):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            result = await git_handler._validate_directory("test-sid", "/file/path", "test_response")
            assert result is False
    ...
    def test_make_path_relative_to_git_absolute_path(self, git_handler):
        ...
        with patch('subprocess.run', return_value=mock_result), \
            ...
            result = git_handler._make_path_relative_to_git("/git/repo/relative/path.py", "/git/repo")
    ...
    def test_check_file_git_status(self, git_handler):
        ...
        with patch('subprocess.run', side_effect=[status_result, ls_result]):
            is_tracked, has_changes = git_handler._check_file_git_status("test_file.py", "/git/repo")
    ...
    async def test_generate_git_diff_not_git_repo(self, git_handler):
        ...
        with patch('asyncio.create_subprocess_exec', return_value=mock_process), \
            ...
            result = await git_handler.generate_git_diff("test_file.py", working_dir="/not/git")
    ...
    async def test_generate_git_diff_with_timestamp(self, git_handler):
        ...
        with patch('asyncio.create_subprocess_exec', side_effect=[git_check, git_root, log_proc, diff_proc]), \
             patch.object(git_check, 'communicate', new_callable=AsyncMock), \
             patch.object(git_root, 'communicate', new_callable=AsyncMock, return_value=(b'/git/repo\n', b'')), \
             patch.object(log_proc, 'communicate', new_callable=AsyncMock, return_value=(b'abc123 Test commit\n', b'')), \
             patch.object(diff_proc, 'communicate', new_callable=AsyncMock, return_value=(b'diff --git a/test.py b/test.py\n', b'')):
...
class TestFileEventHandler:
    ...
    async def test_read_file_safely_success(self, file_handler, tmp_path):
        ...
        with patch('claude_mpm.deployment_paths.get_project_root', return_value=str(tmp_path)):
    ...
    async def test_read_file_safely_file_not_found(self, file_handler, tmp_path):
        ...
        with patch('claude_mpm.deployment_paths.get_project_root', return_value=str(tmp_path)):
            result = await file_handler._read_file_safely(str(nonexistent_file), str(tmp_path))
    ...
    async def test_read_file_safely_file_too_large(self, file_handler, tmp_path):
        ...
        with patch('claude_mpm.deployment_paths.get_project_root', return_value=str(tmp_path)):
            result = await file_handler._read_file_safely(str(test_file), str(tmp_path), max_size=1024)
...
if __name__ == "__main__":
    ...
    print("# Add To Memory:")
    print("Type: pattern")
    print("Content: Comprehensive unit tests for SocketIO handlers with registry validation and 95%+ coverage")
    print("#")
