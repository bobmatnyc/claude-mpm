...
class TestSubprocessLauncherService:
    ...
    def test_launch_subprocess_interactive_basic(self, mock_isatty, mock_close, mock_popen, mock_openpty, service_with_logger_and_websocket):
        ...
        with patch.object(service_with_logger_and_websocket, '_handle_subprocess_io') as mock_io:
    ...
    @patch('tty.setraw')
    def test_launch_subprocess_interactive_with_tty(self, mock_setraw, mock_tcsetattr, mock_tcgetattr, 
                                                   mock_isatty, mock_close, mock_popen, mock_openpty, service):
    ...
    @patch('os.close')
    @patch('sys.stdin.isatty', return_value=False)
    def test_launch_subprocess_interactive_process_cleanup(self, mock_isatty, mock_close, mock_popen, mock_openpty, service):
        ...
        mock_process.wait.side_effect = [None, subprocess.TimeoutExpired("cmd", 2), None]
        mock_popen.return_value = mock_process
    ...
    def test_launch_subprocess_interactive_websocket_integration(self, service_with_logger_and_websocket):
        ...
        with patch('pty.openpty', return_value=(10, 11)), \
            ...
            websocket.claude_status_changed.assert_any_call(
                status="stopped",
                message="Claude subprocess exited with code 0"
            )
