...
class TestEchoToolAdapter:
    ...
    async def test_echo_uppercase(self):
        ...
        invocation = MCPToolInvocation(
            tool_name="echo",
            parameters={"message": "hello", "uppercase": True}
        )
...
class TestCalculatorToolAdapter:
    ...
    async def test_calculator_division(self):
        ...
        invocation = MCPToolInvocation(
            tool_name="calculator",
            parameters={"operation": "divide", "a": 15, "b": 3}
        )
    ...
    async def test_calculator_unknown_operation(self):
        ...
        invocation = MCPToolInvocation(
            tool_name="calculator",
            parameters={"operation": "unknown", "a": 5, "b": 3}
        )
...
class TestSystemInfoToolAdapter:
    ...
    async def test_system_info_missing_type(self):
        ...
        assert result.success is False
        assert "'info_type'" in result.error.lower()  # Should mention missing parameter 'info_type'
    ...
    async def test_system_info_psutil_missing(self):
        ...
        with patch('builtins.__import__', side_effect=lambda name, *args: ImportError("No module named 'psutil'") if name == 'psutil' else __import__(name, *args)):
            invocation = MCPToolInvocation(
                tool_name="system_info",
                parameters={"info_type": "memory"}
            )
            ...
            assert result.success is False or result.success is True  # Either way is acceptable
