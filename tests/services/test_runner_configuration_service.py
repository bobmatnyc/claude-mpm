...
class TestRunnerConfigurationService:
    ...
    def test_initialize_configuration_basic(self, service):
        ...
        with patch('claude_mpm.services.runner_configuration_service.Config') as mock_config:
    ...
    def test_initialize_project_logger_success(self, service):
        ...
        with patch('claude_mpm.services.runner_configuration_service.get_project_logger') as mock_get_logger:
    ...
    def test_initialize_response_logger_success(self, service):
        ...
        with patch('claude_mpm.services.claude_session_logger.get_session_logger') as mock_get_logger:
            mock_get_logger.return_value = mock_logger

            result = service.initialize_response_logger(mock_config, mock_project_logger)
    ...
    def test_register_ticket_manager_disabled(self, service, mock_container):
        ...
        ticket_manager, enable_tickets = service.register_ticket_manager(mock_container, False)
        
        assert ticket_manager is None
        assert enable_tickets is False
        mock_container.register_singleton.assert_not_called()
    
    def test_register_ticket_manager_success(self, service, mock_container):
        """Test successful ticket manager registration."""
        mock_ticket_manager = Mock()
        mock_container.get.return_value = mock_ticket_manager
        
        ticket_manager, enable_tickets = service.register_ticket_manager(mock_container, True)
    ...
    def test_register_system_instructions_service_success(self, service, mock_container):
        ...
        result = service.register_system_instructions_service(mock_container, mock_agent_service)
        
        assert result == mock_system_service
        mock_container.register_factory.assert_called_once()
        mock_container.get.assert_called_once()
    
    def test_register_subprocess_launcher_service_success(self, service, mock_container):
    ...
    def test_create_session_log_file_success(self, service, tmp_path):
        ...
        result = service.create_session_log_file(mock_project_logger, "INFO", config_data)
