...
class TestVersionService:
    ...
    def test_get_version_with_package_import(self):
        ...
        with patch("claude_mpm.__version__", "1.2.3"), patch.object(
            self, "_get_build_number", return_value=None
        ):
            version = self.get_version()
            assert version == "v1.2.3"

    def test_get_version_with_build_number(self):
        """Test version retrieval with build number."""
        with patch("claude_mpm.__version__", "1.2.3+build.123"):
            version = self.get_version()
            assert version == "v1.2.3-build.123"

    def test_get_version_with_importlib_metadata(self):
    ...
    def test_get_version_with_version_file(self):
        ...
        with patch("claude_mpm.__version__", side_effect=ImportError), patch(
            ...
            with patch.object(self, "_get_build_number", return_value=None):
                version = self.get_version()
    ...
    def test_get_build_number_success(self):
    ...
    def test_get_build_number_not_found(self):
    ...
    def test_get_pep440_version_with_build(self):
        """Test PEP 440 version format with build number."""
        with patch("claude_mpm.__version__", "1.2.3"), patch.object(
            self, "get_build_number", return_value=456
        ):
            pep440_version = self.get_pep440_version()
            assert pep440_version == "1.2.3+build.456"

    def test_get_pep440_version_without_build(self):
        """Test PEP 440 version format without build number."""
        with patch("claude_mpm.__version__", "1.2.3"), patch.object(
            self, "get_build_number", return_value=None
        ):
            pep440_version = self.get_pep440_version()
            assert pep440_version == "1.2.3"

    def test_format_version_with_build(self):
        """Test version formatting with build number."""
        formatted = self._format_version("1.2.3", 456)
        assert formatted == "v1.2.3-build.456"

    def test_format_version_without_build(self):
        """Test version formatting without build number."""
        formatted = self._format_version("1.2.3", None)
        assert formatted == "v1.2.3"

    def test_version_with_build_file(self):
    ...
    def test_error_handling_in_version_detection(self):
    ...
    def test_error_handling_in_build_number(self):
