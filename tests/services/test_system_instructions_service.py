...
class TestSystemInstructionsService:
    ...
    def service_with_agent_capabilities(self):
        ...
        mock_agent_service.generate_deployed_agent_capabilities.return_value = "Mock agent capabilities"
        return SystemInstructionsService(agent_capabilities_service=mock_agent_service)
    
    def test_load_system_instructions_project_found(self, service, tmp_path):
    ...
    def test_load_system_instructions_framework_fallback(self, service, tmp_path):
        """Test fallback to framework instructions when project not found."""
        # Mock framework path
        framework_dir = tmp_path / "framework"
        framework_dir.mkdir()
        framework_instructions = framework_dir / "src" / "claude_mpm" / "agents" / "INSTRUCTIONS.md"
        ...
        with patch('claude_mpm.config.paths.paths.project_root', framework_dir), \
    ...
    def test_load_system_instructions_base_pm_fallback(self, service, tmp_path):
    ...
    def test_load_system_instructions_not_found(self, service, tmp_path):
        """Test when no system instructions are found."""
        with patch('claude_mpm.config.paths.paths.project_root', tmp_path / "nonexistent"), \
             patch('pathlib.Path.cwd', return_value=tmp_path / "empty_project"):
            result = service.load_system_instructions()
        
        assert result is None
    
    def test_process_base_pm_content_with_agent_capabilities(self, service_with_agent_capabilities):
...
"""
        ...
        result = service_with_agent_capabilities.process_base_pm_content(base_pm_content)
...
"""
    ...
    def test_create_system_prompt_load_instructions(self, service, tmp_path):
    ...
    def test_create_system_prompt_fallback(self, service, tmp_path):
    ...
    def test_get_version_from_package(self, service, tmp_path):
        ...
        with patch('claude_mpm.config.paths.paths.project_root', tmp_path / "nonexistent"), \
             patch('claude_mpm.services.system_instructions_service.claude_mpm') as mock_module:
            mock_module.__version__ = "2.0.0"
            version = service._get_version()
        
        assert version == "2.0.0"
    
    def test_get_version_unknown(self, service, tmp_path):
