...
class TestToolRegistration:
    ...
    @pytest.mark.asyncio
    async def test_register_tool_invalid_category(self):
    ...
    @pytest.mark.asyncio
    async def test_deregister_nonexistent_tool(self):
...
class TestToolInvocation:
    ...
    async def test_invoke_tool_success(self):
        ...
        mock_tool.get_definition.return_value = mock_definition
        mock_tool.initialize = AsyncMock(return_value=True)
        mock_tool.invoke = AsyncMock(return_value=MCPToolResult(
            success=True,
            data="test result",
            execution_time=0.1
        ))
