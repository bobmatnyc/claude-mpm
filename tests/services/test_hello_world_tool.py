...
class TestHelloWorldTool:
    ...
    async def test_time_based_greeting(self, tool, mock_invocation, hour, expected_greeting):
        """Test time-based greeting for different hours."""
        with patch('claude_mpm.services.mcp_gateway.tools.hello_world.datetime') as mock_datetime:
            mock_now = Mock()
            mock_now.hour = hour
    ...
    async def test_multi_language_greeting(self, tool, mock_invocation, language, expected):
...
class TestHelloWorldToolAsync:
    ...
    async def test_concurrent_invocations(self):
        ...
        results = await asyncio.gather(
            *[tool.invoke(inv) for inv in invocations]
        )
