...
class TestMemoryManager:
    ...
    def test_load_memories_cache_miss(self, memory_manager, mock_cache_manager):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("claude_mpm.services.core.memory_manager.Path.cwd", return_value=Path(tmpdir)):
                # Load memories
                result = memory_manager.load_memories()
            
            # Verify memories were loaded
            assert "actual_memories" in result
            assert "Test PM memory" in result["actual_memories"]  # Check content, not header
    ...
    def test_save_memory_pm(self, memory_manager, mock_cache_manager, mock_path_resolver):
        """Test saving a PM memory."""
        with tempfile.TemporaryDirectory() as tmpdir:
            memories_dir = Path(tmpdir) / ".claude-mpm" / "memories"
            
            # Configure mock to actually create the directory
            mock_path_resolver.ensure_directory.side_effect = lambda p: p.mkdir(parents=True, exist_ok=True) or p
    ...
    def test_save_memory_agent(self, memory_manager, mock_cache_manager, mock_path_resolver):
    ...
    def test_search_memories(self, memory_manager, mock_cache_manager):
        ...
        assert any(r["type"] == "PM" and "Important task" in r["content"] for r in results)
        assert any(r["type"] == "Agent" and "Important agent task" in r["content"] for r in results)
    ...
    def test_load_memories_with_priority(self, memory_manager, mock_cache_manager):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Mock paths
            with patch("claude_mpm.services.core.memory_manager.Path.home", return_value=Path(tmpdir) / "user"):
                with patch("claude_mpm.services.core.memory_manager.Path.cwd", return_value=Path(tmpdir) / "project"):
                    # Load memories
                    result = memory_manager.load_memories()
    ...
    def test_naming_mismatch_warning(self, memory_manager, mock_cache_manager, caplog):
        ...
        mock_cache_manager.get_deployed_agents.return_value = {"test-agent"}  # With dash
