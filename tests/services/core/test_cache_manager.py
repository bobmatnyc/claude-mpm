...
class TestCacheManager(unittest.TestCase):
    ...
    def test_interface_compliance(self):
        ...
        interface_methods = [
            ...
            'get_deployed_agents', 'set_deployed_agents',
            'get_agent_metadata', 'set_agent_metadata',
            'get_memories', 'set_memories',
            'clear_all', 'clear_agent_caches', 'clear_memory_caches',
            'is_cache_valid'
        ]
    ...
    def test_agent_metadata_cache(self):
        ...
        self.cache_manager.set_agent_metadata("test_agent.py", test_metadata, test_mtime)
    ...
    def test_is_cache_valid(self):
        ...
        self.assertTrue(
            self.cache_manager.is_cache_valid(current_time - 1, 5)
        )
        
        # Expired cache (past TTL)
        self.assertFalse(
            self.cache_manager.is_cache_valid(current_time - 10, 5)
        )
        
        # Edge case: exactly at TTL
        self.assertFalse(
            self.cache_manager.is_cache_valid(current_time - 5, 5)
        )
    ...
    def test_multiple_metadata_entries(self):
        ...
        for i in range(5):
            self.assertIsNone(
                self.cache_manager.get_agent_metadata(f"agent_{i}.py")
            )
