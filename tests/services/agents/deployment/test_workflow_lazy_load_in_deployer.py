...
class TestWorkflowLazyLoadInDeployer:
    ...
    # ------------------------------------------------------------------
    # (e) AUTO-GENERATED banner is present and deployment is deterministic
    # ------------------------------------------------------------------

    def test_deployed_file_starts_with_banner(self, tmp_path: Path) -> None:
        """PM_INSTRUCTIONS_DEPLOYED.md must begin with the AUTO-GENERATED banner.

        The banner must be the very first content so that any human who opens
        the file immediately sees the "do not edit" notice.
        """
        agents_path = tmp_path / "agents"
        agents_path.mkdir()
        working_dir = tmp_path / "project"
        ...
        assert deployed.startswith("<!-- AUTO-GENERATED by claude-mpm"), (
            "PM_INSTRUCTIONS_DEPLOYED.md must start with the AUTO-GENERATED banner. "
            "Banner was either missing or not the first content in the file."
        )
        assert "DO NOT EDIT" in deployed[:300], (
            "The AUTO-GENERATED banner must contain 'DO NOT EDIT'."
        )

    def test_deployed_file_is_deterministic(self, tmp_path: Path) -> None:
        """Deploying the same source files twice must produce byte-identical output.

        This verifies that the banner is stable (no timestamps, no per-run
        variation) and therefore never causes spurious file diffs on repeated
        startups.
        """
        agents_path = tmp_path / "agents"
        agents_path.mkdir()
        ...
        _make_system_files(agents_path)
        deployed_first = _deploy(tmp_path, working_dir, agents_path)
        deployed_second = _deploy(tmp_path, working_dir, agents_path)

        assert deployed_first == deployed_second, (
            "PM_INSTRUCTIONS_DEPLOYED.md content differs between two consecutive "
            "deployments of identical source files.  The banner must be fully "
            "deterministic (no timestamps or per-run variation)."
        )
