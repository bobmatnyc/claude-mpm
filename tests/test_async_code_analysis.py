...
class MockSocketIOServer:
    ...
    def broadcast_event(self, event_type: str, data: dict):
        ...
        if event_type in ["code:file:start", "code:node:found"]:
            self.logger.info(f"Event: {event_type} - {data.get('path', data.get('name', 'unknown'))[:50]}")
        elif event_type == "code:analysis:progress":
            self.logger.info(f"Progress: {data.get('percentage', 0):.1f}% - {data.get('message', '')}")
        else:
...
async def test_async_analysis():
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            ...
            if "code:analysis:complete" in event_types_seen or "code:analysis:error" in event_types_seen:
                break
