...
def main():
    ...
    print(f"   Found {tree_stats['files_found']} files in {tree_stats['directories_scanned']} directories")
    ...
    if result['nodes']:
        complex_nodes = sorted(
            [n for n in result['nodes'] if hasattr(n, 'complexity') and n.complexity > 5],
        ...
        if complex_nodes:
            ...
            for node in complex_nodes:
                print(f"   - {node.name} (complexity: {node.complexity}) in {Path(node.file_path).name}")
