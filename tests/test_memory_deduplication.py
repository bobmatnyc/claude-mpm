...
"""
        ...
        async_items = 0
        for line in lines:
            if line.strip().startswith("- ") and "async/await" in line and "database operations" in line:
                async_items += 1
        
        assert async_items == 1, f"Expected 1 async/await item after deduplication, found {async_items}"
...
def test_case_insensitive_matching():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        redis_items = 0
        for line in lines:
            if "redis" in line.lower() and "caching" in line.lower() and line.strip().startswith("- "):
                redis_items += 1
        
        assert redis_items == 1, f"Expected 1 Redis item after deduplication, found {redis_items}"
...
"""
    ...
    deduped_content, removed_count = content_manager.deduplicate_section(content, "Test Section")
    
    # Check that duplicates were removed
    assert removed_count >= 2, f"Should have removed at least 2 duplicates, removed {removed_count}"
    ...
    assert len(test_items) <= 3, f"Expected at most 3 unique items, found {len(test_items)}"
    assert any("error handling" in item for item in test_items), "Error handling item should remain"
