...
from claude_mpm.agents.agent_loader import (
    AgentLoader, get_agent_prompt, list_available_agents,
...
class TestAgentSystemE2E:
    ...
    def setup(self, tmp_path):
        ...
        self.performance_metrics['test_duration'] = time.time() - self.performance_metrics['test_start_time']
    
    def create_test_agent(self, agent_id: str, name: str, category: str = "engineering") -> Dict[str, Any]:
        """
    ...
    def test_agent_discovery_and_loading(self):
        ...
        # Validate results
        assert loader._metrics['agents_loaded'] == 2, f"Should load 2 valid agents, but loaded {loader._metrics['agents_loaded']}"
        assert loader._metrics['validation_failures'] == 1, "Should have 1 validation failure"
        ...
        # Record performance
        self.performance_metrics['operation_times']['discovery_and_loading'] = time.time() - start_time
        logger.info(f"Agent discovery completed in {self.performance_metrics['operation_times']['discovery_and_loading']:.3f}s")
    ...
    def test_agent_prompt_caching_and_performance(self):
        ...
        # Performance metrics
        logger.info(f"First load: {first_load_time:.6f}s, Cached load: {cached_load_time:.6f}s")
    ...
    def test_multi_agent_deployment_lifecycle(self):
        ...
        results = deployment_service.deploy_agents(
            target_dir=self.claude_agents_dir,
            force_rebuild=False
        )
    ...
    def test_concurrent_agent_operations(self):
        ...
        cache_hit_rate = loader._metrics['cache_hits'] / (loader._metrics['cache_hits'] + loader._metrics['cache_misses'])
        assert cache_hit_rate > 0.7, f"Cache hit rate {cache_hit_rate:.2%} should be > 70%"
    ...
    def test_agent_discovery_service_integration(self):
        ...
        for i in range(num_discovery_agents):
            ...
            self.create_test_agent(f"discovery_agent_{i}", f"Discovery Test {i}", category)
        ...
        discovery_service = DeployedAgentDiscovery(
            agents_dir=self.claude_agents_dir
        )
        ...
        analysis_agents = [a for a in discovered_agents if a.get('category') == 'analysis']
        assert len(analysis_agents) == 5  # 15 agents, every 3rd is 'analysis'
        
        # Performance check
        assert discovery_time < 1.0, f"Discovery of {num_discovery_agents} agents took {discovery_time:.3f}s (should be < 1s)"
        logger.info(f"Discovered {num_discovery_agents} agents in {discovery_time:.3f}s")
    ...
    def test_agent_handoff_simulation(self):
        ...
        research_prompt, research_model, research_config = get_agent_prompt_with_model_info(
            "research_test",
        ...
        engineer_prompt, engineer_model, engineer_config = get_agent_prompt_with_model_info(
            "engineer_test",
    ...
    def test_memory_and_resource_usage(self):
        ...
        for i in range(num_memory_agents):
            agent_data = self.create_test_agent(
                f"memory_test_{i}",
                f"Memory Test {i}"
            )
        ...
        logger.info(f"  After loading: {loaded_memory:.1f} MB (+{load_increase:.1f} MB)")
        logger.info(f"  After caching: {cached_memory:.1f} MB (+{cache_increase:.1f} MB)")
        logger.info(f"  After cleanup: {cleared_memory:.1f} MB (-{cleanup_reduction:.1f} MB)")
        
        # Validate reasonable memory usage
        avg_memory_per_agent = (cached_memory - initial_memory) / num_memory_agents
        assert avg_memory_per_agent < 1.0, f"Average memory per agent {avg_memory_per_agent:.2f} MB is too high"
    ...
    def test_production_readiness_checks(self):
        ...
        logger.info(f"Production readiness test results ({operation_count} operations):")
        ...
        assert avg_duration < 0.1, f"Average duration {avg_duration:.3f}s exceeds 100ms threshold"
