...
class TestMemoryGuardianE2E:
    ...
    @pytest.mark.asyncio
    async def test_real_subprocess_memory_monitoring(
        self, temp_workspace, memory_growth_script
    ):
        ...
        try:
            ...
            if len(memory_readings) >= 2:
                ...
                ), f"Memory should grow: {initial_memory}MB -> {final_memory}MB"
            ...
            if final_memory > config.thresholds.warning:
                assert (
                    MemoryState.WARNING in states_seen
                ), "Should reach warning state with real memory growth"
...
"""
    ...
    @pytest.mark.asyncio
    async def test_interrupt_handling_and_graceful_shutdown(
        self, temp_workspace, memory_growth_script
    ):
        ...
        try:
            ...
            assert guardian.process_state in [
...
"""
        ...
        try:
            ...
            assert (
                restart_count > 0
            ), "Should have attempted restart after process crash"
    ...
    async def test_large_state_file_handling(self, temp_workspace):
        ...
        large_state = {
            "conversations": [],
            "metadata": {"version": "1.0.0", "created": time.time()},
        }

        # Generate large conversation data
        for conv_id in range(100):
            conversation = {"id": f"conv_{conv_id}", "messages": []}
                ...
                conversation["messages"].append(
                    {
                        ...
                        "content": f"Test message content {msg_id} "
                        * 100,  # Long content
                        "timestamp": time.time(),
                    }
                )
        ...
        try:
            ...
            assert (
                save_time < 10
            ), f"Save took too long: {save_time}s for {file_size_mb:.1f}MB"
            ...
            assert (
                load_time < 15
            ), f"Load took too long: {load_time}s for {file_size_mb:.1f}MB"
...
"""
            ...
            assert all(
                start_results
            ), "All guardian instances should start successfully"
            ...
            for i, guardian in enumerate(guardians):
                ...
                ), f"Guardian {i} process should be running"
    ...
    async def test_unix_process_groups(self, tmp_path):
        ...
        config = MemoryGuardianConfig(
            ...
            process_command=[
                sys.executable,
                "-c",
                'import time, os; print(f"Process group: {os.getpgrp()}"); time.sleep(10)',
            ],
            working_directory=str(tmp_path),
        )
    ...
    async def test_windows_process_termination(self, tmp_path):
        ...
        config = MemoryGuardianConfig(
            ...
            process_command=[
                sys.executable,
                "-c",
                'import time, os; print(f"PID: {os.getpid()}"); time.sleep(10)',
            ],
            working_directory=str(tmp_path),
        )
