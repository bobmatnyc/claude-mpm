...
class TestAgentDiscoveryService:
    ...
    def temp_templates_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            incomplete_template.write_text(json.dumps({
                "schema_version": "1.2.0",
    ...
    def test_is_agent_excluded_explicit(self, discovery_service):
        ...
        assert not discovery_service._is_agent_excluded('security-agent', excluded_agents, None)
    ...
    def test_is_agent_excluded_case_insensitive(self, discovery_service):
        ...
        assert discovery_service._is_agent_excluded('test-agent', excluded_agents, config)
        assert not discovery_service._is_agent_excluded('security-agent', excluded_agents, config)
    ...
    def test_is_valid_agent_name(self, discovery_service):
        ...
        assert not discovery_service._is_valid_agent_name('test--agent')  # double hyphen
        assert not discovery_service._is_valid_agent_name('-test-agent')  # starts with hyphen
        assert not discovery_service._is_valid_agent_name('test-agent-')  # ends with hyphen
        assert not discovery_service._is_valid_agent_name('123-agent')  # starts with number
