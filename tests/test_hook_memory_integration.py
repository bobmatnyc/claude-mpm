...
"""
        ...
        fresh_memory_file = temp_dir / ".claude-mpm" / "memories" / f"{fresh_agent_id}_memories.md"
        
        # Add initial memory
        memory_manager.add_learning(fresh_agent_id, "Initial memory item")
        
        # Now add remember field
        success = memory_manager.extract_and_update_memory(fresh_agent_id, remember_agent_response)
        ...
        assert "- Initial memory item" in fresh_content, "Initial memory should be preserved"
        for memory in remember_data:
            assert f"- {memory}" in fresh_content, f"Remember field memory should be added: {memory}"
...
"""
        ...
        success = memory_manager.extract_and_update_memory(mixed_agent_id, inline_response)
        assert success, "Inline JSON extraction should succeed"
        
        mixed_memory_file = temp_dir / ".claude-mpm" / "memories" / f"{mixed_agent_id}_memories.md"
        mixed_content = mixed_memory_file.read_text()
        
        assert "- Inline JSON memory" in mixed_content, "Inline JSON memory should be extracted"
...
```
""",
                "expected": ["Second block replaces all"]  # MEMORIES should replace remember
            }
        ]
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            for expected_memory in expected:
                assert f"- {expected_memory}" in content, f"Should find expected memory: {expected_memory}"
