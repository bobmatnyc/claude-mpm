...
def measure_performance():
    ...
    print(f"   First call (cache miss): {first_call:.3f}s - Found {len(agents1)} agents")
    ...
    first_call = time.time() - start
    print(f"   First call (cache miss): {first_call:.3f}s - Generated {len(caps1)} chars")
    ...
    first_call = time.time() - start
    memories_loaded = len(content.get("actual_memories", "")) if "actual_memories" in content else 0
    print(f"   First call (cache miss): {first_call:.3f}s - Loaded {memories_loaded} bytes")
    ...
    speedup = total_time_uncached / total_time_cached if total_time_cached > 0 else float('inf')
    ...
    if total_time_cached < 5.0:
        ...
        print(f"   Original estimated time: ~11.9s")
