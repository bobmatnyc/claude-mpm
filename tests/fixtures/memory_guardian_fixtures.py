...
class MockProcessConfig:
    ...
    memory_pattern: str = (
        "linear"  # 'linear', 'exponential', 'sawtooth', 'random', 'stable'
    )
...
class MockClaudeProcess:
    ...
    def poll(self) -> Optional[int]:
        ...
        with self._lock:
            ...
            if (
                self.config.crash_at_limit
                and self.current_memory >= self.config.max_memory_mb
            ):
                self.is_running = False
    ...
    def _update_memory(self):
        ...
        if pattern == "linear":
            ...
            self.current_memory = min(
                self.config.initial_memory_mb + growth, self.config.max_memory_mb
            )
            ...
            self.current_memory = self.config.initial_memory_mb + random.uniform(
                -noise, noise
            )
        ...
        # Ensure bounds
        self.current_memory = max(
            1.0, min(self.current_memory, self.config.max_memory_mb)
        )
    ...
    def generate_medium_state() -> Dict[str, Any]:
        ...
        for i in range(10):
                ...
                messages.append(
                    {
                        ...
                        "content": f"Test message {j} with content. "
                        * 50,  # ~1KB per message
                        "timestamp": time.time(),
                    }
                )
            ...
            conversations.append(
                {"id": f"conv_{i}", "created": time.time(), "messages": messages}
            )
                ...
                messages.append(
                    {
                        "id": f"msg_{j}",
                        "role": "user" if j % 2 == 0 else "assistant",
                        "content": f"Large test message {j} with substantial content. "
                        * 200,  # ~10KB per message
                        "timestamp": time.time(),
                        "metadata": {
            ...
            conversations.append(
                {
                    ...
                    "metadata": {
                        "total_tokens": sum(
                            msg["metadata"]["tokens"] for msg in messages
                        ),
...
class ConfigurationBuilder:
    ...
    def performance_test_config(workspace: Path) -> MemoryGuardianConfig:
        ...
        return MemoryGuardianConfig(
            ...
            restart_policy=RestartPolicy(
                max_attempts=5, graceful_timeout=1, initial_cooldown=0.1
            ),
            monitoring=MonitoringConfig(
...
class MemoryPatternGenerator:
    ...
    @staticmethod
    def linear_growth(
        duration: float, start_mb: float, end_mb: float, steps: int
    ) -> List[float]:
    ...
    def exponential_growth(
        duration: float, start_mb: float, growth_rate: float, steps: int
    ) -> List[float]:
        """Generate exponential memory growth pattern."""
        return [
            start_mb * (growth_rate ** (i / steps * duration)) for i in range(steps)
        ]
    ...
    @staticmethod
    def random_walk(
        duration: float, start_mb: float, volatility: float, steps: int
    ) -> List[float]:
...
def mock_claude_process_chaotic():
    ...
    config = MockProcessConfig(
        initial_memory_mb=200, memory_pattern="random", pattern_params={"variance": 100}
    )
    return MockClaudeProcess(config)
...
def memory_pattern_linear():
    ...
    return MemoryPatternGenerator.linear_growth(
        duration=30, start_mb=100, end_mb=800, steps=100
    )
...
def memory_pattern_realistic():
    ...
    return MemoryPatternGenerator.realistic_claude_pattern(
        duration=300, steps=150  # 5 minutes
    )
...
def assert_monitoring_activity(guardian: MemoryGuardian, min_samples: int = 1):
    ...
    assert (
        guardian.memory_stats.samples >= min_samples
    ), f"Should have at least {min_samples} memory samples"
    assert guardian.memory_stats.last_check > 0, "Should have recorded memory checks"


def assert_state_transitions(
    guardian: MemoryGuardian, expected_states: List[MemoryState]
):
    ...
    assert (
        guardian.memory_state in expected_states
    ), f"Memory state {guardian.memory_state} not in expected states {expected_states}"


def assert_restart_behavior(
    guardian: MemoryGuardian, expected_restarts: int = None, max_restarts: int = None
):
    ...
    if expected_restarts is not None:
        ...
        ), f"Expected {expected_restarts} restarts, got {actual_restarts}"

    if max_restarts is not None:
        assert (
            actual_restarts <= max_restarts
        ), f"Too many restarts: {actual_restarts} > {max_restarts}"
