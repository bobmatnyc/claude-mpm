...
class TestDaemonProcessManagement:
    ...
    def test_daemon_start_when_not_running(self, mock_paths):
        ...
        """
        with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', mock_paths["pid_file"]):
            with patch('claude_mpm.scripts.socketio_daemon.LOG_FILE', mock_paths["log_file"]):
                with patch('claude_mpm.scripts.socketio_daemon.is_running', return_value=False):
                    with patch('os.fork', return_value=12345) as mock_fork:
    ...
    def test_daemon_start_when_already_running(self, mock_paths):
        ...
        """
        with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', mock_paths["pid_file"]):
            with patch('claude_mpm.scripts.socketio_daemon.is_running', return_value=True):
    ...
    def test_daemon_stop_when_running(self, mock_paths):
        ...
        with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', mock_paths["pid_file"]):
            with patch('claude_mpm.scripts.socketio_daemon.is_running', side_effect=[True, False]):
    ...
    def test_daemon_force_kill_on_hung_process(self, mock_paths):
        ...
        with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', mock_paths["pid_file"]):
            with patch('claude_mpm.scripts.socketio_daemon.is_running', return_value=True):  # Always running
                with patch('os.kill') as mock_kill:
                ...
                with patch('os.fork', return_value=0):  # Child process
                    with patch('os.setsid'):
                        with patch('builtins.open', mock_open()):
                            with patch('claude_mpm.scripts.socketio_daemon.SocketIOServer') as mock_server:
                                # Setup mock server
        ...
        with patch('claude_mpm.scripts.socketio_daemon.PortManager') as mock_pm_class:
            ...
            with patch('claude_mpm.scripts.socketio_daemon.is_running', return_value=False):
                with patch('builtins.print') as mock_print:
                    ...
                    mock_print.assert_any_call("⚠️  Port 8765 is already used by claude-mpm instance:")
...
class TestSignalHandling:
    ...
    def test_sigterm_handler_graceful_shutdown(self):
        ...
        and exit cleanly.
        """
        with patch('claude_mpm.scripts.socketio_daemon.SocketIOServer') as mock_server_class:
        ...
        with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', pid_file):
            with patch('claude_mpm.scripts.socketio_daemon.is_running', return_value=True):
                with patch('claude_mpm.scripts.socketio_daemon.PortManager') as mock_pm:
                    ...
                    assert f"Socket.IO daemon server is running (PID: {os.getpid()})" in captured.out
...
def mock_daemon_environment(tmp_path):
    ...
    with patch('claude_mpm.scripts.socketio_daemon.PID_FILE', paths["pid_file"]):
        with patch('claude_mpm.scripts.socketio_daemon.LOG_FILE', paths["log_file"]):
            with patch('claude_mpm.scripts.socketio_daemon.get_project_root', return_value=tmp_path):
                yield paths
...
def test_full_daemon_lifecycle(mock_daemon_environment):
    ...
    with patch('os.fork', return_value=12345):  # Parent process
        with patch('claude_mpm.scripts.socketio_daemon.PortManager') as mock_pm:
            with patch('claude_mpm.scripts.socketio_daemon.is_running', side_effect=[False, True, True, False]):
                with patch('os.kill'):
                    with patch('sys.exit'):
                        # Setup port manager
                        mock_pm_instance = MagicMock()
