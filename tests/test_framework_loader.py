...
class TestFrameworkLoader:
    ...
    def test_no_framework_found(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "claude_mpm.core.framework_loader.__file__", str(tmp_path / "dummy.py")
        )
    ...
    def test_get_framework_instructions_minimal(self):
        ...
        assert (
            "delegate" in instructions.lower() or "orchestrat" in instructions.lower()
        )
    ...
    def test_npm_global_path_detection(self, mock_run, tmp_path):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout=str(npm_root) + "\n")
