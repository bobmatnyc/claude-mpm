...
class TestMemoryFixesVerification:
    ...
    def test_pm_memory_persistence_to_user_directory(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            assert user_pm_file.exists(), "PM memory should be created in user directory"
            assert not project_pm_file.exists(), "PM memory should NOT be created in project directory"
            
            # Test PM memory updates also go to user directory
            success = memory_manager.add_learning("PM", "guideline", "Always delegate to specialized agents")
            assert success, "PM memory update should succeed"
            
            # Verify update went to user directory
            updated_content = user_pm_file.read_text()
            assert "Always delegate to specialized agents" in updated_content, "PM memory should be updated in user directory"
    ...
    def test_directory_handling_pm_vs_others(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            for agent_id in ["engineer", "research", "qa", "ops"]:
                ...
                project_agent_file = self.project_memories_dir / f"{agent_id}_memories.md"
                
                assert project_agent_file.exists(), f"{agent_id} should use project directory"
                assert not user_agent_file.exists(), f"{agent_id} should NOT use user directory"
...
"""
            ...
            assert not old_pm_file.exists(), "Old PM file should be removed after backup"
            assert "Always delegate to specialized agents" in pm_memory, "PM content should be preserved"
            
            # Verify engineer migration without backup (normal deletion)
            new_engineer_file = self.project_memories_dir / "engineer_memories.md"
            engineer_backup_file = self.project_memories_dir / "engineer_agent.md.backup"
            
            assert new_engineer_file.exists(), "New engineer memory file should exist"
            assert not engineer_backup_file.exists(), "Engineer backup should NOT be created"
            assert not old_engineer_file.exists(), "Old engineer file should be deleted"
            assert "Use type hints in Python code" in engineer_memory, "Engineer content should be preserved"
    ...
    def test_cross_project_pm_memory_accessibility(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            success = memory_manager1.add_learning("PM", "guideline", "Always use TodoWrite for task tracking")
            assert success, "PM memory should be added successfully"
            
            # Switch to second project
            memory_manager2 = AgentMemoryManager(
                config=self.config, 
                working_directory=self.test_project_dir2
            )
            ...
            assert "Always use TodoWrite for task tracking" in pm_memory_project2, "PM memory should persist across projects"
            ...
            assert not project1_pm_file.exists(), "PM memory should NOT exist in project 1"
            assert not project2_pm_file.exists(), "PM memory should NOT exist in project 2"
    ...
    def test_other_agents_use_project_directory_correctly(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            for agent_id in agents:
                ...
                success = memory_manager1.add_learning(agent_id, "guideline", f"Project 1 guideline for {agent_id}")
                ...
                assert project_agent_file.exists(), f"{agent_id} memory should exist in project directory"
                assert not user_agent_file.exists(), f"{agent_id} memory should NOT exist in user directory"
            ...
            for agent_id in agents:
                ...
                success = memory_manager2.add_learning(agent_id, "guideline", f"Project 2 guideline for {agent_id}")
                assert success, f"Memory should be added for {agent_id} in project 2"
                
                # Verify memory is in project 2 directory
                project2_agent_file = self.project_memories_dir2 / f"{agent_id}_memories.md"
                assert project2_agent_file.exists(), f"{agent_id} memory should exist in project 2 directory"
                ...
                assert project1_memory != project2_memory, f"{agent_id} memories should be different between projects"
...
'''
            ...
            success = memory_manager.extract_and_update_memory("engineer", json_response)
...
'''
            ...
            assert not project_pm_file.exists(), "PM memory should NOT be saved to project directory"
    ...
    def test_memory_directory_creation(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            assert self.user_memories_dir.exists(), "User memories directory should be created"
            assert self.project_memories_dir.exists(), "Project memories directory should be created"
    ...
    def test_pre_delegation_hook_memory_injection(self):
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            success = memory_manager.add_learning("engineer", "pattern", "Use dependency injection for better testability")
            ...
            with patch('os.getcwd', return_value=str(self.test_project_dir)):
                ...
                assert "dependency injection for better testability" in updated_context["agent_memory"]
                assert "AGENT MEMORY - PROJECT-SPECIFIC KNOWLEDGE" in updated_context["agent_memory"]
