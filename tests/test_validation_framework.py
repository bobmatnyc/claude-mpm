...
from claude_mpm.hooks.validation_hooks import (
    ValidationHooks, validate_agent_dependencies, 
    validate_security_constraints, ValidationError
)
...
class TestAgentValidator:
    ...
    def test_overrides(self, validator):
        ...
        try:
            ...
            result = validator_with_overrides.validate_agent_config(config, 'test_agent')
            assert 'role' in result.locked_fields
            
            # Test skip validation
            config = {'name': 'skip_agent'}
            result = validator_with_overrides.validate_agent_config(config, 'skip_agent')
...
class TestValidationHooks:
    ...
    async def test_pre_execute_validation(self, hooks):
        ...
        # Valid task
        result = await hooks.run_pre_execute_validation('test_agent', 'Analyze this code')
        assert result.is_valid
    ...
    async def test_security_validation(self):
        ...
        # Dangerous task
        result = await validate_security_constraints('agent', 'rm -rf / --no-preserve-root')
        assert not result.is_valid
