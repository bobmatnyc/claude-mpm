...
class TestMemoryIntegrationFocused(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock_config.get.side_effect = lambda key, default=None: self._get_nested_config(config_data, key, default)
        
        # Create memory manager with config and working directory
        self.memory_manager = AgentMemoryManager(config=mock_config, working_directory=self.working_dir)
