...
class TestConfigNoDuplicateLogging(TestCase):
    ...
    def test_single_success_message():
        ...
        with self.assertLogs('claude_mpm.core.config', level=logging.INFO) as cm:
            ...
            config1 = Config(config_file=Path.cwd() / ".claude-mpm" / "configuration.yaml")
            config2 = Config(config_file=Path.cwd() / ".claude-mpm" / "configuration.yaml")
            config3 = Config()
            
            # Count success messages
            success_messages = [
                msg for msg in cm.output 
                if "✓ Successfully loaded configuration" in msg
            ]
    ...
    def test_reload_prevention():
        ...
        with self.assertLogs('claude_mpm.core.config', level=logging.DEBUG) as cm:
            ...
            skip_messages = [
                msg for msg in cm.output
                if "skipping reload" in msg.lower()
            ]
