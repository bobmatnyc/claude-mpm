...
def test_glob_patterns():
    ...
    with tmp_path as tmpdir:
        ...
        expected_memories = ["PM_memories.md", "Engineer_memories.md", "Research_memories.md", "QA_memories.md"]
        ...
        non_memory_files = ["README.md", "NOTES.md", "old_agent.md", "legacy.md", "documentation.md"]
        all_excluded = True
        ...
        print(f"✓ New pattern (*_memories.md) matched: {len(new_pattern_names)} files (CORRECT)")
        print(f"✓ Filtered out {len(excluded_files)} non-memory files")
        
        # Assert correctness
        assert len(new_pattern_names) == 4, f"Expected 4 memory files, got {len(new_pattern_names)}"
        assert all(name.endswith("_memories.md") for name in new_pattern_names), "All matched files should end with _memories.md"
