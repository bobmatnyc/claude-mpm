...
def test_memory_injection_with_deployment_check():
    ...
    if user_memories_dir.exists():
        ...
        for memory_file in sorted(user_memories_dir.glob("*_memories.md")):
            ...
            memories_to_check.append((memory_file, agent_name, agent_name in deployed, "user"))
    ...
    if project_memories_dir.exists():
        ...
        for memory_file in sorted(project_memories_dir.glob("*_memories.md")):
            ...
            status = "✓ DEPLOYED" if agent_name in deployed else "✗ NOT DEPLOYED"
            print(f"  {status:<15} {memory_file.name:<40} (agent: {agent_name})")
            memories_to_check.append((memory_file, agent_name, agent_name in deployed, "project"))
    ...
    try:
        ...
        content = loader.framework_content if hasattr(loader, 'framework_content') else {}
    ...
    loaded_logs = [msg for level, msg in log_capture if "Loaded" in msg and "memory" in msg.lower()]
    ...
    summary_logs = [msg for level, msg in log_capture if "Memory loading complete" in msg]
    
    print(f"\n✅ Loaded memories ({len(loaded_logs)}):")
    for msg in loaded_logs:
        # Extract key info from message
        if "PM memory" in msg:
            print(f"  • PM memory loaded")
    ...
    if not skipped_logs:
        ...
        for msg in skipped_logs:
            ...
            if "agent" in msg and "not deployed" in msg:
                import re
                match = re.search(r"(\w+_memories\.md).*agent '([\w\s]+)' not deployed", msg)
                if match:
    ...
    if "agent_memories" in content:
        ...
        print("ℹ️  No agent memories in framework_content (all agents might be non-deployed)")
    ...
    if "## Agent Memories" in instructions:
        ...
        agent_count = instructions.count("### ") - instructions.count("### Context-Aware")
    ...
    for memory_file, agent_name, is_deployed, source in memories_to_check:
        if not is_deployed:
            ...
            if any(skip_msg in msg for level, msg in log_capture if "Skipped" in msg):
                checks_passed.append(f"Non-deployed agent '{agent_name}' was properly skipped")
            else:
                checks_failed.append(f"Non-deployed agent '{agent_name}' was not properly skipped")
    
    if not non_deployed_found:
        checks_passed.append("No non-deployed agents found (all memories are for deployed agents)")
    ...
    for memory_file, agent_name, is_deployed, source in memories_to_check:
        if is_deployed:
            ...
            if agent_name not in content.get("agent_memories", {}):
                checks_failed.append(f"Deployed agent '{agent_name}' memory was not loaded")
            else:
