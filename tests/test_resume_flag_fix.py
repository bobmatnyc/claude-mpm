...
def test_filter_function():
    ...
    assert "--resume" in filtered, f"Expected --resume in filtered args, got: {filtered}"
    print("✓ --resume passes through filter")
    
    # Test 2: --mpm-resume should be filtered out
    test_args = ["--mpm-resume", "last", "--resume"]
    filtered = filter_claude_mpm_args(test_args)
    assert "--mpm-resume" not in filtered, f"--mpm-resume should be filtered, got: {filtered}"
...
def test_argument_parsing():
    ...
    assert args.resume == True, f"resume should be True, got: {args.resume}"
    assert args.mpm_resume == "last", f"mpm_resume should be 'last', got: {args.mpm_resume}"
...
def test_command_construction():
    ...
    assert "--resume" in args.claude_args, f"Expected --resume in claude_args, got: {args.claude_args}"
    ...
    args.resume = False
    _ensure_run_attributes(args)
    assert "--resume" not in args.claude_args, f"--resume should not be in claude_args, got: {args.claude_args}"
    ...
    _ensure_run_attributes(args)
    assert args.claude_args.count("--resume") == 1, f"--resume should appear once, got: {args.claude_args}"
