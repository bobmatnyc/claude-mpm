...
class TestStateManagerService(unittest.TestCase):
    ...
    def test_track_delegation(self):
        ...
        self.assertEqual(
            self.state_manager.active_delegations[session_id]["agent_type"],
            agent_type
        )
        ...
        self.assertEqual(
            self.state_manager.delegation_history[session_id]["agent_type"],
            agent_type
        )
        ...
        if request_data:
            ...
            self.assertEqual(
                self.state_manager.delegation_requests[session_id],
                request_data
            )
...
class TestEventTransformation(unittest.TestCase):
    ...
    def test_claude_to_internal_format(self):
        ...
        internal_event = {
            ...
            'data': {
                'response': claude_event.get('response')
            }
        }
...
class TestBatchProcessing(unittest.TestCase):
    ...
    def test_batch_filtering(self):
        ...
        filtered = [
            e for e in events
            if e.get('hook_event_name') in valid_events
        ]
...
class TestEventRouting(unittest.TestCase):
    ...
    def test_priority_routing(self):
        ...
        sorted_events = sorted(
            events,
            key=lambda e: priorities.get(e.get('hook_event_name', ''), 999)
        )
