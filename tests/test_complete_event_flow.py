            ...
            try:
                ...
                async with session.post(url, json=event) as response:
                    if response.status == 204:
                        print(f"   ✅ Success: Server returned 204 No Content")
                    else:
    ...
    try:
        ...
        if received_events:
            ...
            for event in received_events[:3]:  # Show first 3
                print(f"   - {event.get('subtype', 'unknown')}: {event.get('type', 'unknown')}")
        else:
        ...
        try:
            async with session.get("http://localhost:8765/health") as response:
                if response.status == 200:
                    ...
                    print(f"   - Events sent: {data.get('stats', {}).get('events_sent', 0)}")
                    print(f"   - Events buffered: {data.get('stats', {}).get('events_buffered', 0)}")
                else:
...
async def main():
    ...
    print("4. If events still don't appear, the issue is likely with Claude's event format")
