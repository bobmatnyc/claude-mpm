...
def test_dashboard_errors():
    ...
    print("SocketIO server started on port 8080")
    ...
    for i, event in enumerate(test_events, 1):
        ...
        print(
            f"  Sent event {i}: {event.get('type')}.{event.get('subtype', 'null')} with hook_event_name={event.get('hook_event_name', 'undefined')}"
        )
        time.sleep(0.5)
