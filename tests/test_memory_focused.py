...
def test_basic_memory_operations():
    ...
    try:
        ...
        content = memory_file.read_text()
        assert "- Remember field item 1" in content, "Remember field item 1 should be added"
        assert "- Remember field item 2" in content, "Remember field item 2 should be added"
...
def test_multiple_agents():
    ...
    try:
        ...
        for agent_id in agents:
            ...
            success = memory_manager.add_learning(agent_id, f"Memory for {agent_id} agent")
            ...
            # Check content
            content = memory_file.read_text()
            assert f"- Memory for {agent_id} agent" in content, f"Memory should be saved for {agent_id}"
        ...
        assert len(memory_files) == len(agents), f"Should have {len(agents)} memory files"
...
def test_deduplication():
    ...
    try:
        ...
        assert "completely different" in content.lower(), "Unique content should be preserved"
