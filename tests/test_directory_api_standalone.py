...
async def test_directory_api():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        test_cases = [
            ...
            {'path': '/Users/masa/Projects/claude-mpm/src/claude_mpm', 'name': 'claude_mpm directory'},
            {'path': '/Users/masa/Projects/claude-mpm/scripts', 'name': 'scripts directory'},
        ...
        for test_case in test_cases:
            ...
            try:
                url = f"http://localhost:8767/api/directory/list?path={test_case['path']}"
                async with session.get(url) as response:
                    ...
                    if data.get('contents'):
                        ...
                        if len(data['contents']) > 10:
                            print(f"     ... and {len(data['contents']) - 10} more items")
                    else:
                        if data.get('error'):
