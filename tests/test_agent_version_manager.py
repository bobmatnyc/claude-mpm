...
from claude_mpm.services.agents.deployment.agent_version_manager import AgentVersionManager
...
class TestAgentVersionManager:
    ...
    def test_parse_version_integer(self, version_manager):
        """Test parsing integer versions."""
        result = version_manager.parse_version(5)
        assert result == (0, 5, 0)
    
    def test_parse_version_string_integer(self, version_manager):
        """Test parsing string integer versions."""
        result = version_manager.parse_version("5")
        assert result == (0, 5, 0)
    
    def test_parse_version_semantic(self, version_manager):
    ...
    def test_parse_version_extract_number(self, version_manager):
        """Test extracting number from complex strings."""
        result = version_manager.parse_version("version-5-something")
        assert result == (0, 5, 0)
    
    def test_format_version_display(self, version_manager):
    ...
    def test_format_version_display_invalid(self, version_manager):
        """Test version display formatting with invalid input."""
        result = version_manager.format_version_display("invalid")
        assert result == "invalid"
    
    def test_is_old_version_format(self, version_manager):
        ...
        for version_str in old_formats:
            assert version_manager.is_old_version_format(version_str), f"Should be old: {version_str}"
        
        for version_str in new_formats:
            assert not version_manager.is_old_version_format(version_str), f"Should be new: {version_str}"
    ...
    def test_extract_version_from_content(self, version_manager):
        ...
        agent_version = version_manager.extract_version_from_content(content, "AGENT_VERSION:")
        assert agent_version == 5
        
        base_version = version_manager.extract_version_from_content(content, "BASE_AGENT_VERSION:")
        assert base_version == 3
        
        missing_version = version_manager.extract_version_from_content(content, "MISSING:")
        assert missing_version == 0
    
    def test_extract_version_from_frontmatter_semantic(self, version_manager):
...
Content here"""
        ...
        version_tuple, is_old, version_str = version_manager.extract_version_from_frontmatter(content)
        assert version_tuple == (2, 1, 0)
        assert not is_old
        assert version_str == "2.1.0"
    
    def test_extract_version_from_frontmatter_legacy(self, version_manager):
...
Content here"""
    ...
    def test_extract_version_from_frontmatter_old_separate(self, version_manager):
...
Content here"""
    ...
    def test_extract_version_from_frontmatter_missing(self, version_manager):
...
Content here"""
    ...
    def test_check_agent_needs_update_not_system(self, version_manager, temp_dir):
...
User created agent""")
    ...
    def test_check_agent_needs_update_migration_needed(self, version_manager, temp_dir):
...
System agent""")
    ...
    def test_check_agent_needs_update_template_newer(self, version_manager, temp_dir):
...
System agent""")
    ...
    def test_check_agent_needs_update_up_to_date(self, version_manager, temp_dir):
...
System agent""")
    ...
    def test_validate_version_in_content_valid(self, version_manager):
...
Content here"""
    ...
    def test_validate_version_in_content_missing_frontmatter(self, version_manager):
    ...
    def test_validate_version_in_content_missing_version(self, version_manager):
...
Content here"""
    ...
    def test_validate_version_in_content_old_format(self, version_manager):
