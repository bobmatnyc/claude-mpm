...
def run_final_integration_test():
    ...
    try:
        ...
        with unittest.mock.patch('pathlib.Path.home', return_value=test_user_home):
            ...
            user_pm_file = test_user_home / '.claude-mpm' / 'memories' / 'PM_memories.md'
            project_pm_file = test_project_dir / '.claude-mpm' / 'memories' / 'PM_memories.md'
            
            assert user_pm_file.exists(), 'PM memory should be in user directory'
            assert not project_pm_file.exists(), 'PM memory should NOT be in project directory'
            ...
            user_engineer_file = test_user_home / '.claude-mpm' / 'memories' / 'engineer_memories.md'
            project_engineer_file = test_project_dir / '.claude-mpm' / 'memories' / 'engineer_memories.md'
            
            assert project_engineer_file.exists(), 'Engineer memory should be in project directory'
            assert not user_engineer_file.exists(), 'Engineer memory should NOT be in user directory'
...
'''
            ...
            memory_manager.add_learning('PM', 'guideline', 'Cross-project test successful')
