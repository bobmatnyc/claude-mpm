...
class MemorySystemTester:
    ...
    def create_temp_project(
        self, with_project_memory=False, with_project_workflow=False
    ):
...
"""
    ...
    def test_memory_loading_order(self):
        ...
        try:
            ...
            temp_project = self.create_temp_project(
                with_project_memory=True, with_project_workflow=True
            )
            ...
            try:
                ...
                self.log_result(
                    "MEMORY.md and WORKFLOW.md both loaded",
                ...
                project_memory_used = (
                    loader.framework_content.get("project_memory") == "project"
                )
                project_workflow_used = (
                    loader.framework_content.get("project_workflow") == "project"
                )

                self.log_result(
                    "Project-specific MEMORY.md override used",
                    project_memory_used,
                    f"Project memory override: {project_memory_used}",
                )

                self.log_result(
                    "Project-specific WORKFLOW.md override used",
                    project_workflow_used,
                ...
                if has_workflow and has_memory:
                    ...
                    if workflow_pos != -1 and memory_pos != -1:
                        ...
                        self.log_result(
                            "MEMORY.md loads AFTER WORKFLOW.md",
                            order_correct,
    ...
    def test_memory_limits(self):
        ...
        try:
            ...
            self.log_result(
                "Research agent gets 120KB override",
                research_limit == 120,
            ...
            self.log_result(
                "Token calculation logic exists",
        ...
        try:
            ...
            if system_memory_path.exists():
                ...
                has_memory_update_process = (
                    "Memory Update Process (PM Instructions)" in content
                )
                ...
                self.log_result(
                    "PM Memory Update Process documented",
                    has_memory_update_process,
                    "Found PM memory update instructions",
                )

                self.log_result(
                    "Memory trigger words defined",
                    has_trigger_words,
                ...
                self.log_result(
                    "Agent memory routing matrix exists",
                    has_agent_routing,
                ...
                self.log_result(
                    "PM Memory instructions file exists", False, "MEMORY.md not found"
                )
...
"""
            ...
            self.log_result(
                "Can save agent memory", True, f"Memory save attempted for {test_agent}"
            )
            ...
            self.log_result(
                "Can load agent memory",
                has_content,
                f"Loaded {len(loaded_content or '')} chars",
            )

            # Test memory file location
            expected_path = (
                temp_project / ".claude-mpm" / "memories" / f"{test_agent}_agent.md"
            )
            file_exists = expected_path.exists()
            self.log_result(
                "Memory file saved in correct location",
                file_exists,
            ...
            if loaded_content:
                ...
                single_line_facts = all(
                    len(line.strip()) < 200 for line in fact_lines
                )  # Reasonable line length
                self.log_result(
                    "Single-line fact format maintained",
                    single_line_facts,
    ...
    def test_fallback_to_system_memory(self):
        ...
        try:
            ...
            try:
                ...
                system_memory_used = (
                    loader.framework_content.get("project_memory") == "system"
                )
                has_memory = "memory_instructions" in loader.framework_content

                self.log_result(
                    "Falls back to system MEMORY.md",
                    system_memory_used and has_memory,
                    f"System memory used: {system_memory_used}, Has memory: {has_memory}",
                )

                self.log_result(
                    "Does not incorrectly use project override",
