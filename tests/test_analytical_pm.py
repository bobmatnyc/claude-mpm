...
class AnalyticalPMValidator:
    def __init__(self):
        ...
        self.instructions_path = self.project_root / "src/claude_mpm/agents/INSTRUCTIONS.md"
    ...
    def load_files(self) -> Tuple[str, str]:
        ...
        try:
            with open(self.base_pm_path, 'r', encoding='utf-8') as f:
        ...
        for pattern in self.forbidden_patterns:
            ...
            for i, line in enumerate(lines):
                if re.search(pattern, line, re.IGNORECASE):
                    ...
                    is_forbidden_context = any(ctx in context for ctx in forbidden_contexts)
                    
                    if not is_forbidden_context:
                        issues.append(f"Found forbidden pattern '{pattern}' in non-context usage: line {i+1}")
        ...
        for phrase in problem_phrases:
            for i, line in enumerate(lines):
                if phrase.lower() in line.lower():
                    ...
                    if not is_forbidden_context:
                        issues.append(f"Found problematic phrase '{phrase}' in non-context usage: line {i+1}")
        
        return {
            "status": "PASS" if not issues else "FAIL",
            "issues": issues,
            "forbidden_patterns_found": len([i for i in issues if "forbidden pattern" in i]),
    ...
    def test_section_presence(self, content: str) -> Dict[str, Any]:
        ...
        return {
            "status": "PASS" if len(found_sections) == len(self.required_sections) else "FAIL", 
            "issues": issues,
    ...
    def test_analytical_examples(self, content: str) -> Dict[str, Any]:
        ...
        for pattern in example_patterns:
            ...
            for match in matches:
                ...
                if any(indicator in match.lower() for indicator in analytical_indicators):
                    analytical_examples += 1
        
        if examples_found == 0:
            issues.append("No examples found in instructions")
        elif analytical_examples / examples_found < 0.7:
            issues.append(f"Only {analytical_examples}/{examples_found} examples use analytical approach")
        ...
        return {
            ...
            "analytical_percentage": (analytical_examples / examples_found * 100) if examples_found > 0 else 0
        }
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run all tests
            self.validation_results["forbidden_language_check"] = self.test_forbidden_language_removal(combined_content)
            self.validation_results["required_patterns_check"] = self.test_required_analytical_patterns(combined_content)
            self.validation_results["section_presence_check"] = self.test_section_presence(combined_content)
            self.validation_results["json_format_check"] = self.test_json_format_requirements(combined_content)
            self.validation_results["analytical_examples_check"] = self.test_analytical_examples(combined_content)
            
            # Calculate overall score
            passed_tests = sum(1 for test in self.validation_results.values() 
                             if isinstance(test, dict) and test.get("status") == "PASS")
            total_tests = len([k for k in self.validation_results.keys() if k != "overall_score"])
            self.validation_results["overall_score"] = (passed_tests / total_tests) * 100
    ...
    def generate_report(self) -> str:
        ...
        if results["overall_score"] < 90:
            report.append("❌ CRITICAL: PM instructions need improvement before deployment")
            
            failed_tests = [name for name, test in results.items() 
