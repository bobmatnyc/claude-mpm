    ...
    def test_cli_memory_add(self):
        ...
        try:
            ...
            try:
                ...
                test_commands = [
                    ["python", "-m", "claude_mpm.cli", "memory", "add", "engineer", "pattern", "Test pattern from CLI"],
                    ["python", "-m", "claude_mpm.cli", "memory", "add", "qa", "error", "Test error from CLI"], 
                    ["python", "-m", "claude_mpm.cli", "memory", "add", "research", "context", "Test context from CLI"],
                ]
                ...
                created_files = list(memory_dir.glob("*_agent.md")) if memory_dir.exists() else []
                
                self.log_result("CLI commands create memory files", 
                               len(created_files) >= 3,
    ...
    def test_api_memory_add(self):
        ...
        try:
            ...
            test_learnings = [
                ...
                ("research", "strategy", "API test strategy - analyze before implementing"),
                ("data-engineer", "performance", "API test performance - use vectorization"),
            ]
            ...
            self.log_result("Python API add_learning works", 
            ...
            has_api_content = "API test pattern" in loaded_content if loaded_content else False
            
            self.log_result("API-added memories persist", has_api_content,
    ...
    def test_80kb_limit_enforcement(self):
        ...
        try:
            ...
            for i in range(100):  # Add many sections
                section_content = f"- Large memory item {i}: " + "x" * 800  # ~800 chars per item
            ...
            for i, content in enumerate(large_content_sections):
                success = memory_manager.update_agent_memory("engineer", "Test Section", content)
                if success:
                    exceed_count += 1
            ...
            final_size_kb = len(final_memory.encode('utf-8')) / 1024 if final_memory else 0
            
            self.log_result("80KB limit enforced", 
            ...
            self.log_result("Memory optimization triggered when needed", 
                           limit_enforced,
                           f"Memory stayed at {final_size_kb:.1f}KB after {exceed_count} additions")
            
        except Exception as e:
            self.log_result("80KB limit enforcement test", False, f"Exception: {str(e)}")
...
"""
            ...
            try:
                ...
                instructions_with_project = loader_with_project.get_framework_instructions()
                
                has_project_override = "Project Memory Override" in instructions_with_project
                self.log_result("Project-specific memory loads when present", 
            ...
            try:
                ...
                has_system_memory = len(instructions_no_project) > 1000  # System memory should be substantial
                no_project_override = "Project Memory Override" not in instructions_no_project
                
                self.log_result("Falls back to system memory when no project memory", 
    ...
    def test_memory_persistence_across_sessions(self):
        ...
        try:
            ...
            self.log_result("Memory persists across sessions", 
                           persistence_success,
            ...
            retrieval_success = (engineer_memory is not None and 
                               qa_memory is not None and
                               "Session 1" in engineer_memory and
                               "Session 1" in qa_memory)
            
            self.log_result("Memory retrieval API works across sessions", 
                           retrieval_success,
    ...
    def test_performance_under_load(self):
        ...
        try:
            ...
            for i in range(100):
                success = memory_manager.add_learning("engineer", "pattern", f"Pattern {i}")
                if success:
                    small_additions += 1
                    
            small_additions_time = time.time() - start_time
            
            self.log_result("Handles many small additions efficiently", 
            ...
            self.log_result("Handles large memory operations efficiently", 
            ...
            self.log_result("Memory status retrieval is fast", 
            ...
            self.log_result("Cross-reference analysis completes efficiently", 
                           cross_ref_time < 5.0,
    ...
    def test_error_handling_and_edge_cases(self):
        ...
        try:
            ...
            invalid_success = memory_manager.add_learning("", "pattern", "Invalid agent")
            self.log_result("Handles invalid agent IDs gracefully", 
            ...
            long_content_success = memory_manager.add_learning("engineer", "pattern", very_long_content)
            ...
            self.log_result("Handles very long content appropriately", 
            ...
            self.log_result("Handles non-existent agent memory gracefully", 
            ...
            for i in range(10):
                success = memory_manager.add_learning("concurrent_test", "pattern", f"Concurrent {i}")
                if not success:
                    concurrent_success = False
                    
            self.log_result("Handles concurrent-like access", 
                           concurrent_success,
            ...
            is_valid, error_msg = memory_manager.validate_memory_size(invalid_memory_content)
            
            self.log_result("Memory validation works", 
    ...
    def test_memory_optimization_features(self):
        ...
        try:
            ...
            for i in range(5):
                memory_manager.add_learning("engineer", "pattern", "Duplicate pattern for testing")
                memory_manager.add_learning("engineer", "pattern", f"Unique pattern {i}")
                
            # Run optimization
            optimization_result = memory_manager.optimize_memory("engineer")
            
            optimization_success = optimization_result.get("success", False)
            self.log_result("Memory optimization executes", 
            ...
            self.log_result("Bulk memory optimization works", 
                           bulk_success,
