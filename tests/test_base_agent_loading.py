...
def test_base_agent_loader():
    ...
    try:
        ...
        if "/Users/masa/Projects/claude-mpm" in str(path):
            print(f"   ✓ Using LOCAL DEVELOPMENT version (correct!)")
        elif "pipx" in str(path):
            print(f"   ⚠ Using PIPX installation version (should be local dev)")
        elif "site-packages" in str(path):
            print(f"   ⚠ Using SITE-PACKAGES version (should be local dev)")
        else:
...
def test_async_agent_deployment():
    ...
    from claude_mpm.services.agents.deployment.async_agent_deployment import AsyncAgentDeploymentService
...
def main():
    ...
    expected_base_agent = Path.cwd() / "src" / "claude_mpm" / "agents" / "base_agent.json"
    if expected_base_agent.exists():
        ...
        print(f"  Make sure you're running from the claude-mpm project root!")
