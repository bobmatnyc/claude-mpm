...
from claude_mpm.cli.commands.cleanup import cleanup_memory, format_size, parse_size
...
class TestCleanupCommand(unittest.TestCase):
    ...
    def test_cleanup_no_claude_json(self, mock_print, mock_path):
        ...
        args = Namespace(
            days=30, max_size="500KB", archive=True, force=False, dry_run=False
        )
    ...
    @patch("claude_mpm.cli.commands.cleanup.Path")
    @patch("builtins.print")
    def test_cleanup_with_user_cancel_non_tty(
        self, mock_print, mock_path, mock_stdout, mock_stdin
    ):
    ...
    @patch("claude_mpm.cli.commands.cleanup.Path")
    @patch("builtins.print")
    def test_cleanup_with_force_flag(
        self, mock_print, mock_path, mock_stdout, mock_stdin
    ):
