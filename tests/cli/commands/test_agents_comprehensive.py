...
class TestAgentsCommand:
    ...
    def mock_deployment_service(self):
        ...
        service.fix_deployment.return_value = {
            "fixes_applied": ["Fixed frontmatter formatting", "Corrected version field"],
        }
...
class TestListingOperations(TestAgentsCommand):
    ...
    def test_list_deployed_agents_with_warnings(self, command, mock_args, mock_deployment_service):
...
class TestDeploymentOperations(TestAgentsCommand):
    ...
    def test_deploy_agents_force(self, command, mock_args, mock_deployment_service):
        ...
        mock_deployment_service.deploy_project_agents.assert_called_once_with(force=True)

    def test_deploy_agents_no_changes(self, command, mock_args, mock_deployment_service):
    ...
    def test_deploy_agents_with_errors(self, command, mock_args, mock_deployment_service):
        """Test deployment with errors."""
        mock_deployment_service.deploy_system_agents.side_effect = Exception("Deploy failed")
...
class TestCleanupOperations(TestAgentsCommand):
    ...
    def test_clean_agents_none_to_clean(self, command, mock_args, mock_deployment_service):
    ...
    def test_cleanup_orphaned_agents_dry_run(self, command, mock_args):
        ...
        with patch("claude_mpm.services.agents.deployment.multi_source_deployment_service.MultiSourceAgentDeploymentService") as MockService:
...
class TestViewingOperations(TestAgentsCommand):
    ...
    def test_view_agent_not_found(self, command, mock_args, mock_deployment_service):
        ...
        mock_deployment_service.get_agent_details.side_effect = Exception("Agent not found")
...
class TestDependencyOperations(TestAgentsCommand):
    ...
    def test_check_dependencies_all_satisfied(self, command, mock_args, mock_deployment_service):
    ...
    def test_install_dependencies_none_needed(self, command, mock_args, mock_deployment_service):
...
class TestErrorHandling(TestAgentsCommand):
    ...
    def test_deployment_service_import_error(self, mock_args):
        ...
        with patch("claude_mpm.services.AgentDeploymentService", side_effect=ImportError):
            cmd = AgentsCommand()
    ...
    def test_general_exception_handling(self, command, mock_args, mock_deployment_service):
        """Test general exception handling."""
        mock_deployment_service.list_available_agents.side_effect = Exception("Unexpected error")
...
class TestDefaultBehavior(TestAgentsCommand):
    ...
    def test_show_agent_versions_default(self, command, mock_args):
        ...
        with patch("claude_mpm.cli.commands.agents.get_agent_versions_display") as mock_get_versions:
...
class TestManageAgentsFunction:
    ...
    def test_manage_agents_failure(self):
        ...
        with patch.object(AgentsCommand, "execute") as mock_execute:
            mock_execute.return_value = CommandResult.error_result("Failed", exit_code=1)
...
class TestLazyLoading:
    ...
    def test_deployment_service_lazy_loading(self):
        ...
        with patch("claude_mpm.services.AgentDeploymentService") as MockService:
            with patch("claude_mpm.services.agents.deployment.deployment_wrapper.DeploymentServiceWrapper") as MockWrapper:
                # Access the property
...
class TestEdgeCases:
    ...
    def test_view_agent_special_characters(self, command_with_service):
        ...
        assert result.success
        command_with_service._deployment_service.get_agent_details.assert_called_once_with("test-agent_v2.0")
                ...
                with patch("builtins.print"):
                    with patch("pathlib.Path.cwd", return_value=Path("/project")):
                        with patch("pathlib.Path.exists", return_value=False):
                            ...
                            MockLoader.return_value.load_main_config.return_value = mock_config
