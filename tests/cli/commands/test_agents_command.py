...
class TestAgentsCommand:
    ...
    def test_run_default_show_versions(mock_get_versions):
    ...
    @patch('claude_mpm.cli.commands.agents.get_agent_versions_display')
    def test_run_default_json_format(mock_get_versions):
    ...
    @patch('claude_mpm.cli.commands.agents.get_agent_versions_display')
    def test_run_default_no_agents(mock_get_versions):
    ...
    def test_run_list_command():
        ...
        args = Namespace(
            agents_command=AgentCommands.LIST.value,
            format='text'
        )
    ...
    def test_run_deploy_command():
        ...
        args = Namespace(
            agents_command=AgentCommands.DEPLOY.value,
            format='text'
        )
    ...
    def test_run_force_deploy_command():
        """Test force deploy agents command."""
        args = Namespace(
            agents_command=AgentCommands.FORCE_DEPLOY.value,
            format='text'
        )
        
        with patch.object(self.command, '_deploy_agents') as mock_deploy:
            mock_deploy.return_value = CommandResult.success_result("Agents force deployed")
    ...
    def test_run_clean_command():
        """Test clean agents command."""
        args = Namespace(
            agents_command=AgentCommands.CLEAN.value,
            format='text'
        )
    ...
    def test_run_view_command():
        ...
        args = Namespace(
            agents_command=AgentCommands.VIEW.value,
            format='text'
        )
    ...
    def test_run_fix_command():
        """Test fix agents command."""
        args = Namespace(
            agents_command=AgentCommands.FIX.value,
            format='text'
        )
    ...
    def test_run_deps_check_command():
        """Test dependencies check command."""
        args = Namespace(
            agents_command="deps-check",
            format='text'
        )
        
        with patch.object(self.command, '_check_agent_dependencies') as mock_check:
            mock_check.return_value = CommandResult.success_result("Dependencies checked")
    ...
    def test_run_deps_install_command():
        """Test dependencies install command."""
        args = Namespace(
            agents_command="deps-install",
            format='text'
        )
        
        with patch.object(self.command, '_install_agent_dependencies') as mock_install:
            mock_install.return_value = CommandResult.success_result("Dependencies installed")
    ...
    def test_deployment_service_lazy_loading():
        ...
        with patch('claude_mpm.services.AgentDeploymentService') as mock_service_class:
            with patch('claude_mpm.services.agents.deployment.deployment_wrapper.DeploymentServiceWrapper') as mock_wrapper_class:
                mock_service = Mock()
                mock_wrapper = Mock()
    ...
    def test_deployment_service_import_error():
        """Test handling of deployment service import error."""
        # Simulate import error by patching the deployment_service property
        with patch.object(AgentsCommand, 'deployment_service', property(lambda self: None)):
            args = Namespace(
                agents_command=AgentCommands.DEPLOY.value,
                format='text'
            )
    ...
    def test_list_agents_implementation(mock_deployment_service):
    ...
    def test_deploy_agents_with_error():
    ...
    def test_run_with_exception():
        ...
        with patch.object(self.command, '_list_agents', side_effect=Exception("Test error")):
            result = self.command.run(args)
