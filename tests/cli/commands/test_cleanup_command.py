...
class TestCleanupCommand:
    ...
    def test_run_memory_cleanup(mock_shutil, mock_path_class):
        ...
        with patch.object(self.command, '_cleanup_memory') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                "Memory cleaned",
                data={'files_removed': 2, 'space_freed': '10MB'}
            )
    ...
    def test_run_cache_cleanup():
        ...
        with patch.object(self.command, '_cleanup_cache') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                "Cache cleaned",
                data={'cache_cleared': True, 'items_removed': 50}
            )
    ...
    def test_run_temp_cleanup():
        ...
        with patch.object(self.command, '_cleanup_temp') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                "Temp files cleaned",
                data={'files_removed': 10, 'space_freed': '5MB'}
            )
    ...
    def test_run_logs_cleanup():
        ...
        with patch.object(self.command, '_cleanup_logs') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                "Logs cleaned",
                data={'files_removed': 20, 'space_freed': '100MB'}
            )
    ...
    def test_cleanup_with_confirmation():
        ...
        with patch('builtins.input', return_value='y'):
            with patch.object(self.command, '_cleanup_memory') as mock_cleanup:
                mock_cleanup.return_value = CommandResult.success_result("Memory cleaned")
    ...
    def test_cleanup_permission_error(mock_path_class):
        ...
        with patch.object(self.command, '_cleanup_memory') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.error_result(
                "Permission denied",
                data={'error': 'Access denied to some files'}
            )
    ...
    def test_cleanup_with_pattern():
        ...
        with patch.object(self.command, '_cleanup_temp') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                "Temp files cleaned",
                data={'pattern': '*.tmp', 'files_removed': 5}
            )
    ...
    def test_cleanup_with_size_threshold():
        ...
        with patch.object(self.command, '_cleanup_logs') as mock_cleanup:
            mock_cleanup.return_value = CommandResult.success_result(
                ...
                data={'min_size': '10MB', 'files_removed': 3}
            )
    ...
    def test_run_with_exception():
        ...
        with patch.object(self.command, '_cleanup_memory', side_effect=Exception("Cleanup failed")):
            result = self.command.run(args)
