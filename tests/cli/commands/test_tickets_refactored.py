...
class TestTicketsCommand:
    ...
    def test_view_ticket_success(self, mock_view):
        ...
        args = Namespace(
            tickets_command=TicketCommands.VIEW.value,
            ticket_id="TSK-001",
            verbose=True
        )
    ...
    def test_delete_ticket_success(self, mock_delete):
        ...
        args = Namespace(
            tickets_command=TicketCommands.DELETE.value,
            ticket_id="TSK-001",
            force=True
        )
...
class TestTicketLegacyFunctions:
    ...
    @patch('claude_mpm.services.ticket_manager.TicketManager')
    def test_list_tickets_legacy_with_aitrackdown(self, mock_manager_class, mock_subprocess):
    ...
    @patch('subprocess.run')
    @patch('claude_mpm.services.ticket_manager.TicketManager')
    def test_update_ticket_legacy_with_aitrackdown(self, mock_manager_class, mock_subprocess):
    ...
    def test_update_workflow_legacy_success(self, mock_subprocess):
        ...
        assert result == 0
        mock_subprocess.assert_called_once_with(
            ["aitrackdown", "transition", "TSK-001", "ready", "--comment", "Ready for review"],
            check=True,
            capture_output=True,
            text=True
        )
