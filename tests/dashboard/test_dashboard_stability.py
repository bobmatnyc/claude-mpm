...
class ConnectionMonitor:
    ...
    def start_monitoring(self):
        ...
        logger.info(f"[{self.tab_id}] Started connection monitoring at {self.start_time}")
    ...
    def _handle_request(self, request: Request):
        ...
        if request.url.startswith("ws://") or request.url.startswith("wss://") or "socket.io" in request.url:
    ...
    def _handle_response(self, response: Response):
        ...
        if "socket.io" in response.url or response.status == 101:  # WebSocket upgrade
            logger.debug(f"[{self.tab_id}] WebSocket response: {response.status} - {response.url}")
    
    async def take_screenshot(self, name: str):
        """Take a screenshot of the current page"""
        screenshot_path = SCREENSHOT_DIR / f"{self.tab_id}_{name}_{int(time.time())}.png"
...
async def test_single_tab_stability(browser_context: BrowserContext, duration_minutes: int) -> ConnectionMonitor:
    ...
    try:
        ...
        while datetime.now() < end_time:
            ...
            logger.info(f"[single_tab] Monitoring... {remaining.total_seconds():.0f} seconds remaining")
...
async def test_multi_tab_stability(browser_context: BrowserContext, tab_count: int, duration_minutes: int) -> List[ConnectionMonitor]:
    """Test connection stability across multiple tabs"""
    logger.info(f"Starting multi-tab stability test with {tab_count} tabs for {duration_minutes} minutes")
    ...
    try:
        ...
        while datetime.now() < end_time:
            remaining = end_time - datetime.now()
            logger.info(f"[multi_tab] Monitoring {tab_count} tabs... {remaining.total_seconds():.0f} seconds remaining")
...
async def generate_report(single_monitor: ConnectionMonitor, multi_monitors: List[ConnectionMonitor]):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("-" * 30 + "\n")
        f.write(f"Duration: {single_stats['duration_seconds']:.1f} seconds ({single_stats['duration_seconds']/60:.1f} minutes)\n")
        ...
        total_test_duration = single_stats['duration_seconds'] + (multi_monitors[0].get_stats()['duration_seconds'] if multi_monitors else 0)
    ...
    print(f"  Duration: {single_stats['duration_seconds']:.1f} seconds ({single_stats['duration_seconds']/60:.1f} minutes)")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            single_monitor = await test_single_tab_stability(context, TEST_DURATION_MINUTES)
            
            # Short break between tests
            await asyncio.sleep(5)
            
            # Run multi-tab test
            logger.info(f"Phase 2: Multi-tab test with {MULTI_TAB_COUNT} tabs for {MULTI_TAB_TEST_DURATION_MINUTES} minutes")
            multi_monitors = await test_multi_tab_stability(context, MULTI_TAB_COUNT, MULTI_TAB_TEST_DURATION_MINUTES)
