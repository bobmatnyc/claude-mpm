...
class DashboardStabilityTester:
    ...
    async def setup_client(self):
        ...
        async def connect():
            ...
            await self.log(f"Socket.IO client configured with connection settings")
        ...
        async def pong(data):
            self.pong_count += 1
            await self.log(f"🏓 Pong sent to server (latency: {data}ms, count: {self.pong_count})")
        ...
        try:
            ...
            while time.time() < end_time and self.connected:
                ...
                if self.connected:
                    ...
                    await self.log(f"📊 Still connected - uptime: {elapsed}s, pings: {self.ping_count}")
                
            # Record results
            final_uptime = int(time.time() - self.connect_time) if self.connect_time else 0
            ...
            await self.log(f"✅ {duration_minutes}-minute test complete - uptime: {final_uptime}s")
