...
def test_server_startup():
    ...
    try:
        ...
        if server.eventbus_integration:
            ...
            # Check relay
            if hasattr(server.eventbus_integration, 'relay') and server.eventbus_integration.relay:
                relay_stats = server.eventbus_integration.relay.get_stats()
                logger.info(f"✅ EventBus relay stats: {json.dumps(relay_stats, indent=2)}")
        ...
        if server.connection_manager:
            ...
            # Check configuration
            logger.info(f"  - Max buffer size: {server.connection_manager.max_buffer_size}")
            logger.info(f"  - Event TTL: {server.connection_manager.event_ttl}s")
            logger.info(f"  - Health check interval: {server.connection_manager.health_check_interval}s")
            logger.info(f"  - Stale timeout: {server.connection_manager.stale_timeout}s")
        else:
        ...
        logger.info(f"  - Ping interval: {CONNECTION_CONFIG['ping_interval']}s (was 45s)")
        logger.info(f"  - Ping timeout: {CONNECTION_CONFIG['ping_timeout']}s")
        logger.info(f"  - Stale timeout: {CONNECTION_CONFIG['stale_timeout']}s (was 180s)")
        logger.info(f"  - Connection timeout: {CONNECTION_CONFIG.get('connection_timeout', 'not set')}s")
