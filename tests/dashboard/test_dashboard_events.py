...
def test_eventbus_relay():
    ...
    try:
        ...
        if not config.enabled or not config.relay_enabled:
            print("⚠️  EventBus or relay is disabled. Enable with environment variables:")
        ...
        print(f"✓ EventBus instance obtained")
        ...
        # Publish test events
        test_events = [
            ("hook.user_prompt", {"message": "Test user prompt", "timestamp": time.time()}),
