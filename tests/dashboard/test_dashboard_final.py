...
async def main():
    ...
    print("1. Connect to the SocketIO server")
    print("2. Send various test events")
    print("3. Open the dashboard in your browser")
    print("4. You should see the events appearing in real-time\n")
    ...
    try:
        ...
        print(
            f"{BLUE}Watch the dashboard - events should appear in real-time!{RESET}\n"
        )
        ...
        for i, event in enumerate(test_events, 1):
            print(
                f"  [{i}/{len(test_events)}] Sending: {event['event']} - {event.get('message', event.get('agent', ''))}"
            )
        ...
        print("  • Events should have different colors based on type")
        print("  • Timestamps should be visible")
        print("  • Agent names should be highlighted")

        print(f"\n{YELLOW}Keeping connection open for 10 seconds...{RESET}")
        print("Feel free to interact with the dashboard during this time.")
