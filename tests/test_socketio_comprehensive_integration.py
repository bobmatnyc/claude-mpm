...
class IntegrationTestSuite:
    ...
    def create_test_server(self, port=18000, server_id=None):
        ...
        server = StandaloneSocketIOServer(
            host="localhost", 
            port=port,
            server_id=server_id or f"test-server-{port}"
        )
        ...
        try:
            ...
            with open(server.pidfile_path, 'r') as f:
                ...
                try:
                    ...
                    assert pidfile_data["server_id"] == server.server_id, "Server ID mismatch"
            ...
            assert not stale_server.pidfile_path.exists(), "Stale PID file not cleaned up"
        ...
        try:
            ...
            with patch.object(server2, '_validate_process_identity') as mock_validate:
                ...
                try:
                    ...
                    assert "Process 999992 does not exist" in e.validation_errors, "Missing validation error"
    ...
    def test_health_monitoring_integration(self):
        ...
        try:
            ...
            if hasattr(server, 'health_monitor') and server.health_monitor:
                assert server.health_monitor is not None, "Health monitor not initialized"
            ...
            if hasattr(server, 'recovery_manager') and server.recovery_manager:
                assert server.recovery_manager is not None, "Recovery manager not initialized"
                assert server.recovery_manager.enabled is not None, "Recovery manager config missing"
    ...
    def test_file_locking_mechanism(self):
        ...
        try:
            ...
            try:
                ...
                print("   ⚠ Second server was able to create PID file (locking may not be working)")
    ...
    def test_backward_compatibility(self):
        ...
        try:
            ...
            is_running = server.is_already_running()
            print(f"   Legacy PID file handled: {not is_running}")  # Should cleanup and return False
