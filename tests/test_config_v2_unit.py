...
from claude_mpm.manager.screens.config_screen_v2 import YAMLFormWidget, EnhancedConfigEditor
...
def test_field_type_handling():
    ...
    field_types = {field_key: widget_type for field_key, (widget_type, widget) in form.widgets_map.items()}
    ...
    # Basic checks
    assert any('int' in ftype for ftype in field_types.values()), "Should have integer fields"
    assert any('float' in ftype for ftype in field_types.values()), "Should have float fields"
    assert any('bool' in ftype for ftype in field_types.values()), "Should have boolean fields"
    assert any('text' in ftype for ftype in field_types.values()), "Should have text fields"
