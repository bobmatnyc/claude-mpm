...
def test_no_agents_performance():
    ...
    try:
        ...
        if backup_dir and backup_dir.exists():
            ...
            print(f"Restored agents from backup")
...
"""
        ...
        speedup = parse_time / cached_parse_time if cached_parse_time > 0 else float('inf')
        print(f"   Cache speedup: {speedup:.1f}x")
        
        # Verify performance
        if parse_time < 2.0:
            print(f"   ✓ Many agents performance good: {parse_time:.3f}s for {agent_count} agents")
        else:
            print(f"   ⚠ Many agents performance slower: {parse_time:.3f}s for {agent_count} agents")
