...
class TestMCPLockManagement:
    ...
    async def test_start_gateway_with_stale_lock(self, manager):
        ...
        with patch.object(manager, '_load_default_tools', return_value=None):
            with patch('claude_mpm.services.mcp_gateway.manager.MCPConfiguration') as mock_config:
                with patch('claude_mpm.services.mcp_gateway.manager.ToolRegistry') as mock_registry:
                    with patch('claude_mpm.services.mcp_gateway.manager.MCPGateway') as mock_gateway:
                        # Setup mocks
