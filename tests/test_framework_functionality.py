...
def test_agent_capabilities_loading():
    ...
    if parsed_agents:
        ...
        for field in required_fields:
            assert field in sample_agent, f"Agent metadata missing required field: {field}"
...
def test_framework_content_loading():
    ...
    essential_fields = [
        "loaded", "framework_instructions", "workflow_instructions", 
        "memory_instructions", "agents"
    ]
...
def test_full_instruction_generation():
    ...
    if deployed_agents:
        ...
        print(f"✓ Instructions mention {agents_mentioned}/{len(deployed_agents)} deployed agents")
                ...
                if not agent_file.name.startswith("."):
                    try:
                        ...
                        if content.startswith("---"):
                            metadata = loader._parse_agent_metadata(agent_file)
                            if metadata:
                                print(f"✓ Successfully parsed YAML metadata from {agent_file.name}")
