...
class TestAgentLoaderCore:
    ...
    def test_agent_loader_initialization(self, temp_agent_dir, monkeypatch):
        ...
        monkeypatch.setattr("claude_mpm.agents.agent_loader.AGENT_TEMPLATES_DIR", temp_agent_dir)
    ...
    def test_get_agent_metadata(self, temp_agent_dir, monkeypatch):
        ...
        assert metadata["version"] == "1.0.0"  # The get_agent_metadata returns "version", not "agent_version"
...
class TestAgentLoaderCaching:
    ...
    def test_cache_key_generation(self, temp_agent_dir, mock_cache, monkeypatch):
        ...
        assert call_args[0][1].startswith("# Test Agent Instructions")  # Second is the content
    ...
    def test_cache_invalidation(self, temp_agent_dir, mock_cache, monkeypatch):
        ...
        with patch.object(SharedPromptCache, 'get_instance', return_value=mock_cache):
            ...
            clear_agent_cache()
            assert mock_cache.invalidate.call_count >= 2  # At least test_agent and research_agent
...
class TestAgentLoaderErrorHandling:
    ...
    def test_invalid_json_handling(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("claude_mpm.agents.agent_loader.AGENT_TEMPLATES_DIR", templates_dir)
...
class TestDynamicModelSelection:
    ...
    def test_complexity_analysis_integration(self, temp_agent_dir, monkeypatch):
        ...
        assert config["selection_method"] in ["dynamic_complexity_based", "agent_default"]
    ...
    def test_model_threshold_mapping(self):
        ...
        for score, expected_model in test_cases:
            ...
            complexity_result = {
                ...
                "complexity_level": ComplexityLevel.HIGH if score > 70 else (
                    ComplexityLevel.MEDIUM if score > 30 else ComplexityLevel.LOW
                ),
            ...
            if expected_model == ModelType.HAIKU:
                assert MODEL_THRESHOLDS[ModelType.HAIKU]["min_complexity"] <= score <= MODEL_THRESHOLDS[ModelType.HAIKU]["max_complexity"]
            elif expected_model == ModelType.SONNET:
                assert MODEL_THRESHOLDS[ModelType.SONNET]["min_complexity"] <= score <= MODEL_THRESHOLDS[ModelType.SONNET]["max_complexity"]
            else:
...
class TestBackwardCompatibility:
    ...
    def test_legacy_agent_functions(self, temp_agent_dir, monkeypatch):
        ...
        with patch("claude_mpm.agents.agent_loader.prepend_base_instructions", side_effect=lambda x, **k: f"BASE\n{x}"):
...
class TestPerformance:
    ...
    def test_multiple_agent_loading_performance(self, tmp_path, monkeypatch):
        ...
        for i in range(50):
            agent_data = {
                ...
                "instructions": f"Instructions for agent {i}" * 100  # Make it reasonably sized
            }
    ...
    def test_cache_efficiency(self, temp_agent_dir, monkeypatch):
        ...
        assert access_times[0] > sum(access_times[1:]) / 9  # First is slower than average of others
...
class TestEdgeCases:
    ...
    def test_empty_templates_directory(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("claude_mpm.agents.agent_loader.AGENT_TEMPLATES_DIR", empty_dir)
