...
- Write comprehensive tests"""
    ...
    initial_lines = len([line for line in initial_memory.strip().split('\n') if line.strip()])
...
- Monitor application metrics"""
    ...
    updated_lines = len([line for line in updated_memory.strip().split('\n') if line.strip()])
    ...
    def check_memory_update_needed(agent_data, new_memory_content, new_line_count):
        ...
        if current_memory_lines != new_line_count:
            return True, f"Line count changed: {current_memory_lines} -> {new_line_count}"
    ...
    needs_update, reason = check_memory_update_needed(agent_data, updated_memory, updated_lines)
    ...
    if needs_update:
        ...
        def insert_memory_section(instructions, memory_content):
            ...
            for i, line in enumerate(lines):
                ...
                elif memory_section_index is not None and line.startswith('## ') and i > memory_section_index:
                    memory_end_index = i
                    break
        ...
        updated_instructions = insert_memory_section(agent_data['instructions'], updated_memory)
        ...
        agent_data['capabilities']['memory_size_kb'] = round(len(updated_memory.encode('utf-8')) / 1024, 2)
        ...
        updated_frontmatter_yaml = yaml.dump(updated_frontmatter, default_flow_style=False)
        ...
        print(f"   ✓ Contains updated content: {'Docker containerization' in file_content}")
