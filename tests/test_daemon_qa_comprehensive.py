...
# Test configuration
TEST_RESULTS = []
DAEMON_SCRIPT = Path(__file__).parent.parent / "src" / "claude_mpm" / "scripts" / "socketio_daemon_hardened.py"
...
def cleanup_daemon():
    ...
    try:
        result = subprocess.run(['pgrep', '-f', 'socketio_daemon'], capture_output=True, text=True)
...
def test_basic_operations():
    ...
    if pid > 0 and supervisor_pid > 0 and port > 0:
        log_test("Basic startup", True, f"Server PID: {pid}, Supervisor: {supervisor_pid}, Port: {port}", "Basic")
        
        # Test port accessibility
        if check_port_listening(port):
            log_test("Port accessibility", True, f"Port {port} accepting connections", "Basic")
        else:
            log_test("Port accessibility", False, f"Port {port} not responding", "Basic")
    else:
    ...
    if result.returncode == 0 and "RUNNING" in result.stdout:
        ...
        log_test("Status command", False, f"Status command failed: {result.stderr}", "Basic")
    
    # Test 3: Metrics generation
    metrics = get_metrics()
    if metrics and metrics.get('status'):
        log_test("Metrics generation", True, f"Status: {metrics.get('status')}", "Basic")
    else:
...
def test_crash_recovery():
    ...
    if initial_pid == 0:
        log_test("Recovery setup", False, "Could not start daemon for recovery test", "Recovery")
        return
    
    # Test 1: Server crash recovery
    try:
        os.kill(initial_pid, signal.SIGKILL)
        log_test("Crash simulation", True, f"Killed server process {initial_pid}", "Recovery")
    ...
    if new_pid > 0 and new_pid != initial_pid and new_supervisor_pid == supervisor_pid:
        ...
        if check_port_listening(port):
            log_test("Service continuity", True, f"Service restored on port {port}", "Recovery")
        else:
        ...
        if metrics.get('restarts', 0) > 0:
            log_test("Restart tracking", True, f"Restarts: {metrics['restarts']}", "Recovery")
        else:
            log_test("Restart tracking", False, "Restart not tracked", "Recovery")
    else:
        log_test("Automatic recovery", False, f"Recovery failed. New PID: {new_pid}", "Recovery")
    ...
    if final_pid > 0:
        log_test("Multiple crash recovery", True, f"Survived multiple crashes, PID: {final_pid}", "Recovery")
    else:
        log_test("Multiple crash recovery", False, "Failed to recover from multiple crashes", "Recovery")
...
def test_health_monitoring():
    ...
    try:
        ...
        if get_daemon_pid() == 0:
            log_test("Health monitoring setup", False, "Could not start daemon", "Health")
            return
        ...
        if metrics and metrics.get('health_checks_passed', 0) > 0:
            log_test("Health checks execution", True, f"Passed: {metrics['health_checks_passed']}", "Health")
        else:
            log_test("Health checks execution", False, "No health checks recorded", "Health")
        
        # Test 2: Health status tracking
        if metrics.get('status') in ['healthy', 'running']:
            log_test("Health status tracking", True, f"Status: {metrics['status']}", "Health")
        else:
            log_test("Health status tracking", False, f"Unexpected status: {metrics.get('status')}", "Health")
        
        # Test 3: Health timestamp updates
        if metrics.get('last_health_check'):
            log_test("Health timestamp updates", True, "Health check timestamp present", "Health")
        else:
            log_test("Health timestamp updates", False, "No health check timestamp", "Health")
...
def test_configuration_management():
    ...
    try:
        ...
        if 9000 <= port <= 9010:
            log_test("Custom port range", True, f"Using port {port} from custom range", "Config")
        else:
        ...
        if "Max Retries: 15" in result.stdout:
            ...
            log_test("Config in status output", False, "Custom config not reflected", "Config")
        ...
        if get_daemon_pid() > 0:
            log_test("Invalid config handling", True, "Gracefully handled invalid config", "Config")
        else:
            log_test("Invalid config handling", False, "Failed to handle invalid config", "Config")
...
def test_process_management():
    ...
    if pid_file.exists() and supervisor_pid_file.exists():
        ...
        log_test("PID file creation", False, "PID files not created properly", "Process")
    ...
    if supervisor_pid > 0:
        try:
            ...
            if not is_process_alive(supervisor_pid):
                log_test("SIGTERM handling", True, "Supervisor responded to SIGTERM", "Process")
            else:
                log_test("SIGTERM handling", False, "Supervisor did not respond to SIGTERM", "Process")
        except Exception as e:
            log_test("SIGTERM handling", False, f"Error sending SIGTERM: {e}", "Process")
    
    # Test 3: PID file cleanup after shutdown
    if not pid_file.exists() and not supervisor_pid_file.exists():
        log_test("PID file cleanup", True, "PID files cleaned up on shutdown", "Process")
    else:
    ...
    if first_pid > 0 and second_pid == first_pid:
        ...
        log_test("Concurrent protection", False, f"Multiple instances running: {first_pid}, {second_pid}", "Process")
...
def test_error_handling():
    ...
    try:
        ...
        if port > 8765:  # Should find alternative port
            log_test("Port conflict handling", True, f"Found alternative port {port}", "Error")
        else:
            log_test("Port conflict handling", False, "Did not handle port conflict", "Error")
    ...
    try:
        ...
        log_test("Permission error handling", True, "Permission test completed", "Error")
    finally:
        try:
...
def test_performance_load():
    ...
    port = get_daemon_port()
    if port == 0:
        log_test("Load test setup", False, "Could not start daemon for load test", "Performance")
        return
    ...
    start_time = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=LOAD_TEST_CONNECTIONS) as executor:
        futures = [executor.submit(test_connection) for _ in range(LOAD_TEST_CONNECTIONS)]
        results = [f.result() for f in concurrent.futures.as_completed(futures, timeout=LOAD_TEST_DURATION)]
    
    duration = time.time() - start_time
    success_count = sum(results)
    
    if success_count >= LOAD_TEST_CONNECTIONS * 0.8:  # 80% success rate
        log_test("Concurrent connections", True, f"{success_count}/{LOAD_TEST_CONNECTIONS} connections successful in {duration:.2f}s", "Performance")
    else:
        log_test("Concurrent connections", False, f"Only {success_count}/{LOAD_TEST_CONNECTIONS} connections successful", "Performance")
    ...
    if pid > 0 and supervisor_pid > 0:
        try:
            ...
            memory_growth = (final_server_memory - initial_server_memory) / 1024 / 1024  # MB
            
            if memory_growth < 10:  # Less than 10MB growth
                log_test("Memory stability", True, f"Memory growth: {memory_growth:.2f}MB", "Performance")
            else:
                log_test("Memory stability", False, f"Excessive memory growth: {memory_growth:.2f}MB", "Performance")
                
        except Exception as e:
            log_test("Memory stability", False, f"Could not measure memory: {e}", "Performance")
...
def test_integration():
    ...
    wrapper_script = Path(__file__).parent.parent / "scripts" / "socketio_daemon_wrapper.py"
    ...
    if wrapper_script.exists():
        ...
        if get_daemon_pid() > 0:
            ...
            if get_daemon_pid() == 0:
                log_test("Wrapper script stop", True, "Wrapper successfully stopped daemon", "Integration")
            else:
                log_test("Wrapper script stop", False, "Wrapper failed to stop daemon", "Integration")
        else:
            log_test("Wrapper script integration", False, "Wrapper failed to start daemon", "Integration")
    ...
    if final_metrics and final_metrics.get('restarts', 0) > initial_metrics.get('restarts', 0):
        log_test("Metrics persistence", True, f"Restart count incremented: {final_metrics.get('restarts')}", "Integration")
    else:
        log_test("Metrics persistence", False, "Metrics not properly updated across restart", "Integration")
...
def generate_report():
    ...
    for category, stats in categories.items():
        ...
        print(f"{status} {category}: {stats['passed']}/{total_cat} passed ({percentage:.1f}%)")
    ...
    critical_passed = sum(categories.get(cat, {}).get('passed', 0) for cat in critical_categories)
    critical_total = sum(categories.get(cat, {}).get('passed', 0) + categories.get(cat, {}).get('failed', 0) for cat in critical_categories)
    ...
    if critical_total > 0:
        ...
        if critical_percentage >= 95:
            ...
            recommendation = "The daemon meets production requirements with excellent reliability."
...
def main():
    ...
    for test_category in test_categories:
        try:
            ...
            log_test(f"{test_category.__name__} execution", False, f"Test category failed: {e}", "System")
            import traceback
            traceback.print_exc()
        finally:
            cleanup_daemon()
