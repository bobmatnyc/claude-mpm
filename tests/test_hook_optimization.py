...
from claude_mpm.services.optimized_hook_service import OptimizedHookService, HookConfig
from claude_mpm.hooks.base_hook import BaseHook, PreDelegationHook, PostDelegationHook, HookContext, HookResult, HookType
...
def test_hook_caching():
    ...
    loaded_instances = sum(1 for config in service._hook_configs.values() if config.loaded_instance is not None)
...
def test_hook_registration_and_execution():
    ...
    for i in range(10):
        ...
        assert "parallel_hook_executed" in result.data, "Parallel hook should have executed"
...
def main():
    ...
    print(f"  ✓ Working correctly: {'✅' if singleton_result['singleton_working'] else '❌'}")
    ...
    print(f"\n📊 Metrics Collection:")
    print(f"  ✓ Metrics collected: {'✅' if metrics_result['metrics_collected'] else '❌'}")
    print(f"  ✓ Performance tracking: {'✅' if metrics_result['performance_difference'] else '❌'}")
    ...
    print(f"  ✓ Error handling: {'✅' if management_result['nonexistent_handling'] else '❌'}")
    
    print(f"\n🏃 Performance Benchmark:")
    for count, results in benchmark_results.items():
        print(f"  ✓ {count} executions: {results['throughput']:.1f} exec/sec, {results['avg_time_ms']:.3f}ms avg")
