        ...
        try:
            ...
            if hasattr(module, 'language'):
                ...
                try:
                    ...
                    print(f"   ⚠️  tree-sitter core not available, but module import works")
                    results.append((package_name, True, "Import works, core missing"))
                except Exception as e:
                    print(f"   ❌ Failed to create Language object: {e}")
                    results.append((package_name, False, f"Language creation failed: {e}"))
            else:
...
def main():
    ...
    # Overall assessment
    print(f"\n🎯 Overall Results:")
    print(f"   📦 Package imports: {successful_imports}/{len(import_results)} successful")
    print(f"   🔗 Language objects: {successful_languages}/{len(language_results)} successful")
    print(f"   🐍 Python 3.13 compat: {'✅' if python313_compat else '❌'}")
    
    overall_success = (
        successful_imports > 0 and  # At least some packages work
        python313_compat  # Python 3.13 compatibility is essential
    )
