    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if len(node.value) > 10 and any(keyword in node.value.lower() for keyword in ['key', 'secret', 'password', 'token']):
        ...
        for func in functions[:5]:  # Show first 5
            print(f"      - {func['name']} (line {func['line']}, {'async' if func['is_async'] else 'sync'})")
        if len(functions) > 5:
            print(f"      ... and {len(functions) - 5} more functions")
        
        print(f"   📦 Imports found: {len(imports)}")
        print(f"   🔐 Potential secrets found: {len([s for s in hardcoded_strings if s['potential_secret']])}")
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if complexity > 10:
                    complex_functions.append({'name': node.name, 'complexity': complexity, 'line': node.lineno})
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if hasattr(node, 'end_lineno'):
                    ...
                    if length > 50:
                        long_functions.append({'name': node.name, 'length': length, 'line': node.lineno})
        
        print(f"   📏 Long functions (>50 lines): {len(long_functions)}")
        for func in long_functions:
            print(f"      - {func['name']} ({func['length']} lines, starts line {func['line']})")
...
"""
    ...
    try:
        ...
        if result.returncode == 0 and "PASS: Python AST was correctly prioritized" in result.stdout:
...
def main():
    ...
    if passed == total:
        ...
        print("   The code_analyzer agent correctly uses Python's native AST for .py files")
    else:
