...
class TestMemoryGuardianStress:
    ...
    async def test_rapid_memory_growth_stress(self, stress_workspace, stress_config):
        ...
        try:
            ...
            while time.time() - start_time < max_duration:
                ...
                if current_memory:
                    stress_results["max_memory_seen"] = max(
                        stress_results["max_memory_seen"], current_memory
                    )
            ...
            for i, (timestamp, from_state, to_state) in enumerate(
                stress_results["state_transitions"]
            ):
                print(f"    {i+1}. {timestamp:.2f}s: {from_state} -> {to_state}")

            # Stress test assertions
            assert (
                stress_results["max_memory_seen"] > stress_config.thresholds.warning
            ), "Should exceed warning threshold"
            assert (
                len(stress_results["state_transitions"]) >= 1
            ), "Should have state transitions"
            assert (
                stress_results["monitoring_samples"] > 10
            ), "Should have many monitoring samples"

            # Should trigger at least one restart for rapid growth
            if stress_results["max_memory_seen"] > stress_config.thresholds.emergency:
                assert (
                    stress_results["restarts_triggered"] >= 1
                ), "Should trigger restart for emergency memory"
    ...
    async def test_memory_thrashing_stress(self, stress_workspace, stress_config):
        ...
        try:
            ...
            if len(memory_readings) >= 2:
                ...
                memory_variance = sum(
                    (x - sum(memory_values) / len(memory_values)) ** 2
                    for x in memory_values
                ) / len(memory_values)
                ...
                assert (
                    memory_variance > 100
                ), f"Memory should vary during thrashing: {memory_variance:.1f}"
                assert (
                    len(memory_readings) > 50
                ), "Should have many readings during thrashing"
        ...
        try:
            with patch.object(guardian, "start_process", side_effect=mock_start):
                with patch.object(
                    ...
                    assert (
                        successful_restarts < num_concurrent_restarts
                    ), "Should limit concurrent restarts"
                    assert (
                        successful_restarts <= stress_config.restart_policy.max_attempts
                    ), "Should respect max attempts"
                    assert (
                        exceptions == 0
                    ), "Should not have exceptions during restart storm"
    ...
    async def test_large_state_file_stress(self, stress_workspace, stress_config):
        ...
        huge_state = {
            "conversations": [],
            "metadata": {"stress_test": True, "created": time.time()},
        }
        ...
        while current_size < conversation_size_target:
            ...
            for msg_id in range(200):  # 200 messages per conversation
                message = {
                    ...
                    "content": f"Stress test message {msg_id} with large content. "
                    * 500,  # ~20KB per message
                    "timestamp": time.time(),
                    "metadata": {
        ...
        try:
            ...
            for cycle in range(5):  # 5 stress cycles
                ...
                assert len(loaded_state["conversations"]) == len(
                    huge_state["conversations"]
                )

                print(
                    f"  Cycle {cycle + 1}: Save {save_time:.2f}s, Load {load_time:.2f}s, Size {file_size_mb:.1f}MB"
                )
            ...
            assert (
                final_size_mb > 30
            ), f"Should create large file: {final_size_mb:.1f}MB"
            assert (
                avg_save_time < 30
            ), f"Save too slow under stress: {avg_save_time:.2f}s"
            assert (
                avg_load_time < 45
            ), f"Load too slow under stress: {avg_load_time:.2f}s"
            assert (
                max(save_times) < 60
            ), f"Max save time too slow: {max(save_times):.2f}s"
    ...
    async def test_extended_monitoring_endurance(self, stress_config):
        ...
        try:
                ...
                while pattern_index < len(memory_pattern):
                    try:
                        ...
                        endurance_stats["memory_peak"] = max(
                            endurance_stats["memory_peak"], current_memory
                        )
            ...
            print(
                f"  Monitoring duration: {endurance_stats['monitoring_duration']:.2f}s"
            )
            ...
            print(
                f"  Samples per second: {endurance_stats['total_samples'] / endurance_stats['monitoring_duration']:.1f}"
            )

            # Endurance assertions
            assert (
                endurance_stats["total_samples"] > 100
            ), "Should have many samples during endurance test"
            assert (
                endurance_stats["state_transitions"] > 5
            ), "Should have multiple state transitions"
            assert (
                endurance_stats["error_count"] == 0
            ), "Should have no errors during endurance test"
            assert (
                endurance_stats["monitoring_duration"] > 20
    ...
    async def test_resource_exhaustion_scenarios(self, stress_config):
        ...
        for scenario in exhaustion_scenarios:
            ...
            try:
                if scenario == "memory_pressure":
                    ...
                    with patch(
                        "claude_mpm.utils.platform_memory.check_memory_pressure",
                        ...
                        if health_monitor:
                            (
                                valid,
                                message,
                            ) = await health_monitor.validate_before_start()
                    ...
                    with patch("psutil.Process.cpu_percent", return_value=95.0):
                        ...
                        with patch.object(
                            guardian, "get_memory_usage", return_value=300
                        ):
                    ...
                    with patch(
                        "shutil.disk_usage", return_value=(1000000000, 100000, 100000)
                    ):  # Very low free space
                    ...
                    with patch(
                        "subprocess.Popen", side_effect=OSError("Cannot create process")
                    ):
                        try:
                            success = await guardian.start_process()
                            scenario_results[scenario] = {
        ...
        for scenario, result in scenario_results.items():
            assert result.get(
                "handled_gracefully", False
            ), f"Scenario {scenario} not handled gracefully"
    ...
    async def test_high_frequency_event_processing(self, stress_config):
        ...
        try:
            ...
            with patch.object(
                guardian, "get_memory_usage", side_effect=high_freq_memory_reading
            ):
                guardian.start_monitoring()
            ...
            ), f"Should handle high frequency: {events_per_second:.1f}/s"
            assert samples_recorded > 0, "Should record samples despite high frequency"
            assert (
                len(memory_readings) > 100
            ), f"Should collect many readings: {len(memory_readings)}"
    ...
    async def test_chaos_engineering_scenarios(self, stress_workspace, stress_config):
        ...
        try:
                ...
                while time.time() - start_time < chaos_duration:
                    ...
                    if random.random() < 0.05:  # 5% chance
                        ...
                        try:
                            ...
                            with patch.object(
                                guardian, "start_process", return_value=True
                            ):
                                with patch.object(
                                    guardian, "terminate_process", return_value=True
                                ):
                                    restart_success = await guardian.restart_process(
                                        "Chaos intervention"
                                    )
                                    if restart_success:
            ...
            chaos_results["system_stability"] = (
                guardian._initialized and not guardian._shutdown
            )
            ...
            assert chaos_results[
                "system_stability"
            ], "System should maintain stability under chaos"

            # Should handle at least some chaos events
            if chaos_results["unexpected_events"] > 0:
                print(
                    f"  System survived {chaos_results['unexpected_events']} chaos events"
                )
...
class TestSystemInstabilitySimulation:
    ...
    async def test_network_instability_simulation(self):
        ...
        try:
            ...
            with patch(
                "asyncio.sleep", side_effect=lambda x: asyncio.sleep(x * 0.1)
            ):  # Speed up
