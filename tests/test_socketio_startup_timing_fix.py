...
from claude_mpm.cli.commands.run import _check_socketio_server_running, _start_standalone_socketio_server
...
class TestSocketIOStartupTimingFix(unittest.TestCase):
    ...
    def test_improved_timing_constants(self):
        ...
        self.assertIn("max_attempts = 30", source, "max_attempts should be increased to 30")
        self.assertIn("initial_delay = 1.0", source, "initial_delay should be increased to 1.0s")  
        self.assertIn("max_delay = 3.0", source, "max_delay should be increased to 3.0s")
        
        # Verify initial daemon startup delay is present
        self.assertIn("time.sleep(0.5)", source, "Initial daemon startup delay should be present")
    ...
    def test_health_check_retry_logic(self):
        ...
        self.assertIn("for retry in range(max_retries)", source, "Should iterate through retries")
        self.assertIn("timeout=10", source, "HTTP timeout should be increased to 10s")
        self.assertIn("settimeout(2.0)", source, "TCP timeout should be increased to 2.0s")
    ...
    def test_better_error_messages(self):
        ...
        self.assertIn("Server may still be starting", source, "Should mention server may still be starting")
        self.assertIn("initialization can take 15+ seconds", source, "Should mention 15+ second initialization time")
        self.assertIn("daemon process might be running", source, "Should mention daemon might be running")
    ...
    def test_startup_timing_simulation(self, mock_subprocess, mock_health_check):
        ...
        self.assertEqual(mock_health_check.call_count, 9, "Should make 9 health check attempts")
    ...
    def test_health_check_retry_behavior(self, mock_urlopen, mock_socket):
        ...
        mock_urlopen.side_effect = [
            ...
            MagicMock(getcode=lambda: 200, read=lambda: b'{"status": "ok"}')  # Finally ready
        ]
