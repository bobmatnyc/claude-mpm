...
class DaemonTestManager:
    ...
    def get_daemon_info(self):
        ...
        # Get supervisor PID
        supervisor_pid_file = self.deployment_root / ".claude-mpm" / "socketio-supervisor.pid"
    ...
    def get_metrics(self) -> dict:
        """Get daemon metrics."""
        metrics_file = self.deployment_root / ".claude-mpm" / ".claude-mpm/socketio-metrics.json"
...
def investigate_recovery_issue():
    ...
    try:
        ...
        if not manager.start_daemon_background():
            log_test("Recovery investigation setup", False, "Could not start daemon", "Investigation")
            return
        
        initial_server_pid, supervisor_pid, port = manager.get_daemon_info()
        print(f"Initial state: Server PID {initial_server_pid}, Supervisor PID {supervisor_pid}, Port {port}")
        ...
        try:
            ...
            log_test("Process validation", True, f"Server: {server_proc.status()}, Supervisor: {supervisor_proc.status()}", "Investigation")
        except Exception as e:
            log_test("Process validation", False, f"Error checking processes: {e}", "Investigation")
        ...
        try:
            ...
            log_test("Kill simulation", False, f"Could not kill process: {e}", "Investigation")
            return
        ...
        for i in range(20):  # 20 seconds, check every second
            ...
            print(f"Check {i+1}: Server PID {new_server_pid}, Supervisor PID {new_supervisor_pid}, Port {new_port}")
            ...
            if new_server_pid > 0 and new_server_pid != initial_server_pid:
                ...
                if manager.check_port_listening(new_port, timeout=3):
                    log_test("Detailed recovery test", True, f"Recovery successful in {i+1} seconds", "Investigation")
                else:
                    log_test("Detailed recovery test", False, f"Process recovered but port not listening", "Investigation")
                return
        
        log_test("Detailed recovery test", False, "No recovery detected in 20 seconds", "Investigation")
        ...
        if final_supervisor_pid == supervisor_pid:
            ...
            log_test("Supervisor persistence", True, "Supervisor still running", "Investigation")
        else:
            log_test("Supervisor persistence", False, "Supervisor also died", "Investigation")
...
def investigate_config_issue():
    ...
    try:
        ...
        test_configs = [
            {'SOCKETIO_PORT_START': '9500', 'SOCKETIO_PORT_END': '9510', 'expected_range': (9500, 9510)},
            {'SOCKETIO_PORT_START': '9700', 'SOCKETIO_PORT_END': '9700', 'expected_range': (9700, 9700)},  # Single port
        ]
        ...
        for i, config in enumerate(test_configs):
            ...
            if manager.start_daemon_background():
                ...
                print(f"Daemon started on port {port}, expected range {expected_start}-{expected_end}")
                
                if expected_start <= port <= expected_end:
                    log_test(f"Config test {i+1}", True, f"Port {port} in range {expected_start}-{expected_end}", "Config")
                else:
                    log_test(f"Config test {i+1}", False, f"Port {port} outside range {expected_start}-{expected_end}", "Config")
                    
                    # Check what port manager is doing
                    print("Investigating port selection...")
                    result = subprocess.run([sys.executable, str(DAEMON_SCRIPT), "status"], 
                                          capture_output=True, text=True)
                    if "Port Range:" in result.stdout:
                        port_line = [line for line in result.stdout.split('\n') if 'Port Range:' in line][0]
                        print(f"Status reports: {port_line}")
            else:
                log_test(f"Config test {i+1} setup", False, "Could not start daemon", "Config")
...
def test_error_handling():
    ...
    try:
        ...
        if manager.start_daemon_background():
            log_test("Invalid config handling", True, "Daemon started despite invalid config", "Error")
        else:
            log_test("Invalid config handling", False, "Daemon failed to start with invalid config", "Error")
        ...
        try:
            ...
            if manager.start_daemon_background():
                ...
                if daemon_port != test_port:
                    log_test("Port conflict handling", True, f"Found alternative port {daemon_port}", "Error")
                else:
                    log_test("Port conflict handling", False, f"Used conflicted port {daemon_port}", "Error")
            else:
                log_test("Port conflict handling", False, "Could not start daemon with port conflict", "Error")
        ...
        try:
            ...
            log_test("Permission error resilience", True, "Daemon handles permission errors gracefully", "Error")
        except Exception:
            log_test("Permission error resilience", True, "Permission test completed", "Error")
...
def test_performance():
    ...
    try:
        if not manager.start_daemon_background():
            log_test("Performance test setup", False, "Could not start daemon", "Performance")
            return
        ...
        if success_count >= 16:  # 80% success rate
            log_test("Concurrent connections", True, f"{success_count}/20 connections successful in {duration:.2f}s", "Performance")
        else:
            log_test("Concurrent connections", False, f"Only {success_count}/20 connections successful", "Performance")
        ...
        if server_pid > 0 and supervisor_pid > 0:
            try:
                ...
                server_growth = (final_server_memory - initial_server_memory) / 1024 / 1024  # MB
                supervisor_growth = (final_supervisor_memory - initial_supervisor_memory) / 1024 / 1024  # MB
                
                if server_growth < 5 and supervisor_growth < 5:  # Less than 5MB growth each
                    log_test("Memory stability", True, f"Growth: Server {server_growth:.2f}MB, Supervisor {supervisor_growth:.2f}MB", "Performance")
                else:
                    log_test("Memory stability", False, f"Excessive growth: Server {server_growth:.2f}MB, Supervisor {supervisor_growth:.2f}MB", "Performance")
...
def test_integration():
    ...
    try:
        ...
        if manager.start_daemon_background():
            ...
            # Restart daemon
            result = subprocess.run([sys.executable, str(DAEMON_SCRIPT), "restart"], 
            ...
            if new_server_pid > 0 and new_server_pid != initial_server_pid:
                log_test("Restart command", True, f"Restart successful: {initial_server_pid} -> {new_server_pid}", "Integration")
            else:
                log_test("Restart command", False, f"Restart failed or same PID: {new_server_pid}", "Integration")
        else:
            log_test("Restart test setup", False, "Could not start daemon", "Integration")
        ...
        if metrics:
            if 'uptime_seconds' in metrics and 'status' in metrics:
                log_test("Metrics persistence", True, f"Metrics available with uptime: {metrics.get('uptime_seconds')}s", "Integration")
            else:
                log_test("Metrics persistence", False, "Metrics incomplete", "Integration")
        else:
