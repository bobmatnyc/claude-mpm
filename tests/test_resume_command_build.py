...
def test_run_command_with_resume():
    ...
    with patch('claude_mpm.core.claude_runner.ClaudeRunner') as MockRunner:
        ...
        assert "--resume" in claude_args, f"Expected --resume in filtered args, got: {claude_args}"
    ...
    with patch('claude_mpm.core.claude_runner.ClaudeRunner') as MockRunner:
        ...
        assert "--resume" in claude_args, f"Expected --resume in filtered args, got: {claude_args}"
        assert "--model" in claude_args, f"Expected --model in filtered args, got: {claude_args}"
        assert "opus" in claude_args, f"Expected 'opus' in filtered args, got: {claude_args}"
...
def test_actual_command_execution():
    ...
    with patch('claude_mpm.core.claude_runner.ClaudeRunner.setup_agents'):
        ...
        assert "--dangerously-skip-permissions" in cmd, "Command should include permissions flag"
        
        print(f"✓ Command correctly includes --resume flag")
