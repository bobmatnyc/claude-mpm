...
def test_basic_startup():
    ...
    if pid > 0 and supervisor_pid > 0:
        log_test("Daemon startup", True, f"Server PID: {pid}, Supervisor PID: {supervisor_pid}, Port: {port}")
        
        # Check if port is listening
        if check_port_listening(port):
            log_test("Port listening", True, f"Port {port} is accepting connections")
        else:
            log_test("Port listening", False, f"Port {port} is not accepting connections")
    else:
...
def test_crash_recovery():
    ...
    if new_pid > 0 and new_pid != initial_pid and new_supervisor_pid == supervisor_pid:
        log_test("Automatic recovery", True, f"Server restarted with new PID: {new_pid}")
        
        # Check if port is still listening
        if check_port_listening(port):
            log_test("Service continuity", True, f"Service recovered on same port {port}")
        else:
            log_test("Service continuity", False, "Service not listening after recovery")
        
        # Check metrics for restart count
        metrics = get_metrics()
        if metrics.get('restarts', 0) > 0:
            log_test("Restart tracking", True, f"Restarts tracked: {metrics['restarts']}")
        else:
...
def test_health_monitoring():
    ...
    if metrics:
        ...
        if health_passed > 0:
            log_test("Health checks running", True, f"Health checks passed: {health_passed}")
        else:
        ...
        if metrics.get('status') == 'healthy' or metrics.get('status') == 'running':
            ...
            log_test("Health status", False, f"Unexpected status: {metrics.get('status')}")
        
        if metrics.get('last_health_check'):
            log_test("Health check timestamp", True, f"Last check: {metrics['last_health_check']}")
        else:
...
def test_configuration():
    ...
    if 9000 <= port <= 9010:
        ...
        log_test("Custom port range", False, f"Port {port} not in custom range 9000-9010")
    ...
    # Reset environment
    for key in ['SOCKETIO_MAX_RETRIES', 'SOCKETIO_PORT_START', 'SOCKETIO_PORT_END', 'SOCKETIO_LOG_LEVEL']:
...
def test_concurrent_protection():
    ...
    if second_pid == first_pid:
        log_test("Concurrent protection", True, "Second instance prevented, same PID maintained")
    else:
        log_test("Concurrent protection", False, f"Second instance started with PID {second_pid}")
