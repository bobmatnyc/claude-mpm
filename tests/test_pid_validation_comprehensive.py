...
def test_comprehensive_validation_scenarios():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        results.append(("valid_non_server_process", not is_running))  # Should return False after port check
        print(f"   Result: {'PASS' if not is_running else 'FAIL'} - correctly identified non-server process")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   Result: {'PASS' if not is_running and not pidfile_exists else 'FAIL'} - stale PID file cleaned up")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   Result: {'PASS' if not is_running and not pidfile_exists else 'FAIL'} - corrupted PID file handled")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   Result: {'PASS' if not is_running and not pidfile_exists else 'FAIL'} - empty PID file handled")
