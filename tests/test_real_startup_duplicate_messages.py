...
""")
        ...
        for line in full_output.split('\n'):
            if any(keyword in line.lower() for keyword in ['configuration', 'config', 'loaded']):
                print(f"  {line[:120]}")
        ...
        if success_count == 0:
            print("⚠ No success messages found (may be normal if version command doesn't load config)")
            return True
