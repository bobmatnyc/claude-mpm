...
class TestClaudeHookHandlerShellScript(unittest.TestCase):
    ...
    def setUp(self):
        ...
        actual_script = project_root / "src" / "claude_mpm" / "scripts" / "claude-hook-handler.sh"
...
''')
    ...
    def test_script_with_stdin_input(self):
        ...
        mock_python = self.test_dir / "venv" / "bin" / "python"
        mock_python.parent.mkdir(parents=True)
        mock_python.write_text('''#!/usr/bin/env python3
import json
import sys
...
''')
        ...
        debug_log = Path("/tmp/claude-mpm-hook.log")
...
''')
        ...
        dot_venv_python.write_text('''#!/bin/bash
echo "Using .venv Python"
echo '{"action": "continue"}'
exit 0
''')
...
'''
        ...
        success_python.write_text('''#!/bin/bash
echo '{"action": "continue"}'
exit 0
''')
        ...
        fail_python.write_text('''#!/bin/bash
exit 1
''')
