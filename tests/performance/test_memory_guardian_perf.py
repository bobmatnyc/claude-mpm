...
class PerformanceProfiler:
    ...
    def end_measurement(self, name: str) -> Dict[str, Any]:
        ...
        deltas = {
            ...
            "connections_delta": end_metrics["connections"]
            - start_metrics["connections"],
        }
    ...
    def compare_to_baseline(self) -> Dict[str, Any]:
        ...
        return {
            "duration_since_baseline": current["timestamp"]
            - self.baseline_metrics["timestamp"],
            "cpu_delta": current["cpu_percent"] - self.baseline_metrics["cpu_percent"],
            "memory_rss_delta_mb": (
                current["memory_rss"] - self.baseline_metrics["memory_rss"]
            )
            / (1024 * 1024),
            "memory_vms_delta_mb": (
                current["memory_vms"] - self.baseline_metrics["memory_vms"]
            )
            / (1024 * 1024),
...
class TestMemoryGuardianPerformance:
    ...
    def performance_config(self):
        ...
        return MemoryGuardianConfig(
            ...
            restart_policy=RestartPolicy(
                max_attempts=10, initial_cooldown=0.1, graceful_timeout=1
            ),
        ...
        try:
            ...
            with patch.object(guardian, "get_memory_usage", return_value=500.0):
                ...
                for interval_name, interval in [
                    ...
                    actual_checks = (
                        guardian.memory_stats.samples
                        if guardian.memory_stats.samples > 0
                    ...
                    monitoring_results[interval_name] = {
                        ...
                        "memory_overhead_mb": results["memory_rss_delta"]
                        / (1024 * 1024),
                        "overhead_per_check_ms": (
                            results["duration"] / actual_checks * 1000
                        )
                        if actual_checks > 0
                        else 0,
                    }
            ...
            for interval_name, result in monitoring_results.items():
                ...
                print(
                    f"  Checks: {result['actual_checks']:.0f} (expected: {result['expected_checks']:.0f})"
                )
            ...
            assert (
                normal_result["cpu_overhead"] < 10.0
            ), f"CPU overhead too high: {normal_result['cpu_overhead']}%"
            assert (
                normal_result["memory_overhead_mb"] < 10.0
            ), f"Memory overhead too high: {normal_result['memory_overhead_mb']}MB"
            assert (
                normal_result["overhead_per_check_ms"] < 5.0
            ), f"Per-check overhead too high: {normal_result['overhead_per_check_ms']}ms"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                for size_name, data_generator in [
                    ...
                    serialize_results = profiler.end_measurement(
                        f"serialize_{size_name}"
                    )
                    ...
                    deserialize_results = profiler.end_measurement(
                        f"deserialize_{size_name}"
                    )
                    ...
                    serialization_results[size_name] = {
                        ...
                        "serialize_throughput_mb_s": file_size_mb
                        / serialize_results["duration"],
                        "deserialize_throughput_mb_s": file_size_mb
                        / deserialize_results["duration"],
                        "serialize_memory_delta_mb": serialize_results[
                            "memory_rss_delta"
                        ]
                        / (1024 * 1024),
                        "deserialize_memory_delta_mb": deserialize_results[
                            "memory_rss_delta"
                        ]
                        / (1024 * 1024),
                    }
                    ...
                    print(
                        f"  Serialize: {serialize_results['duration']:.3f}s ({serialization_results[size_name]['serialize_throughput_mb_s']:.1f} MB/s)"
                    )
                    print(
                        f"  Deserialize: {deserialize_results['duration']:.3f}s ({serialization_results[size_name]['deserialize_throughput_mb_s']:.1f} MB/s)"
                    )
                    print(
                        f"  Memory delta: +{serialization_results[size_name]['serialize_memory_delta_mb']:.1f}MB / +{serialization_results[size_name]['deserialize_memory_delta_mb']:.1f}MB"
                    )

                # Performance assertions
                medium_result = serialization_results["medium"]
                assert (
                    medium_result["serialize_time"] < 5.0
                ), f"Serialize too slow: {medium_result['serialize_time']}s"
                assert (
                    medium_result["deserialize_time"] < 5.0
                ), f"Deserialize too slow: {medium_result['deserialize_time']}s"
                assert (
                    medium_result["serialize_throughput_mb_s"] > 1.0
                ), f"Serialize throughput too low: {medium_result['serialize_throughput_mb_s']} MB/s"

            finally:
                await manager.shutdown()

    def _generate_state_data(
        self, conv_count: int, msg_per_conv: int
    ) -> Dict[str, Any]:
    ...
    @pytest.mark.asyncio
    async def test_restart_cycle_performance_analysis(
        self, performance_config, profiler
    ):
        ...
        try:
            ...
            for scenario_name, delay in scenarios:
                ...
                with patch.object(guardian, "start_process", side_effect=mock_start):
                    with patch.object(
                        guardian, "terminate_process", side_effect=mock_terminate
                    ):
                        # Measure restart performance
                        profiler.start_measurement(f"restart_{scenario_name}")

                        success = await guardian.restart_process(
                            f"Performance test {scenario_name}"
                        )
                        ...
                        restart_results[scenario_name] = {
                            ...
                            "memory_delta_mb": results["memory_rss_delta"]
                            / (1024 * 1024),
                        ...
                        print(
                            f"  Memory delta: {restart_results[scenario_name]['memory_delta_mb']:.2f}MB"
                        )
                        print(
                            f"  Overhead: {restart_results[scenario_name]['restart_overhead_ms']:.1f}ms"
                        )
            ...
            assert (
                immediate_result["duration"] < 1.0
            ), f"Immediate restart too slow: {immediate_result['duration']}s"

            normal_result = restart_results["normal_delay"]
            assert (
                normal_result["duration"] < 10.0
            ), f"Normal restart too slow: {normal_result['duration']}s"
    ...
    async def test_resource_usage_profiling(self, performance_config, profiler):
        ...
        try:
            ...
            with patch.object(guardian, "get_memory_usage") as mock_memory:
                ...
                for i, memory_val in enumerate(memory_values):
                    ...
                    sample = {
                        ...
                        "memory_delta_from_baseline_mb": baseline_comparison[
                            "memory_rss_delta_mb"
                        ],
                        "threads": process.num_threads(),
            ...
            max_memory_delta = max(
                s["memory_delta_from_baseline_mb"] for s in resource_samples
            )
            ...
            print(
                f"  Memory states seen: {set(s['memory_state'] for s in resource_samples)}"
            )
            ...
            ), f"Memory growth too high: {max_memory_delta}MB"
            assert total_samples > len(
                memory_values
            ), "Should have multiple monitoring samples"
    ...
    async def test_scalability_multiple_instances(self, profiler):
        ...
        try:
            ...
            for i in range(instance_count):
                config = MemoryGuardianConfig(
                    ...
                    thresholds=MemoryThresholds(
                        warning=500, critical=1000, emergency=1500
                    ),
                    monitoring=MonitoringConfig(
                        check_interval=0.1 + (i * 0.02)
                    ),  # Stagger intervals
            ...
            for guardian in instances:
                ...
                guardian.process = MagicMock()
                guardian.process.poll.return_value = None
                guardian.process_pid = os.getpid() + len(
                    instances
                )  # Fake different PIDs
                guardian.process_state = ProcessState.RUNNING
            ...
            with patch(
                "claude_mpm.services.infrastructure.memory_guardian.MemoryGuardian.get_memory_usage",
                return_value=300.0,
            ):
            ...
            print(
                f"  Startup memory delta: {startup_results['memory_rss_delta'] / (1024 * 1024):.2f}MB"
            )
            print(f"  Monitoring duration: {monitoring_results['duration']:.3f}s")
            print(
                f"  Monitoring memory delta: {monitoring_results['memory_rss_delta'] / (1024 * 1024):.2f}MB"
            )
            ...
            monitoring_memory_mb = monitoring_results["memory_rss_delta"] / (
                1024 * 1024
            )

            assert (
                startup_results["duration"] < 10.0
            ), f"Multi-instance startup too slow: {startup_results['duration']}s"
            assert (
                startup_memory_mb < 100.0
            ), f"Startup memory usage too high: {startup_memory_mb}MB"
            assert (
                monitoring_memory_mb < 100.0
            ), f"Monitoring memory usage too high: {monitoring_memory_mb}MB"
            assert (
                monitoring_results["cpu_delta"] < 30.0
            ), f"CPU usage too high: {monitoring_results['cpu_delta']}%"
        ...
        try:
            ...
            for sample_size in sample_sizes:
                ...
                for i in range(sample_size):
                    memory_mb = (
                        base_memory + (i * leak_rate) + (i * 0.1 * (i % 10))
                    )  # Add some noise
                ...
                print(
                    f"  Memory delta: {leak_detection_results[sample_size]['memory_delta_mb']:.2f}MB"
                )
                print(
                    f"  Leak detected: {leak_detection_results[sample_size]['leak_detected']}"
                )
                print(
                    f"  Processing rate: {leak_detection_results[sample_size]['samples_per_ms']:.1f} samples/ms"
                )

            # Performance assertions
            result_1000 = leak_detection_results[1000]
            assert (
                result_1000["detection_time"] < 0.5
            ), f"Leak detection too slow for 1000 samples: {result_1000['detection_time']}s"
            assert (
                result_1000["samples_per_ms"] > 100
            ), f"Processing rate too low: {result_1000['samples_per_ms']} samples/ms"
    ...
    async def test_dashboard_metrics_performance(self, profiler):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                print(
                    f"  Metrics collection: {avg_collection_time*1000:.3f}ms avg ({100/collection_results['duration']:.1f} ops/s)"
                )
                print(
                    f"  Dashboard data: {avg_dashboard_time*1000:.3f}ms avg ({50/dashboard_results['duration']:.1f} ops/s)"
                )
                print(
                    f"  Prometheus export: {avg_prometheus_time*1000:.3f}ms avg ({20/prometheus_results['duration']:.1f} ops/s)"
                )
                ...
                # Performance assertions
                assert (
                    avg_collection_time < 0.01
                ), f"Metrics collection too slow: {avg_collection_time*1000:.3f}ms"
                assert (
                    avg_dashboard_time < 0.02
                ), f"Dashboard data generation too slow: {avg_dashboard_time*1000:.3f}ms"
                assert (
                    avg_prometheus_time < 0.05
                ), f"Prometheus export too slow: {avg_prometheus_time*1000:.3f}ms"
    ...
    async def test_concurrent_operations_performance(
        ...
        try:
            ...
            with patch.object(guardian, "get_memory_usage", return_value=400.0):
                with patch.object(guardian, "start_process", return_value=True):
                    with patch.object(guardian, "terminate_process", return_value=True):
                        ...
                        for i in range(3):
                            operations.append(
                                guardian.restart_process(f"Concurrent test {i}")
                            )
            ...
            print(
                f"  Successful restarts: {successful_restarts}/{len(restart_results)}"
            )
            print(
                f"  Memory delta: {concurrent_results['memory_rss_delta'] / (1024 * 1024):.2f}MB"
            )
            print(f"  CPU delta: {concurrent_results['cpu_delta']:.2f}%")

            # Performance assertions
            assert (
                concurrent_results["duration"] < 15.0
            ), f"Concurrent operations too slow: {concurrent_results['duration']}s"
            assert (
                successful_ops >= len(operations) * 0.8
            ), f"Too many operation failures: {successful_ops}/{len(operations)}"
            # Should prevent most concurrent restarts
            assert (
                successful_restarts <= 1
            ), f"Too many concurrent restarts allowed: {successful_restarts}"
...
class TestPlatformSpecificPerformance:
    ...
    async def test_windows_performance(self):
        ...
        assert (
            avg_time_ms < 15.0
        ), f"Windows memory reading too slow: {avg_time_ms:.3f}ms"
