...
class TestMemoryGuardianPerformance:
    ...
    async def test_memory_monitoring_overhead(self):
        ...
        print(
            f"Monitoring overhead - CPU: {cpu_overhead:.2f}%, Memory: {memory_overhead:.2f}MB"
        )
    ...
    async def test_state_serialization_performance(self, large_state_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(
                f"State performance - Save: {save_time:.3f}s, Restore: {restore_time:.3f}s, Size: {file_size_mb:.2f}MB"
            )
    ...
    async def test_restart_cycle_performance(self):
        ...
        config = MemoryGuardianConfig(
            enabled=True, process_command=["python", "-c", "import time; time.sleep(1)"]
        )
        ...
        with patch.object(
            guardian, "start_process", new_callable=AsyncMock
        ) as mock_start:
            with patch.object(
                guardian, "terminate_process", new_callable=AsyncMock
            ) as mock_terminate:
    ...
    async def test_memory_trend_analysis_performance(self):
        ...
        assert (
            analysis_time < 0.1
        ), f"Trend analysis too slow: {analysis_time:.3f}s for {num_samples} samples"

        print(
            f"Trend analysis time: {analysis_time*1000:.2f}ms for {num_samples} samples"
        )
    ...
    async def test_dashboard_metrics_collection_performance(self):
        ...
        assert (
            avg_time < 0.01
        ), f"Metrics collection too slow: {avg_time*1000:.2f}ms per collection"
    ...
    async def test_concurrent_monitoring_performance(self):
        ...
        assert (
            cpu_usage < 20.0
        ), f"CPU usage too high with {len(guardians)} monitors: {cpu_usage}%"
        assert memory_growth < 100.0, f"Memory growth too high: {memory_growth}MB"

        print(
            f"Concurrent monitoring ({len(guardians)} instances) - CPU: {cpu_usage:.2f}%, Memory growth: {memory_growth:.2f}MB"
        )
    ...
    async def test_large_conversation_file_handling(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                write_time < 10.0
            ), f"Writing {file_size_mb:.1f}MB too slow: {write_time:.2f}s"
            assert (
                read_time < 10.0
            ), f"Reading {file_size_mb:.1f}MB too slow: {read_time:.2f}s"

            print(
                f"Large file handling ({file_size_mb:.1f}MB) - Write: {write_time:.2f}s, Read: {read_time:.2f}s"
            )
    ...
    async def test_memory_leak_in_monitoring(self):
        ...
        config = MemoryGuardianConfig(
            enabled=True, monitoring=MonitoringConfig(check_interval=0.01)
        )
        ...
        assert (
            average_growth_per_second < 1.0
        ), f"Possible memory leak: {average_growth_per_second:.2f}MB/s growth"

        print(
            f"Memory stability test - Growth: {memory_growth:.2f}MB over 10s ({average_growth_per_second:.3f}MB/s)"
        )
    ...
    async def test_cache_effectiveness(self):
        ...
        ), f"Cache ineffective: {second_time:.3f}s vs {first_time:.3f}s"

        print(
            f"Cache effectiveness - First: {first_time*1000:.2f}ms, Second: {second_time*1000:.2f}ms"
        )
...
class TestResourceUsageProfile:
    ...
    async def test_peak_resource_usage(self):
        ...
        print(
            f"Peak resource usage - CPU: {peak_cpu:.2f}%, Memory: {peak_memory:.2f}MB"
        )
