...
def test_memory_loading_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        expected_memories = ["PM_memories.md", "engineer_memories.md", "qa_memories.md", "research_memories.md"]
        ...
        print(f"Filtering efficiency: {reduction}/{old_count} = {reduction/old_count*100:.1f}% reduction")
        
        # Assert correctness
        assert new_count == 4, f"Expected 4 memory files, got {new_count}"
        assert all(name.endswith("_memories.md") for name in actual_memories), "All matched files should end with _memories.md"
