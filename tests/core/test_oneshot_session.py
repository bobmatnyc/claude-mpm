...
class TestOneshotSession:
    ...
    def test_setup_infrastructure_basic(self, oneshot_session):
        ...
        with patch.object(oneshot_session, '_prepare_environment', return_value={'ENV': 'test'}), \
             patch.object(oneshot_session, '_build_command', return_value=['claude', '--test']):
    ...
    def test_setup_infrastructure_with_user_pwd(self, oneshot_session, tmp_path):
        ...
        try:
            with patch.object(oneshot_session, '_prepare_environment', 
    ...
    def test_execute_command_success(self, oneshot_session):
        ...
        with patch.object(oneshot_session, '_build_final_command', 
                          return_value=['claude', '--model', 'opus', '--print', 'test context\n\ntest prompt']), \
             patch.object(oneshot_session, '_notify_execution_start'), \
             patch.object(oneshot_session, '_run_subprocess', return_value=(True, "success response")):
            
            success, response = oneshot_session.execute_command(prompt, context, infrastructure)
    ...
    def test_execute_command_no_context(self, oneshot_session):
        ...
        with patch.object(oneshot_session, '_build_final_command') as mock_build, \
             patch.object(oneshot_session, '_notify_execution_start'), \
             patch.object(oneshot_session, '_run_subprocess', return_value=(True, "response")):
    ...
    def test_build_final_command_with_context(self, oneshot_session):
        ...
        with patch.object(oneshot_session, '_get_simple_context', return_value="simple context"):
            oneshot_session.runner._create_system_prompt.return_value = "system prompt"
            
            result = oneshot_session._build_final_command(prompt, context, infrastructure)
            
            expected = ['claude', '--append-system-prompt', 'system prompt', '--print', 'test context\n\ntest prompt']
    ...
    def test_build_final_command_no_system_prompt(self, oneshot_session):
        ...
        with patch.object(oneshot_session, '_get_simple_context', return_value="simple context"):
            oneshot_session.runner._create_system_prompt.return_value = "simple context"  # Same as simple context
    ...
    def test_run_subprocess_timeout(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=timeout_error), \
             patch.object(oneshot_session, '_handle_timeout', return_value=(False, "timeout")) as mock_handle:
    ...
    def test_run_subprocess_file_not_found(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=FileNotFoundError()), \
             patch.object(oneshot_session, '_handle_claude_not_found', return_value=(False, "not found")) as mock_handle:
    ...
    def test_run_subprocess_permission_error(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=perm_error), \
             patch.object(oneshot_session, '_handle_permission_error', return_value=(False, "permission denied")) as mock_handle:
    ...
    def test_run_subprocess_keyboard_interrupt(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=KeyboardInterrupt()), \
             patch.object(oneshot_session, '_handle_keyboard_interrupt', return_value=(False, "interrupted")) as mock_handle:
    ...
    def test_run_subprocess_memory_error(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=mem_error), \
             patch.object(oneshot_session, '_handle_memory_error', return_value=(False, "memory error")) as mock_handle:
    ...
    def test_run_subprocess_unexpected_error(self, oneshot_session):
        ...
        with patch('subprocess.run', side_effect=unexpected_error), \
             patch.object(oneshot_session, '_handle_unexpected_error', return_value=(False, "unexpected")) as mock_handle:
    ...
    def test_cleanup_session_error_handling(self, oneshot_session):
        ...
        oneshot_session.runner.project_logger = Mock()
        oneshot_session.runner.project_logger.get_session_summary.side_effect = Exception("test error")
    ...
    def test_setup_websocket_success(self, oneshot_session):
        ...
        with patch('claude_mpm.services.socketio_server.SocketIOClientProxy', return_value=mock_proxy), \
    ...
    def test_setup_websocket_import_error(self, oneshot_session):
        ...
        with patch('claude_mpm.services.socketio_server.SocketIOClientProxy', side_effect=ImportError()):
            oneshot_session._setup_websocket()
    ...
    def test_handle_successful_response(self, oneshot_session, capsys):
        ...
        oneshot_session.runner.websocket_server.claude_output.assert_called_once_with(response, "stdout")
    ...
    def test_handle_error_response(self, oneshot_session, capsys):
        ...
        # Check WebSocket notifications
        oneshot_session.runner.websocket_server.claude_output.assert_called_once_with(error_msg, "stderr")
        oneshot_session.runner.websocket_server.claude_status_changed.assert_called_once_with(
            status="error",
...
class TestOneshotSessionIntegration:
    ...
    def test_full_oneshot_workflow_success(self, mock_runner):
        ...
        with patch('time.time', return_value=1234567890), \
            ...
            # Execute command
            exec_success, response = session.execute_command(prompt, context, infrastructure)
    ...
    def test_full_oneshot_workflow_with_error(self, mock_runner):
        ...
        with patch('time.time', return_value=1234567890), \
            ...
            exec_success, response = session.execute_command(prompt, None, infrastructure)
    ...
    def test_full_oneshot_workflow_with_websocket(self, mock_runner):
        ...
        with patch('time.time', return_value=1234567890), \
            ...
            mock_proxy.claude_output.assert_called_once_with("WebSocket response", "stdout")
...
if __name__ == "__main__":
    ...
    print("# Add To Memory:")
    print("Type: pattern")
    print("Content: Comprehensive unit tests for OneshotSession with 95%+ coverage including error scenarios")
    print("#")
