...
class TestOneshotSession:
    ...
    def test_initialize_session_success(self):
        ...
        with patch("time.time", return_value=1234567890), patch(
            ...
            success, error = self.initialize_session(prompt)

            assert success is True
            assert error is None
            assert self.start_time == 1234567890
            assert self.session_id == "test-session-id"

    def test_initialize_session_mpm_command(self):
        """Test session initialization with MPM command."""
        prompt = "/mpm:test"
        self.runner._handle_mpm_command.return_value = True

        success, error = self.initialize_session(prompt)

        assert success is True
        assert error is None
        self.runner._handle_mpm_command.assert_called_once_with("/mpm:test")

    def test_initialize_session_with_websocket(self):
        """Test session initialization with WebSocket enabled."""
        self.runner.enable_websocket = True

        with patch.object(self, "_setup_websocket") as mock_setup:
            success, error = self.initialize_session("test")

            assert success is True
            mock_setup.assert_called_once()

    def test_initialize_session_with_project_logger(self):
        """Test session initialization with project logger."""
        self.runner.project_logger = Mock()

        success, error = self.initialize_session("test prompt")
    ...
    def test_deploy_agents_setup_failure(self, capsys):
        """Test agent deployment when setup_agents fails."""
        self.runner.setup_agents.return_value = False
    ...
    def test_setup_infrastructure_basic(self):
    ...
    def test_setup_infrastructure_with_user_pwd(self, tmp_path):
        ...
        try:
            ...
            ), patch.object(self, "_build_command", return_value=["claude"]):
                result = self.setup_infrastructure()

                assert result["working_dir_changed"] is True
                assert result["env"]["CLAUDE_WORKSPACE"] == str(test_dir)
                assert self.original_cwd == original_cwd
                assert os.getcwd() == str(test_dir)
        finally:
            # Restore original directory
            os.chdir(original_cwd)

    def test_setup_infrastructure_invalid_directory(self):
        ...
        with patch.object(
            ...
            assert self.original_cwd is None
    ...
    def test_execute_command_no_context(self):
    ...
    def test_build_final_command_with_context(self):
    ...
    def test_build_final_command_no_system_prompt(self):
        ...
        with patch.object(
            ...
            result = self._build_final_command(prompt, None, infrastructure)

            expected = ["claude", "--print", "test prompt"]
            assert result == expected

    def test_notify_execution_start(self):
    ...
    def test_run_subprocess_success(self):
        ...
        with patch("subprocess.run", return_value=mock_result), patch.object(
            ...
            success, response = self._run_subprocess(cmd, env, prompt)
    ...
    def test_run_subprocess_timeout(self):
        ...
        with patch("subprocess.run", side_effect=timeout_error), patch.object(
            self, "_handle_timeout", return_value=(False, "timeout")
        ) as mock_handle:
            success, response = self._run_subprocess(cmd, env, prompt)
    ...
    def test_run_subprocess_permission_error(self):
    ...
    def test_run_subprocess_memory_error(self):
    ...
    def test_run_subprocess_unexpected_error(self):
    ...
    def test_cleanup_session_basic(self):
    ...
    def test_cleanup_session_with_original_cwd(self, tmp_path):
        """Test session cleanup with original directory restoration."""
        original_dir = str(tmp_path)
        self.original_cwd = original_dir

        with patch("os.chdir") as mock_chdir:
            self.cleanup_session()
            mock_chdir.assert_called_once_with(original_dir)

    def test_cleanup_session_with_project_logger(self):
        """Test session cleanup with project logger."""
        self.runner.project_logger = Mock()
        self.runner.project_logger.get_session_summary.return_value = {
    ...
    def test_cleanup_session_with_websocket(self):
        ...
        self.runner.websocket_server.session_ended.assert_called_once()

    def test_cleanup_session_error_handling(self):
    ...
    def test_setup_websocket_success(self):
        """Test successful WebSocket setup."""
        self.session_id = "test-session"
        self.runner.websocket_port = 8080
        ...
        with patch(
            ...
            assert self.runner.websocket_server == mock_proxy
    ...
    def test_setup_websocket_import_error(self):
    ...
    def test_setup_websocket_connection_error(self):
    ...
    def test_build_command_basic(self):
        ...
        result = self._build_command()

        expected = ["claude", "--model", "opus", "--dangerously-skip-permissions"]
        assert result == expected

    def test_build_command_with_claude_args(self):
    ...
    def test_handle_successful_response(self, capsys):
        """Test successful response handling."""
        self.start_time = time.time() - 5.0  # 5 seconds ago
        response = "Test response"
        prompt = "Test prompt"

        self.runner.response_logger = Mock()
        self.runner.websocket_server = Mock()
        self.runner.project_logger = Mock()
        self.runner.enable_tickets = False
        ...
        self.runner.response_logger.log_response.assert_called_once()

        # Check WebSocket notifications
        self.runner.websocket_server.claude_output.assert_called_once_with(
            response, "stdout"
        )

    def test_handle_successful_response_with_delegation(self, capsys):
        ...
        self.runner.websocket_server = Mock()
        self.runner._contains_delegation.return_value = True
        self.runner._extract_agent_from_response.return_value = "engineer"

        self._handle_successful_response(response, prompt)

        self.runner.websocket_server.agent_delegated.assert_called_once_with(
            agent="engineer", task=prompt[:100], status="detected"
        )

    def test_handle_successful_response_with_tickets(self, capsys):
        """Test successful response handling with ticket extraction."""
        self.start_time = time.time()
        response = "Response with tickets"
        prompt = "Test prompt"

        self.runner.enable_tickets = True
        self.runner.ticket_manager = Mock()

        self._handle_successful_response(response, prompt)

        self.runner._extract_tickets.assert_called_once_with(response)

    def test_handle_error_response(self, capsys):
        ...
        self._handle_error_response(error_msg, return_code)
    ...
    def test_handle_timeout(self, capsys):
        """Test timeout error handling."""
        timeout_error = subprocess.TimeoutExpired(["cmd"], 30)
        self.runner.project_logger = Mock()

        success, msg = self._handle_timeout(timeout_error)
    ...
    def test_handle_claude_not_found(self, capsys):
        """Test Claude not found error handling."""
        self.runner.project_logger = Mock()

        success, msg = self._handle_claude_not_found()
    ...
    def test_handle_permission_error(self, capsys):
        ...
        success, msg = self._handle_permission_error(perm_error)
    ...
    def test_handle_keyboard_interrupt(self, capsys):
        ...
        success, msg = self._handle_keyboard_interrupt()
    ...
    def test_handle_memory_error(self, capsys):
        """Test memory error handling."""
        mem_error = MemoryError("Out of memory")
        self.runner.project_logger = Mock()

        success, msg = self._handle_memory_error(mem_error)
    ...
    def test_handle_unexpected_error(self, capsys):
        ...
        success, msg = self._handle_unexpected_error(unexpected_error)
    ...
    def test_get_simple_context(self):
        ...
        with patch(
            ...
            result = self._get_simple_context()
...
class TestOneshotSessionIntegration:
    ...
    def test_full_oneshot_workflow_success(self):
    ...
    def test_full_oneshot_workflow_with_error(self):
    ...
    def test_full_oneshot_workflow_with_websocket(self):
