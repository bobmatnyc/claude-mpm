...
class TestPathResolution:
    ...
    def test_detect_framework_path_development(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            with patch.object(Path, "exists") as mock_exists:
                mock_exists.side_effect = lambda self: str(self).endswith(
                    "INSTRUCTIONS.md"
                )
    ...
    def test_detect_framework_path_packaged(self):
        ...
        with patch("claude_mpm.core.framework_loader.files") as mock_files:
            ...
            loader = FrameworkLoader()
            assert (
                loader.framework_path is None
            )  # Packaged installations have no file path
    ...
    def test_load_instructions_precedence(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            with patch.object(Path, "home", return_value=tmp_path):
                ...
                assert (
                    "project"
                    in loader.framework_content.get(
                        "custom_instructions_source", ""
                    ).lower()
                )
...
class TestMemorySystem:
    ...
    def test_load_agent_specific_memories(self, tmp_path):
        ...
        (memories_dir / "engineer_memories.md").write_text(
            "# Engineer Memory\nPattern: MVC"
        )
    ...
    def test_legacy_format_migration(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            ...
            with patch.object(loader, "_migrate_memory_file") as mock_migrate:
                content = {}
                with patch.object(
                    loader, "_get_deployed_agents", return_value={"engineer"}
                ):
                    loader._load_actual_memories(content)
...
# Analyzer Agent"""
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            ...
            with patch.object(
                loader, "_get_deployed_agents", return_value={"analyzer"}
            ):
...
class TestOutputStyle:
    ...
    def test_output_style_manager_initialization(self):
        ...
        with patch(
            "claude_mpm.core.output_style_manager.OutputStyleManager"
        ) as MockManager:
...
class TestCaching:
    ...
    def test_ttl_expiration(self):
        ...
        # Check expiration
        if (
            time.time() - loader._agent_capabilities_cache_time
            > loader.CAPABILITIES_CACHE_TTL
        ):
            loader._agent_capabilities_cache = None

        if (
            time.time() - loader._deployed_agents_cache_time
            > loader.DEPLOYED_AGENTS_CACHE_TTL
        ):
...
# PM Agent""",
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            ...
            with patch.object(
                loader, "_get_deployed_agents", return_value={"engineer", "qa", "pm"}
            ):
                capabilities = loader._generate_agent_capabilities_section()
...
class TestBackwardCompatibility:
    ...
    def test_legacy_memory_format_support(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=tmp_path):
            ...
            with patch.object(loader, "_migrate_memory_file") as mock_migrate:
                with patch.object(
                    loader, "_get_deployed_agents", return_value={"engineer", "qa"}
                ):
                    content = {}
                    loader._load_actual_memories(content)
