...
def test_dashboard_event_flow():
    ...
    def on_claude_event(data):
        print(f"   📨 Dashboard received event - Type: {data.get('type')}, Subtype: {data.get('subtype')}")
    ...
    if received_events:
        ...
        test_events = [
            {"hook_event_name": "PreToolUse", "hook_input_data": {"tool": "read_file", "path": "/test/file.txt"}},
            {"hook_event_name": "PostToolUse", "hook_input_data": {"tool": "read_file", "result": "success"}},
            {"hook_event_name": "AssistantResponse", "hook_input_data": {"response": "Test response"}},
        ]
        ...
        for test_event in test_events:
            ...
            try:
                response = requests.post('http://localhost:8765/api/events', json=test_event, timeout=2)
                print(f"   Sent {test_event['hook_event_name']}: {response.status_code}")
            except Exception as e:
        ...
        success = len(received_events) >= 1  # At least the first event should be received
    else:
