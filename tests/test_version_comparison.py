...
def main():
    ...
    print(f"  - qa.json (v1.0.0) - LOWER than system")
    print(f"  - engineer.json (v5.0.0) - HIGHER than system")
    print()
        ...
        for agent_file in sorted(deployed_files):
            ...
            if agent_file.stem == "qa":
                if version == "3.2.0" and source == "system":
                    print(f"    ✓ Correctly using system version (project had v1.0.0)")
                else:
                    print(f"    ✗ ERROR: Expected system v3.2.0, got {source} v{version}")
            
            elif agent_file.stem == "engineer":
                if version == "5.0.0" and source == "project":
                    print(f"    ✓ Correctly using project version (higher than system)")
                else:
