...
class TestMemoryGuardianLifecycle:
    ...
    async def test_restart_with_state_preservation(self, memory_guardian):
        ...
        with patch.object(memory_guardian, "start_process") as mock_start:
            with patch.object(memory_guardian, "terminate_process") as mock_terminate:
                ...
                # Perform restart
                success = await memory_guardian.restart_process(
        ...
        assert (
            restored_state["user_data"]["session_id"]
            == test_state["user_data"]["session_id"]
        )
    ...
    async def test_memory_leak_detection_and_response(self, memory_guardian):
        ...
        assert (
            trend.is_leak_suspected
        ), "Memory leak should be detected with 25MB/sample growth"
        ...
        with patch.object(memory_guardian, "get_memory_usage") as mock_memory:
            with patch.object(memory_guardian, "restart_process") as mock_restart:
                mock_memory.return_value = base_memory + (
                    19 * growth_rate
                )  # High memory
                mock_restart.return_value = True
                ...
                if (
                    trend.is_leak_suspected
                    and mock_memory.return_value
                    > memory_guardian.config.thresholds.critical
                ):
    ...
    async def test_graceful_degradation_under_stress(self, memory_guardian):
        ...
        await degradation.disable_feature(
            "detailed_logging", "Memory pressure detected"
        )
    ...
    async def test_concurrent_operations_safety(self, memory_guardian):
        ...
        with patch.object(memory_guardian, "start_process") as mock_start:
            with patch.object(memory_guardian, "terminate_process") as mock_terminate:
                ...
                monitor_results = [
                    r for r in results[3:] if r is None
                ]  # monitor_memory returns None
    ...
    async def test_error_recovery_and_resilience(self, memory_guardian):
        ...
        with patch.object(
            memory_guardian, "_save_state", side_effect=Exception("Disk full")
        ):
            try:
        ...
        with patch.object(
            memory_guardian,
            "get_memory_usage",
            side_effect=Exception("Memory access failed"),
        ):
            try:
                await memory_guardian.monitor_memory()
        ...
        with patch.object(
            memory_guardian,
            "start_process",
            side_effect=Exception("Process start failed"),
        ):
            try:
    ...
    async def test_state_persistence_across_restarts(
        self, memory_guardian, temp_workspace
    ):
        ...
        for i in range(3):
            state = {
                ...
                "timestamp": time.time(),
                "user_sessions": [
                    {"id": f"session_{j}", "active": True} for j in range(5)
                ],
                "configuration": {"memory_limit": 1000 + (i * 100), "restart_count": i},
            }
...
class TestMemoryGuardianEdgeCases:
    ...
    async def edge_case_config(self, tmp_path):
        ...
        return MemoryGuardianConfig(
            enabled=True,
            thresholds=MemoryThresholds(
                warning=1, critical=2, emergency=3
            ),  # Very low thresholds
            restart_policy=RestartPolicy(
                max_attempts=1, attempt_window=1
            ),  # Very restrictive
    ...
    async def test_extremely_rapid_memory_growth(self, edge_case_config):
        ...
        with patch.object(guardian, "get_memory_usage") as mock_memory:
            with patch.object(guardian, "restart_process") as mock_restart:
                ...
                assert (
                    restart_triggered
                ), "Should trigger immediate restart for extreme memory spike"
...
class TestMemoryGuardianPlatformSpecific:
    ...
    async def test_unix_signal_handling(self):
        ...
        guardian.process.poll = MagicMock(
            side_effect=[None, None, 0]
        )  # Process exits after 2 polls
