...
class TestMemoryGuardianIntegration:
            ...
            for i in range(5):
                ...
                if i < 3:
                    ...
                    if memory_guardian.restart_protection:
                        stats = (
                            memory_guardian.restart_protection.get_restart_statistics()
                        )
    ...
    async def test_state_preservation_across_restarts(self, memory_guardian):
        ...
        if state_manager:
            ...
            with patch.object(memory_guardian, "start_process", return_value=True):
                with patch.object(
                    memory_guardian, "terminate_process", return_value=True
                ):
    ...
    async def test_graceful_degradation(self, memory_guardian):
        ...
        if degradation:
            ...
            await degradation.degrade_feature(
                "memory_monitoring", "Test failure", "basic mode"
            )
    ...
    async def test_error_recovery(self, memory_guardian):
        ...
        with patch.object(
            memory_guardian, "_save_state", side_effect=Exception("Save failed")
        ):
            # Should not crash
        ...
        with patch.object(
            memory_guardian, "get_memory_usage", side_effect=Exception("Memory error")
        ):
...
class TestGracefulDegradationUnit:
    ...
    async def test_feature_degradation(self):
        ...
        # Degrade the feature
        await degradation.degrade_feature(
            "test_feature", "Test reason", "fallback mode"
        )
