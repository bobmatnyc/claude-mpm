...
async def test_ticket_workflow():
    ...
    list_invocation = MCPToolInvocation(
        tool_name="ticket_list", parameters={"limit": 5, "type": "task"}
    )
        ...
        if result.success:
            ...
            if isinstance(result.data, list):
                for ticket in result.data[:5]:
                    print(
                        f"   - {ticket.get('id', 'Unknown')}: {ticket.get('title', 'Unknown')}"
                    )
            else:
    ...
    if ticket_id and ticket_id != "Unknown":
        ...
        view_invocation = MCPToolInvocation(
            ...
            parameters={"ticket_id": ticket_id, "format": "json"},
        )
    ...
    search_invocation = MCPToolInvocation(
        tool_name="ticket_search", parameters={"query": "MCP", "limit": 5}
    )
