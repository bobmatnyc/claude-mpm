...
def test_configuration():
    ...
    assert config.get('response_logging.enabled', True) == False, "Response logging should be disabled"
    assert config.get('response_tracking.enabled', True) == False, "Response tracking should be disabled"
    print("✅ Old response logging disabled")
    
    # Check event aggregator is enabled
    assert config.get('event_aggregator.enabled', False) == True, "Event aggregator should be enabled"
    print("✅ Event aggregator enabled")
    
    # Check activity directory
    activity_dir = config.get('event_aggregator.activity_directory', '')
    assert activity_dir == '.claude-mpm/activity', f"Activity directory should be .claude-mpm/activity, got {activity_dir}"
...
def test_aggregator_initialization():
    ...
    assert aggregator.save_dir == expected_dir, f"Save dir should be {expected_dir}, got {aggregator.save_dir}"
