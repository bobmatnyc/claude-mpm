...
def run_hook_handler_test(
    ...
    hook_handler_path = (
        project_root / "src/claude_mpm/hooks/claude_hooks/hook_handler.py"
    )
...
def test_debug_mode_scenarios() -> Dict[str, Dict]:
    ...
    test_scenarios = [
        ...
        (
            ...
            "false",
            False,
            "CLAUDE_MPM_HOOK_DEBUG=false should disable debug",
        ),
        (
            "false_upper",
            "FALSE",
        ...
        (
            ...
            "no",
            True,
            "CLAUDE_MPM_HOOK_DEBUG=no should enable debug (only 'false' disables)",
        ),
        ...
        (
            ...
            "0",
            True,
            "CLAUDE_MPM_HOOK_DEBUG=0 should enable debug (only 'false' disables)",
        ),
        (
            "disabled",
            "disabled",
            True,
            "CLAUDE_MPM_HOOK_DEBUG=disabled should enable debug",
        ),
    ]
    ...
    for test_name, env_value, expected_debug, description in test_scenarios:
        ...
        if test_passed:
            print(
                f"   ✅ PASSED: Debug is {'ON' if actual_debug else 'OFF'} as expected"
            )
        else:
            print(
                f"   ❌ FAILED: Debug is {'ON' if actual_debug else 'OFF'}, expected {'ON' if expected_debug else 'OFF'}"
            )
        ...
        if actual_debug and stderr:
            debug_lines = [
                ...
                if any(
                    indicator in line
                    for indicator in ["✅", "❌", "[DEBUG]", "Memory", "Response"]
                )
            ][:3]
            if debug_lines:
...
def print_summary(results: Dict[str, Dict]):
    ...
    false_working = all(
        results[name]["success"] for name in false_tests if name in results
    )
    if false_working:
        print(
...
def main():
    ...
    print(
        "Testing that debug mode is enabled by default and can be controlled via CLAUDE_MPM_HOOK_DEBUG"
    )
