...
- docs/ - Documentation''',
        ...
        print("Test Summary")
        print("="*60)
        print(f"✓ Delegation tracking: {'PASS' if session_id in handler.delegation_requests or len(response_files) > 0 else 'FAIL'}")
...
def test_configuration_loading():
    ...
    if config_path.exists():
        ...
        print(f"  session_directory: {response_config.get('session_directory', '.claude-mpm/responses')}")
        print(f"  track_all_interactions: {response_config.get('track_all_interactions', False)}")
    else:
