...
def test_agent_exclusion():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if excluded_found:
            print(f"   ❌ ERROR: Excluded agents were deployed: {', '.join(excluded_found)}")
        else:
        ...
        config.set('agent_deployment.excluded_agents', ['Research', 'Data_Engineer'])  # Wrong case
        ...
        if 'research' in deployed_agents and 'data_engineer' in deployed_agents:
            print(f"   ✅ Case-sensitive matching works: agents not excluded with wrong case")
        else:
