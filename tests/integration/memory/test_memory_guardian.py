...
async def test_memory_monitoring():
    ...
    try:
        ...
        for i in range(30):
            ...
            symbol = "✓" if state == "normal" else "⚠" if state == "warning" else "⚡" if state == "critical" else "🔥"
            print(f"   [{i+1:2d}] Memory: {memory_mb:6.2f}MB | State: {state:9s} {symbol}")
...
async def test_restart_policy():
    ...
    try:
        ...
        for i in range(5):
            ...
            print(f"   - Consecutive failures: {status['restarts']['consecutive_failures']}")
...
async def test_state_hooks():
    ...
    def save_hook(state):
        ...
        print(f"   State saved: PID={state.get('process_pid')}, "
