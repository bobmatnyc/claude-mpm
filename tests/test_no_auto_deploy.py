...
def test_no_automatic_deployment():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logger.info("Testing deploy_agents() - should NOT create system instructions...")
        ...
        for file_path in instructions_in_claude:
            if file_path.exists():
                logger.error(f"❌ FAILED: {file_path} was created automatically (should NOT happen)")
                return False
        
        logger.info("✅ PASS: No system instructions were automatically created in .claude/")
        ...
        for file_path in instructions_in_mpm:
            if not file_path.exists():
                logger.error(f"❌ FAILED: {file_path} was NOT created by explicit deployment")
                return False
        
        logger.info("✅ PASS: System instructions were deployed to .claude-mpm/ when explicitly requested")
        ...
        if loader.framework_content.get("custom_instructions"):
            logger.info("✅ PASS: Framework loader found custom instructions in .claude-mpm/")
        else:
            logger.warning("⚠️  Framework loader didn't find custom instructions (may be expected if not in test dir)")
...
def test_framework_loader_paths():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (claude_dir / "INSTRUCTIONS.md").write_text("# Wrong Instructions\nShould not be loaded.")
        ...
        try:
            ...
            if "Test Instructions" in custom_instructions:
                ...
                if "Wrong Instructions" in custom_instructions:
                    logger.error("❌ FAILED: Framework loader also read from .claude/ (should NOT)")
                    return False
                return True
            else:
