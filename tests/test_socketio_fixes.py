...
def test_python_environment_detection():
    ...
    from claude_mpm.scripts.socketio_daemon import get_python_executable, PYTHON_EXECUTABLE
    ...
    if in_venv or venv_env:
        ...
        if 'venv' in PYTHON_EXECUTABLE or PYTHON_EXECUTABLE == sys.executable:
...
def test_event_loop_initialization():
    ...
    try:
        ...
        server = SocketIOServer(host="localhost", port=18765)  # Use different port for testing
...
def main():
    ...
    results.append(("Python Environment Detection", test_python_environment_detection()))
