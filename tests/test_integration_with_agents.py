...
from claude_mpm.services.async_session_logger import AsyncSessionLogger, LogFormat, log_response_async
...
def test_backward_compatibility():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_requests = [
            ("User asked for code review", "Reviewed Python code and found 3 issues", {"agent": "engineer"}),
            ("Deploy application request", "Successfully deployed v1.2.3 to production", {"agent": "ops"}),
            ("Query database for metrics", "Retrieved 1,245 records in 0.15s", {"agent": "data_engineer"}),
        ]
        ...
        print(f"  ✓ Original logger created {original_files} files in {original_time*1000:.3f}ms")
        ...
        print(f"  ✓ Queue time: {queue_time*1000:.3f}ms (vs original: {original_time*1000:.3f}ms)")
        print(f"  ✓ Total time: {total_time*1000:.3f}ms")
        print(f"  ✓ Performance improvement: {(original_time - queue_time)/original_time*100:.1f}%")
        ...
        essential_fields = ["request_summary", "response_content", "metadata", "session_id"]
        fields_match = all(field in both for field in essential_fields for both in [original_data, async_data])
        ...
        return {
            ...
            "performance_improvement_pct": (original_time - queue_time)/original_time*100,
...
def test_async_convenience_function():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_responses = [
            ("Global function test 1", "First test of global async logging", {"agent": "qa"}),
            ("Global function test 2", "Second test with different metadata", {"agent": "test", "priority": "high"}),
            ("Global function test 3", "Third test for consistency", {"agent": "integration"}),
        ]
        ...
        print(f"  ✓ Global function queued {len(test_responses)} responses in {queue_time*1000:.3f}ms")
        print(f"  ✓ Average queue time per response: {queue_time/len(test_responses)*1000:.3f}ms")
...
def test_session_id_handling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logger2 = AsyncSessionLogger(
            base_dir=Path(tmpdir) / "manual_test",
            enable_async=True
        )
        ...
        return {
            ...
            "success": env_detection_works and manual_session_set and env_session_files > 0 and manual_session_files > 0
        }
...
def test_metadata_handling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        expected_unique = len(test_cases)  # Each test case has different expected agent name
...
def test_error_recovery():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  ✓ Logger stats: queued={stats['queued']}, dropped={stats['dropped']}, errors={stats['errors']}")
        ...
        invalid_test_cases = [
            ...
            ("", "", {}),  # Empty strings
            ("Very long request " * 1000, "Very long response " * 1000, {"agent": "stress", "data": "x" * 10000}),  # Large data
        ]
...
def main():
    ...
    print(f"  ✓ Performance improvement: {compatibility_result['performance_improvement_pct']:.1f}%")
    print(f"  ✓ Files created: {compatibility_result['async_files']}/{compatibility_result['original_files']}")
    print(f"  ✓ Field compatibility: {'✅' if compatibility_result['fields_match'] else '❌'}")
    ...
    print(f"\n🆔 Session Handling:")
    print(f"  ✓ Environment detection: {'✅' if session_result['env_detection_works'] else '❌'}")
    ...
    print(f"  ✓ Error recoveries: {error_result['error_recoveries']}/{error_result['total_invalid_cases']}")
    print(f"  ✓ Graceful degradation: {'✅' if error_result['graceful_degradation'] else '❌'}")
