...
try:
    ...
    from claude_mpm.services.recovery_manager import (
        RecoveryManager, GradedRecoveryStrategy, CircuitBreaker,
        RecoveryAction, CircuitState, RecoveryEvent
    )
...
class MockProcess:
    ...
    def __init__(self, pid=1234, running=True, status="running", 
                 cpu_percent=25.5, memory_mb=128, num_fds=50, 
                 num_threads=10, create_time=None):
        self.pid = pid
...
class TestProcessResourceChecker:
    ...
    async def test_process_resource_checker_healthy(self, mock_process_class, mock_process):
    ...
    async def test_process_resource_checker_critical(self, mock_process_class):
        ...
        mock_process_class.return_value = critical_mock
        ...
        warning_critical = [m for m in metrics if m.status in [HealthStatus.WARNING, HealthStatus.CRITICAL]]
...
class TestAdvancedHealthMonitor:
    ...
    async def test_health_monitor_initialization(self, health_config):
        ...
        assert monitor.check_interval == health_config['check_interval']
        assert monitor.history_size == health_config['history_size']
    ...
    async def test_add_checker(self, health_config, mock_process):
        ...
        with patch('claude_mpm.services.health_monitor.psutil.Process', return_value=mock_process):
            checker = ProcessResourceChecker(pid=1234)
...
class TestCircuitBreaker:
    ...
    def test_circuit_breaker_half_open_success(self):
        ...
        cb = CircuitBreaker(failure_threshold=2, timeout_seconds=0.1, success_threshold=2)
...
class TestRecoveryManager:
    ...
    async def test_recovery_manager_initialization(self, recovery_config):
        ...
        assert manager.check_interval == recovery_config['check_interval']
...
class TestStandaloneSocketIOServerIntegration:
    ...
    @patch('claude_mpm.services.standalone_socketio_server.HEALTH_MONITORING_AVAILABLE', True)
    def test_server_health_monitoring_initialization(self):
        """Test that server initializes health monitoring when available."""
        with patch('claude_mpm.services.standalone_socketio_server.AdvancedHealthMonitor') as mock_monitor, \
             patch('claude_mpm.services.standalone_socketio_server.RecoveryManager') as mock_recovery:
    ...
    @patch('claude_mpm.services.standalone_socketio_server.HEALTH_MONITORING_AVAILABLE', False)
