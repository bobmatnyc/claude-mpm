...
def test_concurrent_access():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        # Submit all tasks
        futures = [executor.submit(concurrent_access_test, i) for i in range(num_threads)]
    ...
    if len(set(capabilities_sizes)) == 1 and capabilities_sizes[0] != -1:
        print(f"  ✅ All threads got same capabilities size: {capabilities_sizes[0]} chars")
    else:
        print(f"  ❌ Inconsistent capabilities sizes: {set(capabilities_sizes)}")
    
    if all(memories_loaded):
        print(f"  ✅ All threads successfully loaded memories")
    else:
        print(f"  ❌ Some threads failed to load memories")
...
def test_cache_expiry():
    ...
    if time2 < time1 and time3 > time2:
        ...
        print(f"   Cache expired correctly after TTL")
    else:
