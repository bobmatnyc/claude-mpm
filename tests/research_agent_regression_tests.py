...
class TestResearchAgentRegressions:
    ...
    def research_agent_template(self):
        ...
        template_path = Path(__file__).parent.parent / "src/claude_mpm/agents/templates/research.json"
    ...
    def test_mandatory_file_reading_requirement(self, research_agent_template):
        ...
        assert "MANDATORY verification" in instructions or "MANDATORY file content reading" in constraints
    ...
    def test_search_completeness_requirements(self, research_agent_template):
        ...
        assert ("ALL 5 REQUIRED" in instructions or 
                "multiple search strategies" in instructions.lower())
        ...
        assert "Strategy C:" in instructions or "Import/dependency analysis" in instructions
        assert "Strategy D:" in instructions or "Directory structure examination" in instructions
    ...
    def test_similarity_search_scenario_coverage(self, research_agent_template):
        ...
        assert "Multiple search strategies" in instructions or "search strategies" in instructions
    ...
    def test_time_limit_flexibility(self, research_agent_template):
        ...
        assert "NEVER follow rigid time limits if investigation incomplete" in instructions
        
        # Verify quality over speed
        assert "thorough investigation that takes longer is ALWAYS better" in instructions
...
class TestOriginalFailureScenarioSimulation:
    ...
    def test_similarity_search_in_api_services_scenario(self):
        ...
        assert all(expected_behavior.values()), "New agent should exhibit all required behaviors"
    ...
    def test_large_codebase_handling_scenario(self):
        ...
        assert all(expected_behavior.values()), "Agent should handle large codebases thoroughly"
    ...
    def test_complex_multi_file_feature_scenario(self):
        ...
        assert all(expected_behavior.values()), "Agent should discover complex multi-file features"
