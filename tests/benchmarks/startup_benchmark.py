...
class StartupBenchmark:
    ...
    def __init__(self):
        ...
        if not self.claude_mpm_script.exists():
            raise FileNotFoundError(
                f"claude-mpm script not found at {self.claude_mpm_script}"
            )
    ...
    def analyze_results(self, measurements: List[StartupMeasurement]) -> Dict:
        ...
        return {
            ...
            "by_mode": mode_stats,
            "failed_measurements": [
                {"mode": m.mode, "error": m.error_message, "timestamp": m.timestamp}
...
async def main():
    ...
    parser.add_argument(
        "--samples", type=int, default=5, help="Number of samples per test"
    )
    parser.add_argument(
