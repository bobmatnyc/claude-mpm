...
class CacheBenchmark:
    ...
    async def test_cache_basic_operations(self, cache: SharedPromptCache, 
                                         operations: int = 1000) -> CacheTestResult:
    ...
    async def test_cache_hit_rate_patterns(self, cache: SharedPromptCache, 
                                          pattern: str = "mixed", 
                                          operations: int = 1000) -> CacheTestResult:
        """Test cache hit rates with different access patterns."""
        self.logger.info(f"📊 Testing cache hit rates with {pattern} pattern ({operations} ops)...")
        ...
        try:
            ...
            for i in range(cache_size):
                ...
                value = {"pattern": pattern, "id": i, "data": self.generate_test_data(500)}
                cache.set(key, value, ttl=600)
            
            # Test different access patterns
            if pattern == "sequential":
                # Sequential access - high hit rate expected
                keys = [f"pattern_{pattern}_{i % cache_size}" for i in range(operations)]
            
            elif pattern == "random":
                # Random access - mixed hit/miss rate
                keys = [f"pattern_{pattern}_{random.randint(0, cache_size * 2)}" for _ in range(operations)]
            
            elif pattern == "mixed":
                # Mixed pattern - 70% hits, 30% misses
                hit_keys = [f"pattern_{pattern}_{i % cache_size}" for i in range(operations * 7 // 10)]
                miss_keys = [f"pattern_{pattern}_miss_{i}" for i in range(operations * 3 // 10)]
    ...
    async def test_cache_concurrency(self, cache: SharedPromptCache, 
                                   concurrent_workers: int = 10, 
                                   operations_per_worker: int = 100) -> CacheTestResult:
        """Test cache performance under concurrent load."""
        self.logger.info(f"⚡ Testing cache concurrency ({concurrent_workers} workers, {operations_per_worker} ops each)...")
        ...
        async def worker_task(worker_id: int):
            ...
            for i in range(operations_per_worker):
                ...
                if random.random() < 0.3:  # 30% set operations
                    value = {"worker": worker_id, "iteration": i, "data": self.generate_test_data(200)}
    ...
    async def test_cache_memory_efficiency(self, cache: SharedPromptCache) -> CacheTestResult:
        ...
        try:
            ...
            expected_size_mb = sum(size * entries_per_size for size in data_sizes) / (1024 * 1024)
            memory_efficiency = (expected_size_mb / memory_growth * 100) if memory_growth > 0 else 0
            ...
            return CacheTestResult(
                ...
                miss_rate_percent=100 - (retrieved_count / (len(data_sizes) * 10) * 100),
                operations_per_second=total_operations / duration if duration > 0 else 0,
    ...
    async def test_cache_ttl_behavior(self, cache: SharedPromptCache) -> CacheTestResult:
        ...
        try:
            ...
            for ttl in ttl_values:
                for i in range(entries_per_ttl):
                    ...
                    value = {"ttl": ttl, "created": time.time(), "data": self.generate_test_data(100)}
            ...
            immediate_rate = (immediate_hits / total_operations * 100) if total_operations > 0 else 0
            partial_rate = (partial_hits / total_operations * 100) if total_operations > 0 else 0
            final_rate = (final_hits / total_operations * 100) if total_operations > 0 else 0
    ...
    async def run_comprehensive_cache_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_methods = [
                ...
                (self.test_cache_hit_rate_patterns, {"pattern": "sequential", "operations": 500}),
                (self.test_cache_hit_rate_patterns, {"pattern": "random", "operations": 500}),
                (self.test_cache_hit_rate_patterns, {"pattern": "mixed", "operations": 500}),
                (self.test_cache_concurrency, {"concurrent_workers": 5, "operations_per_worker": 50}),
            ...
            if successful_results:
                avg_hit_rate = statistics.mean([r.hit_rate_percent for r in successful_results])
                avg_ops_per_sec = statistics.mean([r.operations_per_second for r in successful_results])
            ...
            benchmark_results = {
                ...
                "aggregate_metrics": {
                    ...
                    "cache_effectiveness_score": min(100, avg_hit_rate * 2)  # Score out of 100
                },
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        print(f"\n📊 AGGREGATE METRICS:")
        print(f"{'='*60}")
        print(f"Average Hit Rate:        {metrics.get('average_hit_rate_percent', 0):.1f}%")
        print(f"Average Ops/Sec:         {metrics.get('average_operations_per_second', 0):.0f}")
        print(f"Total Operations:        {metrics.get('total_operations', 0):,}")
        print(f"Cache Effectiveness:     {metrics.get('cache_effectiveness_score', 0):.1f}/100")
        ...
        print(f"Actual Hit Rate:         {metrics.get('average_hit_rate_percent', 0):.1f}%")
        ...
        for result in results.get("test_results", []):
            ...
            print(f"{result['test_name']:25} {status} {hit_rate:6.1f}% hit rate, {ops_per_sec:6.0f} ops/s")
