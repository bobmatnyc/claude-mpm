...
class ConnectionBenchmark:
    ...
    async def test_basic_connection_reliability(
        self, attempts: int = 100, concurrent_limit: int = 10
    ) -> ConnectionTestResult:
        """Test basic connection reliability without pooling."""
        self.logger.info(
            f"🔌 Testing basic connection reliability ({attempts} attempts)..."
        )
    ...
    async def test_connection_pool_performance(
        ...
        self.logger.info(
            f"🏊 Testing connection pool performance (pool: {pool_size}, requests: {total_requests})..."
        )
        ...
        if self.baseline_failure_rate > 0:
            error_reduction = (
                (self.baseline_failure_rate - failure_rate) / self.baseline_failure_rate
            ) * 100
    ...
    async def test_socketio_connection_stability(
        self, connections: int = 20, duration_seconds: int = 30
    ) -> ConnectionTestResult:
        """Test Socket.IO connection stability under load."""
        self.logger.info(
            f"🔄 Testing Socket.IO connection stability ({connections} connections, {duration_seconds}s)..."
        )
        ...
        async def test_socketio_connection(connection_id: int):
            ...
            try:
                ...
                if (
                    len(active_connections) < connections * 0.8
    ...
    async def test_connection_recovery(
        self, recovery_cycles: int = 5, connections_per_cycle: int = 10
    ) -> ConnectionTestResult:
        """Test connection recovery and reconnection behavior."""
        self.logger.info(
            f"🔄 Testing connection recovery ({recovery_cycles} cycles, {connections_per_cycle} connections each)..."
        )
    ...
    async def run_comprehensive_connection_benchmark(self) -> Dict[str, Any]:
        ...
        tests = [
            (
                self.test_basic_connection_reliability,
                {"attempts": 50, "concurrent_limit": 5},
            ),
            (
                self.test_connection_pool_performance,
                {"pool_size": 5, "total_requests": 100, "concurrent_requests": 10},
            ),
            (
                self.test_socketio_connection_stability,
                {"connections": 10, "duration_seconds": 15},
            ),
            (
                self.test_connection_recovery,
                {"recovery_cycles": 3, "connections_per_cycle": 5},
            ),
        ]
        ...
        if successful_results:
            avg_success_rate = statistics.mean(
                [r.success_rate_percent for r in successful_results]
            )
            avg_failure_rate = statistics.mean(
                [r.failure_rate_percent for r in successful_results]
            )
            avg_error_reduction = (
                statistics.mean(
                    [
                        r.error_reduction_percent
                        for r in successful_results
                        if r.error_reduction_percent is not None
                    ]
                )
                if any(
                    r.error_reduction_percent is not None for r in successful_results
                )
                else 0.0
            )
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"Average Failure Rate:    {metrics.get('average_failure_rate_percent', 0):.1f}%"
        )
        print(
            f"Error Reduction:         {metrics.get('average_error_reduction_percent', 0):.1f}%"
        )
        print(
            f"Total Attempts:          {metrics.get('total_connection_attempts', 0):,}"
        )
        print(
            f"Total Successes:         {metrics.get('total_successful_connections', 0):,}"
        )
        print(
            f"Reliability Score:       {metrics.get('overall_reliability_score', 0):.1f}/100"
        )
        ...
        for result in results.get("test_results", []):
            ...
            print(
                f"{result['test_name']:30} {status} {success_rate:6.1f}% success, {error_reduction:5.1f}% error reduction"
            )
