...
"""
    ...
    def test_frontmatter_required_fields(self, tmp_path):
        ...
        required_fields = ['name', 'description', 'version', 'base_version', 'tools', 'model']
        ...
        for missing_field in required_fields:
            ...
            assert missing_field not in frontmatter or frontmatter[missing_field] is None
...
"""
            ...
            assert is_valid == should_be_valid, f"Version '{version}' ({description}) validation mismatch"
    ...
    def test_frontmatter_tools_format(self, tmp_path):
        ...
        test_cases = [
            ...
            ("Read, Write, Edit, Grep, Glob, LS, WebSearch, TodoWrite", True, "Many tools"),
            ("", False, "Empty tools"),
            ("Read Write Edit", True, "Space-separated (legacy)"),
            ("[Read, Write, Edit]", True, "List format (parsed as string)"),  # Will be parsed as string
        ]
...
"""
            ...
            assert is_valid == should_be_valid, f"Tools '{tools}' ({description}) validation mismatch"
    ...
    def _validate_frontmatter(self, frontmatter: Optional[Dict[str, Any]]) -> List[str]:
        ...
        if 'base_version' in frontmatter:
            if not self._is_valid_semantic_version(frontmatter['base_version']):
                errors.append(f"Invalid base_version format: {frontmatter['base_version']}")
