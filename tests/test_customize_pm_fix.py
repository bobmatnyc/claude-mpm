...
def test_customize_pm_location():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            print(f"✅ customize-pm correctly writes to {expected_file.relative_to(Path(tmpdir))}")
...
"""
            ...
            assert loader.framework_content.get("custom_instructions"), "Custom instructions not loaded"
            ...
            print(f"✅ Framework loader correctly loads from {instructions_file.relative_to(Path(tmpdir))}")
...
def test_no_claude_md_created():
    ...
    agent_manager_file = Path(__file__).parent.parent / "src" / "claude_mpm" / "cli" / "commands" / "agent_manager.py"
    ...
    for pattern in bad_patterns:
        if pattern in content and ".claude-mpm" not in content[content.find(pattern)-50:content.find(pattern)+50]:
