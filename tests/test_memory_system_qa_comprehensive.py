...
class TestMemorySystemQA:
    ...
    def test_file_naming_new_format():
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            memory_manager = AgentMemoryManager(
                config=self.config, 
                working_directory=self.test_project_dir
            )
            ...
            assert expected_file.exists(), f"Memory file should be created as {agent_id}_memories.md"
            ...
            assert not old_format_file.exists(), f"Should not create old format {agent_id}_agent.md"
            assert not intermediate_file.exists(), f"Should not create intermediate format {agent_id}.md"
    ...
    def test_file_naming_no_agent_suffix():
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            for agent_id in agent_types:
                ...
                assert not old_format_file.exists(), f"Should not create {agent_id}_agent.md"
                assert not intermediate_file.exists(), f"Should not create {agent_id}.md"
    ...
    def test_user_level_memories_creation():
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            assert memory_manager.user_memories_dir.exists(), "User memories directory should be created"
            assert memory_manager.user_memories_dir == self.user_memories_dir, "User memories dir should point to correct location"
            ...
            assert "User-level memories" in readme_content, "README should mention user-level memories"
            assert "global defaults" in readme_content, "README should mention global defaults"
...
"""
            ...
            assert "type hints in Python code" in loaded_memory, "Should load user memory content"
            assert "composition over inheritance" in loaded_memory, "Should contain user guidelines"
...
"""
            ...
            assert "type hints in Python code" in aggregated_memory, "Should include user guidelines"
            assert "composition over inheritance" in aggregated_memory, "Should include user patterns"
            
            # Verify project content is included
            assert "FastAPI for REST APIs" in aggregated_memory, "Should include project guidelines"
            assert "microservices architecture" in aggregated_memory, "Should include project architecture"
            assert "PostgreSQL" in aggregated_memory, "Should include project tech stack"
            
            # Verify project overrides user for duplicates
            assert "Avoid hardcoded configuration" in aggregated_memory, "Should include project-specific mistakes"
            
            # Verify both unique items in Common Mistakes are preserved
            error_handling_count = aggregated_memory.count("Don't ignore error handling")
            assert error_handling_count == 1, "Should not duplicate identical items"
            
            # Verify aggregation metadata
            assert "Aggregated from user-level and project-level memories" in aggregated_memory, "Should indicate aggregation"
...
"""
            ...
            assert old_format_file.exists(), "Old format file should exist before migration"
            ...
            assert not old_format_file.exists(), "Old format file should be removed after migration"
            
            # Verify content was preserved
            assert "Legacy system with old naming" in loaded_memory, "Content should be preserved during migration"
            assert "Research patterns" in loaded_memory, "All content should be migrated"
...
"""
            ...
            assert not old_user_file.exists(), "Old format file should be removed from user dir"
            
            # Verify content
            assert "Write clear documentation" in loaded_memory, "User content should be preserved"
...
"""
            ...
            assert "Unit tests for all functions" in aggregated, "User content should be preserved"
            assert "Integration tests for workflows" in aggregated, "User strategies should be included"
            
            # Verify project overrides work
            assert "pytest-django for this project" in aggregated, "Project should override user guidelines"
            assert "Django framework" in aggregated, "Project architecture should be included"
            
            # Verify project extends user content
            assert "Load testing with Locust" in aggregated, "Project should extend testing strategies"
            assert "E2E testing with Selenium" in aggregated, "All project strategies should be included"
...
"""
            ...
            with patch('pathlib.Path.cwd', return_value=self.test_project_dir):
                ...
                assert "Current PM Memories" in framework_instructions, "Should include PM memories section"
                ...
                assert "delegate" in instructions_lower, "Should include delegation concepts"
...
"""
            ...
            assert "Infrastructure as Code" in aggregated, "Should include migrated content"
            assert "version control" in aggregated, "Should include user content"
            assert "principle of least privilege" in aggregated, "Should include user security guidelines"
            
            # 6. Test adding new learning
            success = memory_manager.add_learning(agent_id, "pattern", "Use blue-green deployments for zero downtime")
            ...
            assert "blue-green deployments" in updated_memory, "Should include new learning"
    ...
    def test_memory_file_format_consistency():
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            for agent_id in agents:
                ...
                old_file = self.project_memories_dir / f"{agent_id}_agent.md"
                assert not old_file.exists(), f"Old format file should not exist for {agent_id}"
                
                # Verify content structure
                assert f"# {agent_id.capitalize()} Agent Memory" in memory_content, f"Should have proper header for {agent_id}"
                assert "## Project Architecture" in memory_content, f"Should have architecture section for {agent_id}"
                assert "## Implementation Guidelines" in memory_content, f"Should have guidelines section for {agent_id}"
    ...
    def test_error_handling_and_fallbacks():
        ...
        with patch('pathlib.Path.home', return_value=self.test_user_home):
            ...
            assert memory is not None, "Should return default memory for non-existent agent"
...
def run_memory_system_qa_tests():
    ...
    if exit_code == 0:
        ...
        print("✓ Automatic migration from old {agent_name}_agent.md and {agent_name}.md formats")
