...
class AgentsCommand(AgentCommand):
    ...
    def listing_service(self):
        ...
        if self._listing_service is None:
            self._listing_service = AgentListingService(deployment_service=self.deployment_service)
    ...
    def cleanup_service(self):
        ...
        if self._cleanup_service is None:
            self._cleanup_service = AgentCleanupService(deployment_service=self.deployment_service)
        ...
        try:
            ...
            if output_format in ["json", "yaml"]:
                ...
                if agent_versions:
                    ...
                    formatted = self._formatter.format_as_json(data) if output_format == "json" else self._formatter.format_as_yaml(data)
                    print(formatted)
    ...
    def _list_deployed_agents(self, args) -> CommandResult:
        ...
        try:
            verbose = getattr(args, "verbose", False)
            agents, warnings = self.listing_service.list_deployed_agents(verbose=verbose)
    ...
    def _list_agents_by_tier(self, args) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.success_result("Agents listed by tier", data=agents_by_tier)
    ...
    def _deploy_agents(self, args, force=False) -> CommandResult:
        ...
        try:
            ...
            # Combine results
            combined_result = {
                "deployed_count": system_result.get("deployed_count", 0) + project_result.get("deployed_count", 0),
                "deployed": system_result.get("deployed", []) + project_result.get("deployed", []),
                "updated_count": system_result.get("updated_count", 0) + project_result.get("updated_count", 0),
                "updated": system_result.get("updated", []) + project_result.get("updated", []),
                "skipped": system_result.get("skipped", []) + project_result.get("skipped", []),
                "errors": system_result.get("errors", []) + project_result.get("errors", []),
                "target_dir": system_result.get("target_dir") or project_result.get("target_dir"),
            }
    ...
    def _fix_agents(self, args) -> CommandResult:
        ...
        try:
            ...
            if fix_all:
                ...
                if output_format in ["json", "yaml"]:
                    formatted = self._formatter.format_as_json(result) if output_format == "json" else self._formatter.format_as_yaml(result)
                    print(formatted)
                else:
                    # Text output
                        ...
                        for agent_result in result["results"]:
                            ...
                            if agent_result.get("skipped"):
                                ...
                                if dry_run:
                                    ...
                                    if agent_result.get("corrections_made", 0) > 0:
                    ...
                    if dry_run:
                        print(f"  Issues that would be fixed: {result.get('total_corrections_available', 0)}")
                ...
                # Fix specific agent
                result = self.validation_service.fix_agent_frontmatter(agent_name, dry_run=dry_run)
                
                if not result.get("success"):
                    return CommandResult.error_result(result.get("error", "Failed to fix agent"))
    ...
    def _check_agent_dependencies(self, args) -> CommandResult:
        ...
        try:
            ...
            if not result["success"]:
                ...
                return CommandResult.error_result(result.get("error", "Dependency check failed"))
            
            # Print the formatted report
            print(result["report"])
            
            return CommandResult.success_result("Dependency check completed", data=result)
        ...
        try:
            ...
            if not result["success"]:
                if "available_agents" in result:
                    print(f"❌ Agent '{agent_name}' is not deployed")
                    print(f"   Available agents: {', '.join(result['available_agents'])}")
                return CommandResult.error_result(result.get("error", "Installation failed"))
            ...
            if result.get("missing_count") == 0:
                ...
                print(f"✅ Successfully installed {len(result.get('installed', []))} dependencies")
                if result.get("still_missing"):
                    print(f"⚠️  {len(result['still_missing'])} dependencies still missing after installation")
    ...
    def _list_agent_dependencies(self, args) -> CommandResult:
        ...
        try:
            output_format = getattr(args, "format", "text")
            result = self.dependency_service.list_dependencies(format_type=output_format)
            ...
            if output_format == "pip":
                ...
                if result["python_dependencies"]:
                    print(f"Python Dependencies ({len(result['python_dependencies'])}):")
                    print("-" * 30)
                ...
                if result["system_dependencies"]:
                    print(f"System Dependencies ({len(result['system_dependencies'])}):")
                    print("-" * 30)
        ...
        try:
            ...
            if result.get("fixed_python") or result.get("failed_python"):
                ...
                if result.get("fixed_python"):
                    print(f"✅ Successfully installed: {len(result['fixed_python'])} packages")
                
                if result.get("failed_python"):
                    print(f"❌ Failed to install: {len(result['failed_python'])} packages")
