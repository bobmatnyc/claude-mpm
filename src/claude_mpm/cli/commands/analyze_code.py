...
class AnalyzeCodeCommand(BaseCommand):
            ...
            if not args.no_tree:
                ...
                if args.output == 'tree':
                    ...
                    return CommandResult(
                        success=True,
                        message="File tree analysis complete"
                    )
    ...
    def _build_file_tree(self, path: Path, ignore_patterns: Optional[List[str]], 
    ...
    def _analyze_code(self, path: Path, languages: Optional[List[str]],
                     ignore_patterns: Optional[List[str]], max_depth: Optional[int],
                     emit_events: bool, no_cache: bool) -> dict:
        ...
        if RICH_AVAILABLE and self.console:
            ...
            if emit_events:
                self.console.print("[yellow]→ Emitting real-time events to dashboard[/yellow]")
    ...
    def _build_rich_tree(self, rich_tree: Tree, node: dict, depth: int = 0, max_depth: int = 10):
    ...
    def _display_summary(self, result: dict):
        ...
        if result.get('tree'):
            ...
            dirs = sum(1 for c in tree_data.get('children', []) if c['type'] == 'directory')
        ...
        if result.get('nodes'):
            ...
            no_docs = [n for n in nodes if n.node_type in ('class', 'function', 'method') 
