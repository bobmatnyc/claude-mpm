...
def manage_debug(args):
    ...
    elif args.debug_command == "events":
        # Alias for socketio
        return debug_socketio(args, logger)
    elif args.debug_command == "connections":
        return debug_connections(args, logger)
    elif args.debug_command == "services":
        return debug_services(args, logger)
    elif args.debug_command == "agents":
        return debug_agents(args, logger)
    elif args.debug_command == "hooks":
        return debug_hooks(args, logger)
    elif args.debug_command == "cache":
...
def debug_socketio(args, logger):
    ...
    """
    try:
        from ...tools.socketio_debug import SocketIODebugger, DisplayMode
        ...
        print("❌ Debugging tool not available. Please ensure all dependencies are installed.")
        return 1
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    if result == 0:
                        ...
                        print(f"   Status: ⚠️ Not responding (may be stale)")
            except Exception as e:
...
def debug_services(args, logger):
    ...
    try:
        ...
        from ...core.container import DIContainer, ServiceRegistration
        ...
        if args.list:
            ...
            if not services:
                ...
                for service_type, registration in services.items():
                    ...
                    impl_name = registration.implementation.__name__ if hasattr(registration.implementation, '__name__') else str(registration.implementation)
                    print(f"\n🔧 {service_name}")
                    print(f"   Implementation: {impl_name}")
                    print(f"   Lifetime: {registration.lifetime.value}")
                    if registration.instance:
                        print(f"   Status: ✅ Instantiated")
                    else:
                        print(f"   Status: ⏸️ Not instantiated")
                ...
                for service_type, registration in services.items():
                    ...
                    if registration.dependencies:
                        for dep_name, dep_type in registration.dependencies.items():
                            dep_type_name = dep_type.__name__ if hasattr(dep_type, '__name__') else str(dep_type)
                ...
                if location.exists():
                    ...
                    if agent_files:
                        for agent_file in agent_files:
                            ...
                            modified = datetime.fromtimestamp(agent_file.stat().st_mtime)
                            print(f"   • {agent_name}")
                ...
                if memory_dir.exists():
                    ...
                    for mem_file in memory_files:
                        ...
                        try:
                            ...
                            entry_count = len(memory_data) if isinstance(memory_data, list) else 1
                            print(f"\n📝 Agent: {agent_name}")
                            ...
                            if args.verbose and isinstance(memory_data, list) and memory_data:
...
def debug_hooks(args, logger):
    ...
    try:
        ...
        if args.list:
            ...
            # Show statistics
            print("\n📊 Hook Statistics:")
            print(f"   Pre-delegation executed: {hook_service.stats.get('pre_delegation_executed', 0)}")
            print(f"   Post-delegation executed: {hook_service.stats.get('post_delegation_executed', 0)}")
...
def debug_cache(args, logger):
    ...
    try:
        ...
        if args.inspect:
            ...
            if args.confirm or input(f"Clear {file_count} cache files? (y/N): ").lower() == 'y':
                # Clear cache
                import shutil
                try:
            ...
            if cache_manager._capabilities_cache:
                ...
                status = "✅ Valid" if age < cache_manager.capabilities_ttl else "❌ Expired"
                print(f"   Capabilities: {status} (age: {age:.1f}s)")
            else:
                print(f"   Capabilities: ⚫ Empty")
            ...
            if cache_manager._deployed_agents_cache:
                ...
                status = "✅ Valid" if age < cache_manager.deployed_agents_ttl else "❌ Expired"
                print(f"   Deployed Agents: {status} (age: {age:.1f}s)")
            else:
                print(f"   Deployed Agents: ⚫ Empty")
    ...
    try:
        ...
        if args.profile:
            ...
            if operation in operations:
                ...
                print(f"   Available: agent_load, service_init, cache_ops, memory_ops")
...
def add_debug_parser(subparsers):
    ...
    display_group.add_argument(
        "--summary",
        action="store_true",
        help="Show aggregated statistics and summary"
    )
    display_group.add_argument(
        "--raw",
        action="store_true",
    ...
    display_group.add_argument(
        "--pretty",
        action="store_true",
        help="Enhanced formatted output with colors"
    )
    ...
    socketio_parser.add_argument(
        "--output",
        "-o",
        help="Save events to file (JSONL format)"
    )
    socketio_parser.add_argument(
        "--quiet",
        "-q",
