...
SKIP_DIRS = {
    ...
    "dist", "build", ".tox", "htmlcov", ".pytest_cache",
    ".mypy_cache", "coverage", ".idea", ".vscode", "env",
    ".coverage", "__MACOSX", ".ipynb_checkpoints"
}

# Dotfiles that are allowed to be shown
DOTFILE_EXCEPTIONS = {
    ".env.example", ".env.sample", ".gitlab-ci.yml",
    ".travis.yml", ".dockerignore", ".editorconfig",
...
def should_show_item(item_name, item_path, is_directory):
    ...
    if is_directory:
        ...
        if base_name in {"makefile", "dockerfile", "jenkinsfile", "rakefile", "gemfile"}:
            return True
        ...
        try:
            ...
            for item in items:
                ...
                result['contents'].append({
                    ...
                    'is_code_file': not is_directory and any(
                        item.endswith(ext) for ext in CODE_EXTENSIONS
                    )
                })
            ...
            code_file_count = sum(1 for x in result['contents'] if x.get('is_code_file', False))
