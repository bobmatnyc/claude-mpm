...
Provides comprehensive diagnostics for troubleshooting MCP server issues.
...
def diagnose():
    ...
    critical_files = [
        ...
        ("Wrapper Module", project_root / "src" / "claude_mpm" / "mcp" / "wrapper.py"),
        (
            "Server Script",
    ...
    for name, path in critical_files:
        if path.exists():
            ...
            print(
                f"  ✓ {name}: {path.relative_to(project_root) if path.is_relative_to(project_root) else path} ({size} bytes)"
            )
        else:
            ...
            if "mcpServers" in config:
                ...
                for server_name, server_config in config["mcpServers"].items():
                    ...
                    if server_name == "claude-mpm-gateway":
                        ...
                        if args and (
                            "mcp_wrapper.py" in str(args) or "wrapper.py" in str(args)
                        ):
                            print("    ✓ Using wrapper script (recommended)")
                        elif args and "mcp_server.py" in str(args):
                            print(
                                "    ⚠ Using legacy server script (consider updating to wrapper)"
                            )
                        else:
                ...
                wrapper_module = (
                    project_root / "src" / "claude_mpm" / "mcp" / "wrapper.py"
                )
    ...
    wrapper_path = project_root / "src" / "claude_mpm" / "mcp" / "wrapper.py"
    if wrapper_path.exists():
        print(f"Testing wrapper module: {wrapper_path}")
        ...
        print(f"  ✗ Wrapper module not found at {wrapper_path}")
    ...
    if issues:
        ...
        print("  3. Try running the wrapper module manually to see errors:")
...
def main():
    """Entry point for the diagnostics tool."""
    diagnose()
