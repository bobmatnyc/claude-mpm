...
'''
    ...
    def install_hooks(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if hook_script_path.exists() and not force:
                self.logger.info("Hook script already exists. Use --force to overwrite.")
            else:
    ...
    def _install_commands(self) -> None:
        ...
        if not commands_src.exists():
            self.logger.debug("No commands directory found, skipping command installation")
            return
                ...
                if "hooks" not in settings:
                    ...
                    for event in required_events:
                        if event not in settings["hooks"]:
                            issues.append(f"Missing hook configuration for {event} event")
                    ...
                    for event_type in list(settings["hooks"].keys()):
                        ...
                        filtered_hooks = [
                            ...
                            if not (isinstance(h, dict) and 
                                   h.get("hooks", [{}])[0].get("command", "").endswith("claude-mpm-hook.sh"))
                        ]
    ...
    def get_status(self) -> Dict[str, any]:
        ...
        status = {
            ...
            "settings_file": str(self.settings_file) if self.settings_file.exists() else None,
        }
