...
class SocketIODebugger:
    ...
    def _display_event(self, data: Dict[str, Any]):
        ...
        if self.show_raw or self.mode == DisplayMode.RAW:
            ...
            print("-" * 50)
        elif self.mode == DisplayMode.PRETTY or (self.mode == DisplayMode.LIVE and RICH_AVAILABLE):
    ...
    def _display_pretty(self, timestamp: str, event_type: str, event_data: Dict[str, Any]):
    ...
    def _display_basic(self, timestamp: str, event_type: str, event_data: Dict[str, Any]):
    ...
    def _display_summary_rich(self):
        ...
        for event_type, count in sorted(self.stats.event_types.items(), key=lambda x: x[1], reverse=True)[:10]:
        ...
        if self.stats.tool_usage:
            ...
            for tool, count in sorted(self.stats.tool_usage.items(), key=lambda x: x[1], reverse=True)[:10]:
