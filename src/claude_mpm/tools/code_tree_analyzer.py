...
class PythonAnalyzer:
    ...
    def _extract_nodes(self, tree: ast.AST, file_path: Path, 
                      source: str) -> List[CodeNode]:
        ...
        class NodeVisitor(ast.NodeVisitor):
            ...
            def visit_ClassDef(self, node):
                ...
                if self.emitter:
                    self.emitter.emit_node(CodeNodeEvent(
                        file_path=str(file_path),
                        node_type="class",
                        name=node.name,
                        line_start=node.lineno,
...
class MultiLanguageAnalyzer:
    ...
    def _init_parsers(self):
        ...
        if not TREE_SITTER_AVAILABLE:
            self.logger.warning("tree-sitter not available - multi-language support disabled")
            return
        ...
        def walk_tree(node, parent_name=None):
            if node.type == 'class_declaration':
                ...
                if name_node:
                    ...
                    func_node = CodeNode(
                        file_path=str(file_path),
                        node_type="function" if node.type != 'method_definition' else "method",
                        name=func_name,
                        line_start=node.start_point[0] + 1,
                        line_end=node.end_point[0] + 1,
                        parent=parent_name,
    ...
    def _extract_generic_nodes(self, tree, file_path: Path, source: bytes, 
                               language: str) -> List[CodeNode]:
...
class CodeTreeAnalyzer:
    ...
    def analyze_directory(self, directory: Path, 
        ...
        stats = {
            ...
            "functions": sum(1 for n in all_nodes if n.node_type in ("function", "method")),
            "imports": sum(1 for n in all_nodes if n.node_type == "import"),
            "languages": list(set(n.language for n in all_nodes if hasattr(n, 'language'))),
            "avg_complexity": sum(n.complexity for n in all_nodes) / len(all_nodes) if all_nodes else 0
        }
