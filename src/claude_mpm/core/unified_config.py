...
class NetworkConfig(BaseModel):
    ...
    socketio_port: int = Field(default=8765, ge=1024, le=65535, description="SocketIO server port")
    socketio_port_range: List[int] = Field(default=[8765, 8775], description="Port range for SocketIO")
...
class LoggingConfig(BaseModel):
    ...
    retention_days: int = Field(default=30, ge=1, description="Log retention period in days")
...
class AgentConfig(BaseModel):
    ...
    auto_discover: bool = Field(default=True, description="Enable automatic agent discovery")
    precedence: List[str] = Field(default=["project", "user", "system"], description="Agent precedence order")
    enable_hot_reload: bool = Field(default=True, description="Enable hot reloading of agents")
    cache_ttl_seconds: int = Field(default=3600, ge=0, description="Agent cache TTL in seconds")
    ...
    max_concurrent_operations: int = Field(default=10, ge=1, description="Max concurrent agent operations")
...
class MemoryConfig(BaseModel):
    ...
    auto_learning: bool = Field(default=True, description="Enable automatic learning extraction")
    default_size_kb: int = Field(default=80, ge=1, description="Default memory file size limit in KB")
    max_sections: int = Field(default=10, ge=1, description="Maximum sections per memory file")
    max_items_per_section: int = Field(default=15, ge=1, description="Maximum items per section")
    max_line_length: int = Field(default=120, ge=1, description="Maximum line length")
    claude_json_warning_threshold_kb: int = Field(default=500, ge=1, description="Warning threshold for Claude JSON size")
...
class SecurityConfig(BaseModel):
    ...
    max_file_size_mb: int = Field(default=10, ge=1, description="Maximum file size for operations")
    allowed_file_extensions: List[str] = Field(
        default=[".md", ".txt", ".json", ".yaml", ".yml", ".py"],
        description="Allowed file extensions"
    )
    enable_sandbox_mode: bool = Field(default=False, description="Enable sandbox mode for agents")


class PerformanceConfig(BaseModel):
    """Performance and resource configuration."""

    startup_timeout: int = Field(default=60, ge=1, description="Service startup timeout in seconds")
    graceful_shutdown_timeout: int = Field(default=30, ge=1, description="Graceful shutdown timeout")
    max_memory_usage_mb: int = Field(default=1024, ge=128, description="Maximum memory usage in MB")
    ...
    hook_timeout_seconds: int = Field(default=5, ge=1, description="Hook execution timeout")
    session_timeout_seconds: int = Field(default=30, ge=1, description="Session timeout")
    agent_load_timeout_seconds: int = Field(default=10, ge=1, description="Agent loading timeout")

    # Retry and recovery settings
    max_restarts: int = Field(default=3, ge=0, description="Maximum service restarts")
    max_recovery_attempts: int = Field(default=3, ge=0, description="Maximum recovery attempts")

    # Cache settings
    cache_max_size_mb: float = Field(default=100, ge=1, description="Cache maximum size in MB")
    cache_max_entries: int = Field(default=10000, ge=1, description="Cache maximum entries")
    cache_default_ttl_seconds: int = Field(default=300, ge=1, description="Cache default TTL")

    # Sleep and polling intervals
    health_check_interval_seconds: float = Field(default=0.1, ge=0.01, description="Health check interval")
    batch_window_ms: int = Field(default=100, ge=1, description="Batch processing window in milliseconds")
    polling_interval_seconds: float = Field(default=1.0, ge=0.1, description="General polling interval")


class SessionConfig(BaseModel):
    """Session management configuration."""

    max_age_minutes: int = Field(default=30, ge=1, description="Session maximum age in minutes")
    cleanup_max_age_hours: int = Field(default=24, ge=1, description="Session cleanup age in hours")
    archive_old_sessions: bool = Field(default=True, description="Archive old sessions before cleanup")
    session_timeout_minutes: int = Field(default=60, ge=1, description="Session timeout in minutes")
...
class DevelopmentConfig(BaseModel):
    ...
    hot_reload_enabled: bool = Field(default=True, description="Enable hot reloading in development")
...
class UnifiedConfig(BaseSettings):
    ...
    environment: str = Field(default="production", description="Environment (development, testing, production)")
    ...
    # Path configuration
    base_path: Optional[Path] = Field(default=None, description="Base path for Claude MPM")
    config_path: Optional[Path] = Field(default=None, description="Configuration file path")
    
    # Additional settings for backward compatibility
    extra_settings: Dict[str, Any] = Field(default_factory=dict, description="Additional settings")
    ...
    def get_nested(self, key: str, default: Any = None) -> Any:
        ...
        try:
            ...
            for k in keys:
                ...
                elif hasattr(value, '__getitem__') and hasattr(value, '__contains__') and k in value:
                    value = value[k]
                else:
                    return default
            return value
