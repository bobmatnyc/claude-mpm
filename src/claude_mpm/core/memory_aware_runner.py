...
class MemoryAwareClaudeRunner(ClaudeRunner):
    ...
    def __init__(
        ...
        self.max_restarts = ConfigConstants.get_recovery_setting("max_restarts")

        self.logger.info(
            f"Memory-aware runner initialized with monitoring: {enable_monitoring}"
        )
    ...
    def run_interactive_with_monitoring(
        ...
        if not self.enable_monitoring:
            ...
            self.logger.info(
                "Memory monitoring disabled, using standard interactive mode"
            )
        ...
        try:
            asyncio.run(
                self._run_with_monitoring_async(
                    initial_context, enable_state_preservation
                )
            )
    ...
    async def _run_with_monitoring_async(
        self, initial_context: Optional[str], enable_state_preservation: bool
    ):
    ...
    async def _perform_controlled_restart(self):
        ...
        self.logger.info(
            f"Performing controlled restart (attempt {self.restart_count + 1}/{self.max_restarts})"
        )
    ...
    def _display_monitoring_info(self):
        ...
        print(f"Monitoring:")
        print(
            f"  • Check Interval: {self.memory_config.monitoring.normal_interval} seconds"
        )
        print(f"  • Max Restarts:   {self.max_restarts}")
        print(
            f"  • State Preservation: {'Enabled' if self.state_manager else 'Disabled'}"
        )
        print("=" * 60 + "\n")
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        if not self.memory_guardian:
            return {"enabled": False, "message": "Memory monitoring not initialized"}
