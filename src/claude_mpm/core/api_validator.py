...
class APIKeyValidator:
    ...
    def validate_all_keys(self, strict: bool = True) -> Tuple[bool, List[str], List[str]]:
    ...
    def _validate_openai_key(self, api_key: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.errors.append("❌ OpenAI API key lacks required permissions (403 Forbidden)")
                return False
                ...
                self.warnings.append(f"⚠️ OpenAI API returned unexpected status: {response.status_code}")
                return True  # Assume valid for unexpected status codes
                
        except requests.exceptions.Timeout:
            self.warnings.append("⚠️ OpenAI API validation timed out - assuming key is valid")
            return True
    ...
    def _validate_anthropic_key(self, api_key: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.errors.append("❌ Anthropic API key lacks required permissions (403 Forbidden)")
                return False
                ...
                self.warnings.append(f"⚠️ Anthropic API returned unexpected status: {response.status_code}")
                return True
                
        except requests.exceptions.Timeout:
            self.warnings.append("⚠️ Anthropic API validation timed out - assuming key is valid")
            return True
        except requests.exceptions.ConnectionError as e:
    ...
    def _validate_github_token(self, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.warnings.append(f"⚠️ GitHub API returned unexpected status: {response.status_code}")
                return True
                
        except requests.exceptions.Timeout:
            self.warnings.append("⚠️ GitHub API validation timed out - assuming token is valid")
            return True
        except requests.exceptions.ConnectionError as e:
    ...
    def _validate_custom_api(self, api_name: str, validation_config: Dict) -> bool:
        ...
        try:
            ...
            if method == 'GET':
                ...
                self.warnings.append(f"⚠️ Unsupported validation method for {api_name}: {method}")
                return True
            ...
            if response.status_code in valid_status_codes:
                ...
                self.errors.append(f"❌ {api_name} API key is invalid (401 Unauthorized)")
                return False
            elif response.status_code == 403:
                self.errors.append(f"❌ {api_name} API key lacks permissions (403 Forbidden)")
                return False
            else:
