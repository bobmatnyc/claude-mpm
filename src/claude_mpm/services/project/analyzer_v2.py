...
class ProjectAnalyzer(ProjectAnalyzerInterface):
    ...
    def __init__(
        ...
        self.architecture_analyzer = architecture_analyzer or ArchitectureAnalyzerService(
            self.working_directory
        )
    ...
    def _analyze_with_language_service(self, characteristics: ProjectCharacteristics) -> None:
        """Delegate language analysis to specialized service."""
        try:
            characteristics.languages = self.language_analyzer.detect_languages()
            characteristics.primary_language = self.language_analyzer.detect_primary_language()
            characteristics.frameworks = self.language_analyzer.detect_frameworks()
            characteristics.code_conventions = self.language_analyzer.analyze_code_style()
        except Exception as e:
            self.logger.warning(f"Error in language analysis: {e}")
            
    def _analyze_with_dependency_service(self, characteristics: ProjectCharacteristics) -> None:
        """Delegate dependency analysis to specialized service."""
        try:
            characteristics.package_manager = self.dependency_analyzer.detect_package_manager()
    ...
    def _analyze_with_architecture_service(self, characteristics: ProjectCharacteristics) -> None:
    ...
    def _enrich_with_metrics(self, characteristics: ProjectCharacteristics) -> None:
        ...
        try:
            ...
            if metrics.test_to_code_ratio > 0:
                ...
                characteristics.test_patterns.append(f"{ratio_pct}% test coverage ratio")
    ...
    def _ensure_backward_compatibility(self, characteristics: ProjectCharacteristics) -> None:
    ...
    def _infer_architecture_type(self, characteristics: ProjectCharacteristics) -> None:
        ...
        pass  # Architecture type is set by architecture service
        
    def _extract_project_terminology(self, characteristics: ProjectCharacteristics) -> None:
        """Backward compatibility method - already handled by architecture service."""
        pass  # Terminology is extracted by architecture service
    ...
    def _parse_package_json(self, package_path: Path, characteristics: ProjectCharacteristics) -> None:
        ...
        characteristics.web_frameworks = self.dependency_analyzer.detect_web_frameworks(deps["production"])
        characteristics.build_tools = self.dependency_analyzer.get_build_tools()
