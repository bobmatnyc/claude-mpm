...
class ArchitectureAnalyzerService:
    ...
    ENTRY_POINT_PATTERNS = {
        "python": ["main.py", "app.py", "server.py", "run.py", "__main__.py", "wsgi.py"],
    ...
    def _analyze_directory_patterns(self, info: ArchitectureInfo) -> None:
        ...
        important_dirs = [
            ...
            "views", "controllers", "routes", "api", "tests", "docs",
            "config", "utils", "helpers", "core", "modules", "packages"
        ]
        
        info.key_directories = [
            d for d in important_dirs
            if (self.working_directory / d).exists()
        ]
        ...
        for key_dir in ["src", "lib", "app"]:
            ...
            if key_path.exists() and key_path.is_dir():
                subdirs = [
                    d.name for d in key_path.iterdir()
                ...
                if "*" in pattern:
                    ...
                    for base in ["", "src/", "app/", "lib/"]:
                        ...
                        if entry_path.exists() and entry_path.is_file():
                            rel_path = str(entry_path.relative_to(self.working_directory))
                            entry_points.append(rel_path)
    ...
    def _detect_api_patterns(self, info: ArchitectureInfo) -> None:
        ...
        for file_path in source_files:
            try:
                ...
                # REST API patterns
                if any(p in content for p in ["@app.route", "@router.", "@Get(", "@Post("]):
    ...
    def _extract_terminology(self, info: ArchitectureInfo) -> None:
        ...
        common_words = {
            "src", "lib", "app", "main", "test", "tests", "docs",
            "config", "utils", "helpers", "core", "base", "common",
            "shared", "public", "private", "static", "assets", "build",
            "dist", "node", "modules", "vendor", "bin", "obj"
        }
