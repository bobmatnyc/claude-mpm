...
class MetricsCollectorService:
    ...
    CODE_EXTENSIONS = {
        ...
        ".go", ".rs", ".rb", ".php", ".swift", ".kt", ".scala", ".r", ".lua"
    }
    ...
    EXCLUDE_DIRS = {
        ...
        "dist", "build", "target", ".venv", "venv", "env", ".tox",
    ...
    def analyze_file_sizes(self) -> Dict[str, List[Tuple[str, int]]]:
        ...
        for file_path in self._iter_code_files():
            try:
                ...
                file_sizes.append((str(file_path.relative_to(self.working_directory)), lines))
        ...
        for ext, patterns in comment_patterns.items():
            ...
            for file_path in files[:20]:  # Sample files
                try:
                    ...
                    for line in lines:
                        stripped = line.strip()
                        if any(stripped.startswith(p) for p in patterns if isinstance(p, str)):
                            comment_lines += 1
    ...
    def _collect_file_metrics(self, metrics: ProjectMetrics) -> None:
        ...
        for file_path in self._iter_code_files():
            try:
                ...
                file_sizes.append((str(file_path.relative_to(self.working_directory)), file_line_count))
