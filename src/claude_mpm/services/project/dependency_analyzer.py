...
class DependencyAnalyzerService:
    ...
    def detect_package_manager(self) -> Optional[str]:
        ...
        lock_files = ["yarn.lock", "pnpm-lock.yaml", "package-lock.json", 
    ...
    def detect_testing_frameworks(self, dependencies: Optional[List[str]] = None) -> List[str]:
    ...
    def detect_web_frameworks(self, dependencies: List[str]) -> List[str]:
        ...
        web_framework_names = [
            ...
            "express", "koa", "hapi", "fastify", "nestjs",  # JavaScript
            "rails", "sinatra", "hanami",  # Ruby
            "laravel", "symfony", "slim", "lumen",  # PHP
            "spring", "springboot", "struts", "play",  # Java
    ...
    def get_build_tools(self, package_data: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        if package_data and "scripts" in package_data:
            ...
            # Common build tools
            build_tool_names = ["webpack", "rollup", "vite", "parcel", "esbuild",
                              "browserify", "gulp", "grunt", "make", "cmake"]
    ...
    def _analyze_package_json(self, path: Path, dependencies: Dict[str, List[str]]) -> None:
    ...
    def _analyze_python_deps(self, path: Path, dependencies: Dict[str, List[str]]) -> None:
        ...
        try:
            ...
            for dep_list in [dependencies["production"], dependencies["development"]]:
                for dep in dep_list:
                    ...
                    # Check testing
                    if any(test in dep_lower for test in self.TESTING_PACKAGES["python"]):
                        dependencies["testing"].append(dep)
                        
        except Exception as e:
            self.logger.warning(f"Error parsing Python dependencies: {e}")
            
    def _analyze_cargo_toml(self, path: Path, dependencies: Dict[str, List[str]]) -> None:
