...
"""
    ...
    def _cleanup_archives(self, original_filename: str) -> None:
        ...
        cutoff_compress = datetime.now(UTC) - timedelta(days=self.COMPRESS_AFTER_DAYS)
        ...
        cutoff_delete = datetime.now(UTC) - timedelta(days=self.DELETE_AFTER_DAYS)
        for archive in archives:
            mtime = datetime.fromtimestamp(archive.stat().st_mtime, tz=UTC)
    ...
    def list_archives(
        self, filename: str | None = None, include_metadata: bool = False
    ) -> list[dict]:
    ...
    def get_latest_archive(self, filename: str) -> Path | None:
    ...
    def restore_archive(
        self, archive_name: str, target_path: Path | None = None
    ) -> tuple[bool, str]:
    ...
    def get_file_last_modified(self, file_path: Path) -> datetime | None:
    ...
    def generate_documentation_diff_report(
        self, file1: Path, file2: Path | None = None
    ) -> str:
