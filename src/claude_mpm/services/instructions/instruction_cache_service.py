...
class InstructionCacheService:
    ...
    def update_cache(
        ...
        try:
            ...
            # Prepend a stable, deterministic auto-generated banner.  The banner
            # is added at write time and excluded from the content hash so it
            # never causes spurious cache invalidation on repeated rebuilds.
            banner = (
                "<!-- AUTO-GENERATED by claude-mpm — DO NOT EDIT.\n"
                "     Role: assembled system-prompt cache"
                " (BASE_PM + PM_INSTRUCTIONS + WORKFLOW + capabilities).\n"
                "     Rebuilt automatically when content changes.\n"
                "     To customise, edit the source .md files in"
                " ~/.claude-mpm/ or .claude-mpm/ instead.\n"
                "     See docs/developer/instruction-files.md for full details. -->\n"
            )

            # Write content to cache (atomic operation)
            temp_file = self.cache_file.with_suffix(".tmp")
            temp_file.write_text(banner + instruction_content, encoding="utf-8")
