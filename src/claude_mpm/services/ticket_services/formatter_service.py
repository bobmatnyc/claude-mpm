...
class TicketFormatterService:
    ...
    def format_ticket_list(
        ...
        lines = [
            f"Tickets (page {page}, showing {total_shown} tickets):",
            "-" * 80
        ]
    ...
    def format_search_results(
        ...
        lines = [
            f"Search results for '{query}' (showing {len(tickets)}):",
            "-" * 80
        ]
    ...
    def format_operation_result(
        self, operation: str, ticket_id: str, success: bool, message: Optional[str] = None
    ) -> str:
        """
        ...
        if success:
            ...
            msg = message or default_messages.get(operation, f"Operation completed: {ticket_id}")
        else:
            emoji = "❌"
            default_messages = {
            ...
            msg = message or default_messages.get(operation, f"Operation failed: {ticket_id}")
