...
class MonitorClient:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "uptime": (
                (datetime.now() - datetime.fromisoformat(self.stats["last_connected"])).total_seconds()
                if self.stats["last_connected"] and self.connected else 0
            )
        }
    ...
    async def _handle_reconnection(self):
        ...
        self.logger.info(f"Attempting to reconnect in {self.reconnect_delay:.1f} seconds")
    ...
    def send_to_monitor(self, event_name: str, data: Any = None) -> bool:
        ...
        if not self.connected or not self.client:
            self.logger.warning(f"Cannot send event '{event_name}' - not connected to monitor")
            return False
