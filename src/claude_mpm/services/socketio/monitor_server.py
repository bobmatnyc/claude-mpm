...
class MonitorServer(SocketIOServiceInterface):
    ...
    def __init__(self, host: str = None, port: int = None):
        ...
        self.event_buffer_size = monitor_config.get("event_buffer_size", SystemLimits.MAX_EVENTS_BUFFER * 2)
        self.client_timeout = monitor_config.get("client_timeout", 60)
        self.health_monitoring_enabled = monitor_config.get("enable_health_monitoring", True)
    ...
    def start_sync(self):
        ...
        self.logger.info(f"Starting lightweight monitor server on {self.host}:{self.port}")
        ...
        if self.running:
            ...
            self.logger.info(f"Monitor server started successfully on {self.host}:{self.port}")
            return True
        else:
    ...
    async def _start_server(self):
        ...
        try:
            ...
            self.site = web.TCPSite(
                ...
                self.host, 
                self.port,
                shutdown_timeout=1.0  # Fast shutdown
            )
    ...
    async def _send_buffered_events(self, client_id: str):
        ...
        with self.buffer_lock:
            if self.event_buffer:
                self.logger.info(f"Sending {len(self.event_buffer)} buffered events to {client_id}")
                for event in list(self.event_buffer):
                    try:
                        await self.sio.emit(event['type'], event['data'], room=client_id)
                    except Exception as e:
                        self.logger.error(f"Error sending buffered event to {client_id}: {e}")
    ...
    def send_to_client(self, client_id: str, event_type: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            asyncio.run_coroutine_threadsafe(
                self.sio.emit(event_type, data, room=client_id),
                self.loop
            )
            return True
        except Exception as e:
    ...
    def claude_status_changed(self, status: str, pid: Optional[int] = None, message: str = ""):
        ...
        self.claude_pid = pid
        self.broadcast_event("claude_status", {
            "status": status,
            "pid": pid,
            "message": message
        })
    ...
    def claude_output(self, content: str, stream: str = "stdout"):
        """Relay Claude output."""
        self.broadcast_event("claude_output", {
            "content": content,
            "stream": stream
        })
    ...
    def agent_delegated(self, agent: str, task: str, status: str = "started"):
        ...
        self.broadcast_event("agent_delegated", {
            "agent": agent,
            "task": task,
            "status": status
        })
    ...
    def ticket_created(self, ticket_id: str, title: str, priority: str = "medium"):
        ...
        self.broadcast_event("ticket_created", {
            "ticket_id": ticket_id,
            "title": title,
            "priority": priority
        })
    ...
    def memory_loaded(self, agent_id: str, memory_size: int, sections_count: int):
        ...
        self.broadcast_event("memory_loaded", {
            "agent_id": agent_id,
    ...
    def memory_created(self, agent_id: str, template_type: str):
        ...
        self.broadcast_event("memory_created", {
            "agent_id": agent_id,
    ...
    def memory_updated(self, agent_id: str, learning_type: str, content: str, section: str):
        ...
        self.broadcast_event("memory_updated", {
            "agent_id": agent_id,
    ...
    def memory_injected(self, agent_id: str, context_size: int):
        ...
        self.broadcast_event("memory_injected", {
            "agent_id": agent_id,
