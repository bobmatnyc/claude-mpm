...
def find_dashboard_files() -> Optional[Path]:
    ...
    possible_locations = [
        ...
        Path(sys.prefix) / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages" / "claude_mpm" / "dashboard",
        # User site-packages
        Path.home() / ".local" / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages" / "claude_mpm" / "dashboard",
    ]
        ...
        elements = [
            {
                ...
                'methods': [
                    ...
                    {'name': 'mock_method', 'type': 'method', 'line': 15, 'complexity': 1}
                ]
            },
            {
                'name': 'mock_function',
        ...
        elements = [
            {
                ...
                'methods': [
                    ...
                    {'name': 'mockMethod', 'type': 'method', 'line': 10, 'complexity': 1}
                ]
            },
            {
                'name': 'mockFunction',
...
class StableDashboardServer:
    ...
    def setup(self) -> bool:
        ...
        if not DEPENDENCIES_AVAILABLE:
            print("❌ Error: Missing dependencies. Install with: pip install aiohttp python-socketio")
            return False
    ...
    def _setup_socketio_events(self):
        ...
        async def connect(sid, environ):
            ...
            await self.sio.emit('connection_test', {'status': 'connected', 'server': 'stable'}, room=sid)
        ...
        async def code_analyze_file(sid, data):
            print(f"📡 Received file analysis request from {sid}: {data.get('path', 'unknown')}")
        ...
        async def handle_code_analyze_file(sid, data):
            print(f"📡 Received code:analyze:file from {sid}: {data.get('path', 'unknown')}")
        ...
        async def get_git_branch(sid, data):
            ...
            await self.sio.emit('git_branch_response', {'branch': 'main', 'path': data}, room=sid)

        @self.sio.event
        async def request_status(sid, data):
            print(f"📡 Received status request from {sid}")
            await self.sio.emit('status_response', {'status': 'running', 'server': 'stable'}, room=sid)
    ...
    async def _serve_static(self, request):
        ...
        if static_file.exists() and static_file.is_file():
            content_type = 'text/javascript' if file_path.endswith('.js') else 'text/css' if file_path.endswith('.css') else 'text/plain'
            ...
            try:
                for item in sorted(os.listdir(abs_path)):
                    ...
                    result['contents'].append({
                        ...
                        'is_code_file': item.endswith(('.py', '.js', '.ts', '.jsx', '.tsx'))
                    })
...
def create_stable_server(dashboard_path: Optional[Path] = None, **kwargs) -> StableDashboardServer:
