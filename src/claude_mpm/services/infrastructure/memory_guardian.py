...
class MemoryStats:
    ...
    def update(self, memory_mb: float) -> None:
        ...
        if self.samples == 0:
            ...
            self.average_mb = ((self.average_mb * self.samples) + memory_mb) / (
                self.samples + 1
            )
...
class MemoryGuardian(BaseService):
    ...
    def __init__(self, config: Optional[MemoryGuardianConfig] = None):
        ...
        self.log_info(
            f"Memory Guardian initialized with thresholds: "
    ...
    async def restart_process(self, reason: str = "Manual restart") -> bool:
        ...
        attempt = RestartAttempt(
            timestamp=time.time(), reason=reason, memory_mb=memory_mb, success=success
        )
        ...
        if self.restart_protection:
            backoff_applied = (
                self.restart_protection.get_backoff_seconds(
                    self.restart_protection.statistics.consecutive_failures + 1
                )
                if not success
                else 0
    ...
    async def monitor_memory(self) -> None:
        ...
        if self.memory_state != old_state:
            self.log_info(
                f"Memory state changed: {old_state.value} -> {self.memory_state.value} "
        ...
        if self.memory_state == MemoryState.EMERGENCY:
            ...
            await self.restart_process(
                f"Emergency memory threshold exceeded ({memory_mb:.2f}MB)"
            )
            ...
            if self._should_restart_for_critical():
                await self.restart_process(
                    f"Sustained critical memory usage ({memory_mb:.2f}MB)"
                )
        ...
        if self.config.monitoring.log_memory_stats:
            if (
                time.time() - self.memory_stats.last_check
                > self.config.monitoring.log_interval
            ):
    ...
    def _can_restart(self) -> bool:
        ...
        recent_attempts = [
            a for a in self.restart_attempts if a.timestamp >= window_start
        ]
    ...
    def _should_restart_for_critical(self) -> bool:
        ...
        recent_samples = [
            s
            for s in self.restart_attempts
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            self.memory_stats.peak_mb = state.get("memory_stats", {}).get(
                "peak_mb", 0.0
            )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "restart_attempts": [
                a.to_dict() for a in self.restart_attempts[-10:]
            ],  # Last 10
    ...
    async def _initialize_safety_services(self) -> None:
        ...
        try:
            ...
            self.restart_protection = RestartProtection(
                ...
                state_file=Path(self.config.state_file).parent
                / "restart_protection.json"
            ...
            self.graceful_degradation = GracefulDegradation(
                ...
                state_file=Path(self.config.state_file).parent / "degradation.json"
                if self.config.state_file
                else None,
            )
            ...
            if not valid:
                ...
                await self.graceful_degradation.degrade_feature(
                    "automated_monitoring", message, "reduced monitoring frequency"
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "monitoring": {
                ...
                "check_interval": self.config.monitoring.get_check_interval(
                    self.memory_state.value
                ),
