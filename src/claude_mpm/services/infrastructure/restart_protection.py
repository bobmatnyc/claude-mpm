...
class RestartStatistics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                (self.successful_restarts / self.total_restarts * 100)
            ...
            "last_restart_iso": (
                datetime.fromtimestamp(self.last_restart_time).isoformat()
                if self.last_restart_time
                else None
            ),
            "average_interval_minutes": round(self.average_interval_minutes, 2),
            "shortest_interval_minutes": (
                round(self.shortest_interval_minutes, 2)
                if self.shortest_interval_minutes != float("inf")
                else None
            ),
...
class RestartProtection(BaseService):
    ...
    def should_allow_restart(self, current_memory_mb: float = 0) -> Tuple[bool, str]:
        ...
        if self.statistics.consecutive_failures >= self.max_consecutive_failures:
            return (
                False,
                f"Too many consecutive failures ({self.statistics.consecutive_failures})",
            )

        return True, "Restart allowed"

    def record_restart(
        self, reason: str, memory_mb: float, success: bool, backoff_applied: float = 0
    ) -> None:
    ...
    def _check_circuit_breaker(self) -> Tuple[bool, str]:
        ...
        if self.circuit_state == CircuitState.OPEN:
            ...
            if (
                self.circuit_test_allowed_at
                and time.time() >= self.circuit_test_allowed_at
            ):
                self._half_open_circuit_breaker()
                return True, "Circuit half-open, testing recovery"
            else:
                remaining = (
                    self.circuit_test_allowed_at - time.time()
                    if self.circuit_test_allowed_at
                    else 0
                )
    ...
    def _check_restart_frequency(self) -> Tuple[bool, str]:
        ...
        recent_restarts = [r for r in self.restart_history if r.timestamp >= hour_ago]

        if len(recent_restarts) >= self.max_restarts_per_hour:
            return (
                False,
                f"Too many restarts ({len(recent_restarts)}/{self.max_restarts_per_hour} per hour)",
            )
    ...
    def _update_interval_statistics(self) -> None:
        ...
        for i in range(1, len(self.restart_history)):
            interval = (
                self.restart_history[i].timestamp
                - self.restart_history[i - 1].timestamp
            ) / 60
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            self.statistics.successful_restarts = stats_data.get(
                "successful_restarts", 0
            )
            self.statistics.failed_restarts = stats_data.get("failed_restarts", 0)
            self.statistics.consecutive_failures = stats_data.get(
