...
class HealthMonitor(BaseService):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self._verify_system_capabilities():
                self.log_warning(
                    "Some system capabilities unavailable, running in degraded mode"
                )
    ...
    async def check_health(self) -> SystemHealth:
        ...
        health = SystemHealth(status=status, checks=checks, timestamp=start_time)
    ...
    async def validate_before_start(self) -> tuple[bool, str]:
        ...
        if mem.available < 500 * 1024 * 1024:  # Less than 500MB
            return (
                False,
                f"Insufficient memory: {mem.available / (1024*1024):.0f}MB available",
            )
        ...
        if disk.free < self.min_disk_space_gb * 1024 * 1024 * 1024:
            return (
                False,
                f"Insufficient disk space: {disk.free / (1024*1024*1024):.1f}GB available",
            )
                ...
                try:
                    ...
                    if self.check_history and len(self.check_history) > 1:
                        ...
                        if health.status != prev_status:
                            self.log_info(
                                f"Health status changed: {prev_status.value} -> {health.status.value}"
                            )
