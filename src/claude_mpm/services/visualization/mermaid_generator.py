...
class MermaidGeneratorService(SyncBaseService):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._reserved_keywords = {
            'graph', 'subgraph', 'end', 'class', 'classDef', 'click',
            'style', 'linkStyle', 'interpolate', 'flowchart', 'pie',
            'sequenceDiagram', 'gantt', 'stateDiagram', 'erDiagram',
            'journey', 'gitGraph', 'mindmap', 'timeline', 'quadrantChart',
            'sankey', 'xychart', 'block', 'start', 'stop', 'operation',
            'subroutine', 'condition', 'inputoutput', 'parallel', 'database'
        }
    ...
    def _generate_entry_points_diagram(
        ...
        for entry_type, entries in entry_points.items():
            ...
            lines.append(f"    subgraph {subgraph_id}[{entry_type.replace('_', ' ').title()}]")
            ...
            for entry in entries:
                if isinstance(entry, dict):
                    ...
                    try:
                        path_name = Path(file_path).name if file_path != "unknown" else "unknown"
        ...
        for module, deps in dependencies.items():
            ...
            if isinstance(deps, list):
                for dep in deps:
                    ...
                    if not config.include_external and self._is_external_module(dep_name):
                        continue
                ...
                for imp in imports:
                    if isinstance(imp, dict):
                        ...
                        if import_from:
                            ...
                            if not config.include_external and self._is_external_module(import_name):
                                continue
            ...
            if methods:
                for method in methods[:10]:  # Limit to avoid clutter
                    if isinstance(method, dict):
                        ...
                        if config.show_return_types:
                            lines.append(f"        {visibility}{method_sig}: {return_type}")
                        else:
            ...
            if associations:
                for assoc in associations:
                    if isinstance(assoc, str):
                        ...
                        if safe_assoc in [self._sanitize_class_name(c) for c in classes]:
        ...
        try:
            ...
            if "subgraph" in diagram:
                ...
                if subgraph_count > end_count:
                    return False, f"Unmatched subgraph blocks: {subgraph_count} subgraphs, {end_count} ends"
