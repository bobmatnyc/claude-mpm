...
class PortManager:
    ...
    def find_available_port(self, preferred_port: Optional[int] = None) -> Optional[int]:
        ...
        if preferred_port and preferred_port in self.PORT_RANGE:
            if self.is_port_available(preferred_port):
                ...
                if not is_ours:
                    ...
                    self.logger.warning(f"Port {preferred_port} is already used by claude-mpm instance: {instance_info}")
        ...
        if self.is_port_available(self.DEFAULT_PORT):
            ...
            if not is_ours:
                ...
                self.logger.info(f"Default port {self.DEFAULT_PORT} is already used by claude-mpm instance")
        ...
        self.logger.error(f"No available ports in range {self.PORT_RANGE.start}-{self.PORT_RANGE.stop-1}")
        return None
    ...
    def register_instance(self, port: int, pid: int, host: str = "localhost") -> str:
        ...
        self.logger.info(f"Registered SocketIO instance {instance_id} on port {port} (PID: {pid})")
        return instance_id
    ...
    def remove_instance(self, instance_id: str) -> bool:
        ...
        if instance_id in instances:
            ...
            self.save_instances(instances)
            self.logger.info(f"Removed SocketIO instance {instance_id} (port: {instance_info.get('port')})")
            return True
