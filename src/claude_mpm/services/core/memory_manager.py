...
class MemoryManager(IMemoryManager):
        ...
        if cached_memories is not None:
            ...
            if agent_name and "agent_memories" in cached_memories:
                if agent_name in cached_memories["agent_memories"]:
                    ...
                    return {"actual_memories": cached_memories.get("actual_memories", ""), "agent_memories": {}}
        ...
        if agent_name and "agent_memories" in result:
            if agent_name in result["agent_memories"]:
                return {
                    ...
                    "agent_memories": {agent_name: result["agent_memories"][agent_name]}
                }
            else:
                return {"actual_memories": result.get("actual_memories", ""), "agent_memories": {}}
    ...
    def save_memory(self, key: str, value: Any, agent_name: Optional[str] = None) -> None:
        ...
        if memory_file.exists():
            ...
            lines = [f"# {'Agent Memory: ' + agent_name if agent_name else 'PM Memory'}", ""]
    ...
    def search_memories(self, query: str, agent_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if "actual_memories" in memories and memories["actual_memories"]:
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        stats["pm_memory_size"] = len(memories.get("actual_memories", "").encode("utf-8"))
    ...
    def _load_actual_memories(self, deployed_agents: Set[str]) -> Dict[str, Any]:
        ...
        if self._stats["loaded_count"] > 0 or self._stats["skipped_count"] > 0:
            ...
            if self._stats["skipped_count"] > 0:
                summary_parts.append(f"{self._stats['skipped_count']} non-deployed agent memories skipped")
    ...
    def _load_memories_from_directory(
        ...
        if pm_memory_path.exists():
            try:
                ...
                self.logger.error(f"Failed to load PM memory from {pm_memory_path}: {e}")
        ...
        for memory_file in memories_dir.glob("*_memories.md"):
            ...
            if agent_name in deployed_agents:
                try:
                    ...
                    self.logger.error(f"Failed to load agent memory from {memory_file}: {e}")
            else:
