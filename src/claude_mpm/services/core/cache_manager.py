...
class CacheManager(ICacheManager):
    ...
    def __init__(
        ...
        self._fs_cache = FileSystemCache(
            ...
            default_ttl=max(capabilities_ttl, deployed_agents_ttl, metadata_ttl, memories_ttl)
        )
    ...
    def get_capabilities(self) -> Optional[str]:
        ...
        with self._lock:
            if self._capabilities_cache is not None and \
               self.is_cache_valid(self._capabilities_cache_time, self.capabilities_ttl):
    ...
    def get_deployed_agents(self) -> Optional[Set[str]]:
        ...
        with self._lock:
            if self._deployed_agents_cache is not None and \
               self.is_cache_valid(self._deployed_agents_cache_time, self.deployed_agents_ttl):
                age = time.time() - self._deployed_agents_cache_time
                self.logger.debug(f"Cache hit: deployed agents (age: {age:.1f}s)")
                return self._deployed_agents_cache.copy()  # Return a copy to prevent external modification
    ...
    def set_deployed_agents(self, agents: Set[str]) -> None:
        ...
        with self._lock:
            ...
            self.logger.debug(f"Updated deployed agents cache with {len(agents)} agents")
    ...
    def get_memories(self) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            if self._memories_cache is not None and \
               self.is_cache_valid(self._memories_cache_time, self.memories_ttl):
        ...
        with self._lock:
            ...
            stats = {
                "capabilities": {
                    ...
                    "age": current_time - self._capabilities_cache_time if self._capabilities_cache else None,
                    "valid": self.is_cache_valid(self._capabilities_cache_time, self.capabilities_ttl)
                    if self._capabilities_cache else False
                },
                "deployed_agents": {
                    "cached": self._deployed_agents_cache is not None,
                    "count": len(self._deployed_agents_cache) if self._deployed_agents_cache else 0,
                    "age": current_time - self._deployed_agents_cache_time if self._deployed_agents_cache else None,
                    "valid": self.is_cache_valid(self._deployed_agents_cache_time, self.deployed_agents_ttl)
                    if self._deployed_agents_cache else False
                },
                "metadata": {
                    "entries": len(self._agent_metadata_cache)
                },
                "memories": {
                    "cached": self._memories_cache is not None,
                    "age": current_time - self._memories_cache_time if self._memories_cache else None,
                    "valid": self.is_cache_valid(self._memories_cache_time, self.memories_ttl)
                    if self._memories_cache else False
                },
