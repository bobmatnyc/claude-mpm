...
class IMemoryOutputFormatter(ABC):
    ...
    @abstractmethod
    def format_memory_view(self, agent_id: str, memory_content: str, 
    ...
    def format_optimization_results(self, results: Dict[str, Any], 
                                   is_single_agent: bool = True) -> str:
        """Format optimization output."""
        pass
    
    @abstractmethod
    def format_cross_reference(self, cross_ref_data: Dict[str, Any], 
    ...
    @abstractmethod
    def format_agent_memories_summary(self, agent_memories: Dict[str, Dict], 
...
class MemoryOutputFormatter(IMemoryOutputFormatter):
    ...
    def format_status(self, status_data: Dict[str, Any], verbose: bool = False) -> str:
        ...
        if not status_data.get("success", True):
            lines.append(f"{self.emojis['error']} Error getting status: {status_data.get('error', 'Unknown error')}")
            return "\n".join(lines)
        ...
        lines.append(f"{self.emojis['memory']} Memory System Health: {health_emoji} {system_health}")
        
        # Handle null/None values properly
        memory_dir = status_data.get('memory_directory')
        lines.append(f"{self.emojis['file']} Memory Directory: {memory_dir if memory_dir is not None else 'Unknown'}")
        
        lines.append(f"{self.emojis['optimize']} System Enabled: {'Yes' if status_data.get('system_enabled', True) else 'No'}")
        lines.append(f"{self.emojis['build']} Auto Learning: {'Yes' if status_data.get('auto_learning', True) else 'No'}")
        
        total_agents = status_data.get('total_agents')
        lines.append(f"{self.emojis['stats']} Total Agents: {total_agents if total_agents is not None else 'Unknown'}")
        
        lines.append(f"{self.emojis['disk']} Total Size: {status_data.get('total_size_kb', 0):.1f} KB")
        lines.append("")
        
        # Optimization opportunities
        opportunities = status_data.get("optimization_opportunities", [])
        if opportunities:
            lines.append(f"{self.emojis['warning']} Optimization Opportunities ({len(opportunities)}):")
        ...
        if agents:
            ...
            for agent_id, agent_info in sorted(agents.items()):
                if "error" in agent_info:
                    lines.append(f"   {self.emojis['error']} {agent_id}: Error - {agent_info['error']}")
                    continue
                ...
                lines.append(f"      Size: {size_kb:.1f} KB / {size_limit} KB ({utilization:.1f}%)")
    ...
    def format_memory_view(self, agent_id: str, memory_content: str, 
        ...
        if not memory_content:
            lines.append(f"{self.emojis['empty']} No memory found for agent: {agent_id}")
            return "\n".join(lines)
    ...
    def format_optimization_results(self, results: Dict[str, Any], 
        ...
        if is_single_agent:
            ...
            lines.append(f"{self.emojis['success']} Optimization completed for {agent_id}")
            lines.append(f"   Original size: {original_size:,} bytes")
            lines.append(f"   Optimized size: {optimized_size:,} bytes")
            lines.append(f"   Size reduction: {size_reduction:,} bytes ({size_reduction_percent}%)")
            ...
            lines.append(f"   Total size before: {summary.get('total_size_before', 0):,} bytes")
            lines.append(f"   Total size after: {summary.get('total_size_after', 0):,} bytes")
            lines.append(f"   Total reduction: {summary.get('total_size_reduction', 0):,} bytes ({summary.get('total_size_reduction_percent', 0)}%)")
            lines.append(f"   Total duplicates removed: {summary.get('total_duplicates_removed', 0)}")
            lines.append(f"   Total items consolidated: {summary.get('total_items_consolidated', 0)}")
    ...
    def format_cross_reference(self, cross_ref_data: Dict[str, Any], 
        ...
        if query:
            ...
            lines.append(f"{self.emojis['search']} Analyzing all agent memories for patterns...")
        
        if cross_ref_data.get("success") is False:
            lines.append(f"{self.emojis['error']} Analysis failed: {cross_ref_data.get('error', 'Unknown error')}")
            return "\n".join(lines)
    ...
    def format_build_results(self, results: Dict[str, Any]) -> str:
        ...
        if results.get("success"):
            lines.append(f"{self.emojis['success']} Successfully processed documentation")
            ...
            lines.append(f"   Agents affected: {results.get('total_agents_affected', 0)}")
            
            if results.get("agents_affected"):
                lines.append(f"   Affected agents: {', '.join(results['agents_affected'])}")
            ...
            if files_results:
                ...
                for file_path, file_result in files_results.items():
                    if file_result.get("success"):
                        ...
                        lines.append(f"   {file_path}: {extracted} items extracted, {created} memories created")
            ...
            lines.append(f"{self.emojis['error']} Build failed: {results.get('error', 'Unknown error')}")
    ...
    def format_agent_memories_summary(self, agent_memories: Dict[str, Dict], 
        ...
        lines.append(f"{self.emojis['stats']} Found memories for {len(agent_memories)} agents")
        lines.append("")
        ...
        for agent_id, sections in sorted(agent_memories.items()):
            ...
            lines.append(f"   {self.emojis['build']} {len(sections)} sections, {item_count} total items")
        ...
        lines.append(f"{self.emojis['stats']} Total: {total_items} memory items across {len(agent_memories)} agents")
        ...
        if len(agent_memories) > 1:
            ...
            if common_patterns:
                ...
                for pattern, count, agents in common_patterns[:5]:
                    lines.append(f"   • {pattern[:80]}{'...' if len(pattern) > 80 else ''}")
    ...
    def _find_common_patterns(self, agent_memories: Dict[str, Dict]) -> List[Tuple[str, int, List[str]]]:
