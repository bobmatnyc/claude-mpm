...
class IMemoryCRUDService(ABC):
    ...
    @abstractmethod
    def create_memory(self, agent_id: str, template_type: str = "default") -> Dict[str, Any]:
        """
...
class MemoryCRUDService(IMemoryCRUDService):
        ...
        try:
            ...
            if existing_content:
                return {
                    ...
                    "agent_id": agent_id,
                    "existing_file": str(memory_manager.memories_dir / f"{agent_id}_memories.md")
                }
            ...
            self.logger.error(f"Error creating memory for {agent_id}: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "agent_id": agent_id
    ...
    def update_memory(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Added learning to {agent_id} memory in section: {section_name}")
                return {
                    "success": True,
                    "agent_id": agent_id,
            ...
            self.logger.error(f"Error updating memory for {agent_id}: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "agent_id": agent_id,
    ...
    def delete_memory(self, agent_id: str, confirm: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error deleting memory for {agent_id}: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "agent_id": agent_id
