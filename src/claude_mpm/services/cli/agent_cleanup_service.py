...
class IAgentCleanupService(ABC):
    ...
    @abstractmethod
    def clean_deployed_agents(self, agents_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    @abstractmethod
    def get_orphaned_agents(self, agents_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
...
class AgentCleanupService(IAgentCleanupService):
    ...
    def validate_cleanup(self, agents_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not agents_dir.exists():
                validation["info"].append(f"Agents directory does not exist: {agents_dir}")
                return validation
