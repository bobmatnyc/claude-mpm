...
class AgentOutputFormatter(IAgentOutputFormatter):
    ...
    def format_agent_details(
        ...
        if output_format == "json":
            ...
            # Specializations
            if 'specializations' in agent and agent['specializations']:
            ...
            if verbose:
                if 'dependencies' in agent:
                    ...
                    if 'system' in deps and deps['system']:
            ...
            if 'missing' in dependencies:
                if dependencies['missing'].get('python'):
                    ...
                    if len(dependencies['missing']['python']) > 5:
                        lines.append(f"   ... and {len(dependencies['missing']['python']) - 5} more")
    ...
    def format_cleanup_result(
        ...
        if output_format == "json":
            ...
            if 'orphaned' in result and result['orphaned']:
            ...
            if dry_run:
                ...
                # Removed agents
                if 'removed' in result and result['removed']:
    ...
    def _format_agents_as_table(
        ...
        if quiet:
            headers = ["Name"]
            rows = [[agent.get('name', agent.get('file', 'Unknown'))] for agent in agents]
        elif verbose:
            ...
            for agent in agents:
                rows.append([
                    ...
                    agent.get('version', '-'),
                    agent.get('tier', '-'),
                    agent.get('description', '-')[:50],  # Truncate long descriptions
            ...
            for agent in agents:
                rows.append([
                    agent.get('name', agent.get('file', 'Unknown')),
                    agent.get('version', '-'),
                    agent.get('description', '-')[:60]  # Truncate long descriptions
                ])
