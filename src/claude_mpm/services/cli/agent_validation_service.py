...
from ...agents.frontmatter_validator import FrontmatterValidator, ValidationResult
...
class AgentValidationService(IAgentValidationService):
    ...
    def validate_agent(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error validating agent {agent_name}: {e}", exc_info=True)
            return {
                "success": False,
                "agent": agent_name,
                "error": str(e)
    ...
    def fix_agent_frontmatter(
        ...
        try:
            ...
            self.logger.error(
                f"Error fixing agent {agent_name}: {e}", exc_info=True
            )
            return {
                "success": False,
                "agent": agent_name,
        ...
        try:
            ...
            for agent_id, metadata in all_agents.items():
                ...
                agent_result = {
                    ...
                    "corrections_made": len(fix_result.corrections) if not dry_run else 0,
                    "corrections_available": len(fix_result.corrections) if dry_run else 0
                }
    ...
    def check_agent_integrity(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking integrity for agent {agent_name}: {e}",
                exc_info=True
            )
            return {
                "success": False,
        ...
        try:
            ...
            for agent_id, metadata in all_agents.items():
                ...
                if agent_name in name_to_agents:
                    ...
                    if name_to_agents[agent_name] not in deployment_state["duplicate_names"][agent_name]:
                ...
                if path_str in path_to_agents:
                    ...
                    if path_to_agents[path_str] not in deployment_state["conflicting_paths"][path_str]:
            ...
            return {
                ...
                "summary": {
                    ...
                    "healthy_agents": deployment_state["files_found"] - len(deployment_state["duplicate_names"]) - len(deployment_state["conflicting_paths"]),
            ...
            self.logger.error(
                f"Error validating deployment state: {e}", exc_info=True
            )
            return {
                "success": False,
                "error": str(e)
