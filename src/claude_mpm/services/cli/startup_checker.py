...
class IStartupChecker(ABC):
    ...
    @abstractmethod
    def get_startup_warnings(self, resume_enabled: bool = False) -> List['StartupWarning']:
...
class StartupCheckerService(IStartupChecker):
    ...
    def _check_required_directories(self) -> List[StartupWarning]:
        ...
        if not claude_dir.exists():
            self.logger.debug(".claude directory does not exist (will be created on first use)")
        elif not claude_dir.is_dir():
            warnings.append(StartupWarning(
                category='environment',
                message=f".claude path exists but is not a directory",
                severity='warning'
            ))
