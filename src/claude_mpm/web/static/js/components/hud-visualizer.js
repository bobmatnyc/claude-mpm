...
class HUDVisualizer {
    ...
    createNodeFromEvent(event, sessionId) {
        ...
        if (eventType === 'session' && subtype === 'started') {
            ...
            nodeId = `user-prompt-${timestamp.getTime()}`;
            
        } else if (eventType === 'hook' && subtype === 'claude_response') {
            // Claude responses
            nodeType = 'PM';
            label = 'Claude Response';
            nodeId = `claude-response-${timestamp.getTime()}`;
            ...
            nodeId = `log-${level}-${timestamp.getTime()}`;
            
        } else {
            // Generic event node
            nodeType = 'PM';
            label = eventType || 'Event';
            nodeId = `generic-${eventType}-${timestamp.getTime()}`;
        }
