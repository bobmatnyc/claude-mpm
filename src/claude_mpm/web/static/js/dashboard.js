...
class Dashboard {
    ...
    initializeHUDManager() {
        this.hudManager = new HUDManager(this.eventViewer, this.sessionManager);
        
        // Backward compatibility
        this.hudVisualizer = this.hudManager.getHUDVisualizer();
    ...
    setupModuleInteractions() {
        ...
        this.socketManager.onEventUpdate((events) => {
            ...
            // Handle HUD events
            this.hudManager.handleHUDEvents(events);
    ...
    renderAgents() {
        ...
        this.updateAgentsFilterDropdowns(events);
    }
    ...
    renderTools() {
        ...
        const toolHTML = this.eventProcessor.generateToolHTML(toolCallsArray);
        
        toolsList.innerHTML = toolHTML;
        this.exportManager.scrollListToBottom('tools-list');
        
        // Update filter dropdowns
        this.updateToolsFilterDropdowns(toolCallsArray);
    }
    ...
    renderFiles() {
        ...
        const fileHTML = this.eventProcessor.generateFileHTML(filesArray);
    ...
    /**
     * Update agents filter dropdowns
     */
    updateAgentsFilterDropdowns(events) {
        const agentTypes = new Set();
        
        events.forEach(event => {
            // Get inference for this event to find the proper agent name
            ...
            if (inference && inference.agentName && inference.agentName !== 'Unknown') {
                agentTypes.add(inference.agentName);
            } else {
                // Fallback to direct field extraction
                const data = event.data || {};
                const agentName = event.subagent_type || data.subagent_type || 
                                event.agent_type || data.agent_type || 
                                event.agent_id || data.agent_id ||
                                data.delegation_details?.agent_type;
                
                if (agentName && agentName !== 'unknown' && agentName !== 'Unknown') {
                    agentTypes.add(agentName);
                }
            }
        });
    ...
    /**
     * Toggle HUD (backward compatibility)
     */
    toggleHUD() {
    ...
    /**
     * Get HUD mode state (backward compatibility)
     */
    get hudMode() {
        return this.hudManager.isHUDMode();
    }

    /**
     * Set HUD mode state (backward compatibility)
     */
    set hudMode(value) {
        if (value !== this.hudManager.isHUDMode()) {
            this.hudManager.toggleHUD();
        }
    }
